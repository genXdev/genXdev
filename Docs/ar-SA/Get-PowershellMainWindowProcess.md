...
> يُرجع كائن العملية للنافذة التي تستضيف محطة PowerShell.

## Description

* يجتاز شجرة العمليات بدءًا من عملية PowerShell الحالية
  لتحديد النافذة الأم المسؤولة عن استضافة الطرفية.
* إذا لم تكن لعملية الأصل المباشر مقبض نافذة رئيسية، فإنه
  يبحث عن عمليات مماثلة تحتوي على نوافذ رئيسية.
* مفيد لتحديد عملية نافذة الطرفية الفعلية (مثل Windows
  Terminal، ConHost، إلخ) التي تحتوي على جلسة PowerShell.
...
يُرجع العملية التي تستضيف جلسة PowerShell الحالية ويعرض اسمها.
