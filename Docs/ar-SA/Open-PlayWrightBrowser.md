...
> يبدأ تشغيل متصفح يديره Playwright مع ملف تعريف مستخدم دائم.
...
يقوم بتشغيل مثيل متصفح مدعوم من Playwright. تحدد معلمة BrowserType محرك المتصفح ووضع التشغيل المستخدم:

- ChromeNormal / EdgeNormal: يقوم بتشغيل Chrome أو Edge المثبت على نظام التشغيل عبر واجهة قناة Playwright، متجنبًا شريط المعلومات "مسيطر عليه بواسطة الأتمتة" ومشاكل اكتشاف مكافحة البوت.
- ChromiumNormal: يكتشف تلقائيًا Chrome أو Edge (أيهما مثبت ومعين كمتصفح افتراضي للنظام) ويستخدم واجهة القناة. يتراجع إلى Chromium المضمن في Playwright إذا لم يكن أي منهما مثبتًا.
- ChromiumPlaywright: يستخدم دائمًا ثنائي Chromium المضمن في Playwright (وليس متصفح نظام التشغيل أبدًا).
- FirefoxPlaywright: يستخدم ثنائي Firefox المضمن في Playwright. واجهة القناة غير متاحة لـ Firefox في روابط Playwright .NET.
- WebKitPlaywright: يستخدم ثنائي WebKit المضمن في Playwright.

إجراءات مكافحة الاكتشاف المطبقة للمتصفحات القائمة على Chromium:
- تعطيل علامات أتمتة blink (إزالة شريط المعلومات)
- ضبط منفذ العرض على null والبدء في وضع ملء الشاشة لحجم النافذة الأصلي
- استخدام نظام ألوان 'بدون تفضيل' لتجنب الخلفية البيضاء
- تعيين وسائط المتصفح القياسية (عدم التشغيل الأول، عدم التحقق من المتصفح الافتراضي)

يتم تخزين الملفات الشخصية الدائمة لكل BrowserType تحت GenXdev AppData. يتم تخزين المتصفح الذي تم تشغيله وسياقه/صفحته في $Global:GenXdevPlaywright لاستخدامها من قبل cmdlets الأخرى.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-BrowserType` | String | — | — | 0 | `'ChromiumNormal'` | المتصفح الذي سيتم تشغيله: كروم، إيدج، كروميوم (يكتشف تلقائيًا كروم أو إيدج)، فايرفوكس، أو ويب كيت |
| `-Headless` | SwitchParameter | — | — | Named | — | قم بتشغيل المتصفح بدون نافذة مرئية |
| `-Proxy` | String | — | — | Named | — | عنوان URL للخادم الوكيل (على سبيل المثال http://proxy:8080) |
| `-AcceptLang` | String | — | — | Named | — | تعيين رأس HTTP للغة المقبولة من المتصفح |
| `-Width` | Int32 | — | — | Named | `-1` | عرض إطار العرض الأولي بالبكسل |
| `-Height` | Int32 | — | — | Named | `-1` | ارتفاع إطار العرض الأولي بالبكسل |
| `-Force` | SwitchParameter | — | — | Named | — | إغلاق المتصفح الحالي والبدء من جديد |
...
| `-Fullscreen` | SwitchParameter | — | — | Named | — | تكبير الإطار ليملأ الشاشة بأكملها |
...
| `-Minimize` | SwitchParameter | — | — | Named | — | تقليل حجم النافذة بعد تحديد الموضع |
...
| `-PassThru` | SwitchParameter | — | — | Named | — | إرجاع كائن مساعد النافذة لمزيد من التلاعب |
...
| `-OnlyOutputCoords` | SwitchParameter | — | — | Named | — | قم بإخراج الإحداثيات والحجم المحسوب فقط دون وضع النافذة فعليًا |
...
> المتصفح الذي سيتم تشغيله: كروم، إيدج، كروميوم (يكتشف تلقائيًا كروم أو إيدج)، فايرفوكس، أو ويب كيت
...
> عنوان URL للخادم الوكيل (على سبيل المثال http://proxy:8080)
...
> تعيين رأس HTTP للغة المقبولة من المتصفح
...
> ارتفاع إطار العرض الأولي بالبكسل
...
> إغلاق المتصفح الحالي والبدء من جديد
...
> تكبير الإطار ليملأ الشاشة بأكملها
...
> تقليل حجم النافذة بعد تحديد الموضع
...
> إرجاع كائن مساعد النافذة لمزيد من التلاعب
...
> قم بإخراج الإحداثيات والحجم المحسوب فقط دون وضع النافذة فعليًا
