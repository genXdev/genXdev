...
> يفتح استعلام بحث في محرك البحث المحدد باستخدام متصفح ويب.

## Description

توفر هذه الوظيفة واجهة موحدة للبحث عبر محركات بحث متعددة بما في ذلك جوجل، بينج، جيت هب، ويكيبيديا، يوتيوب، وغيرها. تدعم خيارات واسعة لتكوين المتصفح بما في ذلك تحديد موضع النافذة، الحجم، اختيار المتصفح، وتفضيلات اللغة. تقوم الوظيفة بتوجيه الاستعلامات ديناميكيًا إلى وظائف محرك البحث المناسبة.
...
| `-EndPoint` | String | — | — | 1 | `'Google'` | النقطة النهائية لاستدعاء الاستعلام عليها |
| `-Language` | String | — | — | 2 | — | لغة نتائج البحث المُعادة |
| `-Monitor` | Int32 | — | — | 3 | `-1` | الشاشة المستخدمة، 0 = الشاشة الافتراضية، -1 = تجاهل، -2 = الشاشة الثانوية المُهيأة، القيمة الافتراضية هي -1، بدون تحديد موضع |
...
يبحث عن "أوامر PowerShell" على Google مع نتائج باللغة الإنجليزية.
...
يستخدم الاسم المستعار 'q' للبحث عن 'GitHub PowerShell' في Bing باستخدام Chrome في الوضع الخاص.
