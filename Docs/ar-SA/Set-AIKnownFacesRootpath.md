...
> يضبط دليل ملفات صور الوجوه المستخدمة في عمليات GenXdev.AI.

## Description

تقوم هذه الوظيفة بتكوين دليل الوجوه العمومي المستخدم بواسطة وحدة GenXdev.AI لمختلف عمليات التعرف على الوجوه والذكاء الاصطناعي. يمكن تخزين الإعدادات بشكل دائم في التفضيلات (افتراضي)، أو فقط في الجلسة الحالية (باستخدام -SessionOnly)، أو مسحها من الجلسة (باستخدام -ClearSession).
...
يضبط دليل الوجوه باستمرار في التفضيلات.
...
يضبط دليل الوجوه للجلسة الحالية فقط (متغير عام).
...
يمسح إعداد دليل وجوه الجلسة (المتغير العام) دون التأثير على التفضيلات الثابتة.
