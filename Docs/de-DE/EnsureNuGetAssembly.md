...
Diese Funktion stellt sicher, dass die angegebenen NuGet-Paketassemblys heruntergeladen und in die aktuelle PowerShell-Sitzung geladen werden. Sie unterstützt die automatische Erkennung und das Laden mit einem Rückgriff auf ein JSON-Manifest für Konfigurationsüberschreibungen und behandelt Randfälle wie Stub-Abhängigkeiten (z. B. SQLite). Die Installation verwendet ausschließlich die dotnet-CLI für zuverlässiges Paketmanagement und lädt Assemblys direkt aus dem globalen NuGet-Cache, ohne sie zu kopieren. Lazy Loading prüft, ob der Typ bereits verfügbar ist, bevor fortgefahren wird.
...
| `-ManifestPath` | String | — | — | Named | — | Pfad zu packages.json; standardmäßig Modulstamm, wenn ausgelassen. |
| `-Version` | String | — | — | Named | — | Spezifische Version; falls nicht angegeben, wird die höchste aus JSON oder die neueste verwendet. |
| `-TypeName` | String | — | — | Named | — | TypeName zur Überprüfung des Ladens. |
| `-ForceLatest` | SwitchParameter | — | — | Named | — | Bei Fehler auf die neueste Version zurückgreifen. |
| `-Destination` | String | — | — | Named | — | Benutzerdefiniertes Installationsziel; standardmäßig lokaler persistenter oder globaler Cache. |
| `-Description` | String | — | — | Named | — | Optionale Beschreibung der Software und ihres Zwecks für die Einwilligung. |
| `-Publisher` | String | — | — | Named | — | Optionaler Herausgeber oder Anbieter der Software für die Zustimmung. |
| `-ForceConsent` | SwitchParameter | — | — | Named | — | Erzwinge eine Eingabeaufforderung, auch wenn die Einstellung auf Zustimmung festgelegt ist. |
...
> Paketschlüssel aus packages.json oder direkte NuGet-Paket-ID.
...
> Pfad zu packages.json; standardmäßig Modulstamm, wenn ausgelassen.
...
> Spezifische Version; falls nicht angegeben, wird die höchste aus JSON oder die neueste verwendet.
...
> TypeName zur Überprüfung des Ladens.
...
> Bei Fehler auf die neueste Version zurückgreifen.
...
> Benutzerdefiniertes Installationsziel; standardmäßig lokaler persistenter oder globaler Cache.
...
> Optionale Beschreibung der Software und ihres Zwecks für die Einwilligung.
...
> Optionaler Herausgeber oder Anbieter der Software für die Zustimmung.
...
> Erzwinge eine Eingabeaufforderung, auch wenn die Einstellung auf Zustimmung festgelegt ist.
