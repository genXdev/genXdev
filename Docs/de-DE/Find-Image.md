...
> Durchsucht Verzeichnisse nach Bilddateien und Metadaten mit Filterfunktionen und optionaler browserbasierter Galerieanzeige.

## Description

Sucht nach Bilddateien (jpg, jpeg, png, gif, bmp, webp, tiff, tif) im angegebenen Verzeichnis und seinen Unterverzeichnissen. Für jedes Bild werden die zugehörigen description.json-, keywords.json-, people.json- und objects.json-Dateien auf Metadaten überprüft. Kann Bilder basierend auf Schlüsselwortübereinstimmungen, Personenerkennung und Objekterkennung filtern und die Ergebnisse als Objekte zurückgeben. Verwenden Sie -ShowInBrowser, um die Ergebnisse in einem browserbasierten Masonry-Layout anzuzeigen.

Parameterlogik:
- Innerhalb jedes Parametertyps (Schlüsselwörter, Personen, Objekte usw.): ODER-Logik
  Beispiel: -Keywords "Katze","Hund" findet Bilder mit Katze ODER Hund
- Zwischen verschiedenen Parametertypen: UND-Logik
  Beispiel: -Keywords "Katze" -People "Hans" findet Bilder mit Katze UND Hans
- EXIF-Bereichsparameter: Geben Sie [min, max]-Werte zum Filtern von Bereichen an
- Zeichenfolgenparameter: Unterstützen Platzhaltervergleiche mit * und ?

Die Funktion durchsucht Bildverzeichnisse und untersucht alternative Datenströme, die Metadaten im JSON-Format enthalten. Sie kann Schlüsselwörter mit Platzhaltermustern abgleichen, nach bestimmten Personen filtern und nach erkannten Objekten suchen. Standardmäßig werden Bilddatenobjekte zurückgegeben. Verwenden Sie -ShowInBrowser zur Anzeige in einem Webbrowser.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Any` | String[] | — | — | 0 | `@()` | Entspricht einem beliebigen aller möglichen Metadatentypen. |
...
| `-DescriptionSearch` | String[] | — | — | Named | `@()` | ('Der zu suchende Beschreibungstext, Platzhalter sind zulässig.') |
| `-Keywords` | String[] | — | — | Named | `@()` | Die Schlüsselwörter, nach denen gesucht werden soll, Platzhalter sind zulässig. |
| `-People` | String[] | — | — | Named | `@()` | Personen, nach denen gesucht werden soll, Platzhalter zulässig. |
| `-Objects` | String[] | — | — | Named | `@()` | Objekte, nach denen gesucht werden soll, Platzhalter sind erlaubt. |
| `-Scenes` | String[] | — | — | Named | `@()` | Nach Szenenkategorien suchen, Platzhalter erlaubt. |
| `-PictureType` | String[] | — | — | Named | `@()` | Bildtyp zum Filtern (z.B. 'Tageslicht', 'Abend', 'Innenraum' usw.). Unterstützt Platzhalter. |
| `-StyleType` | String[] | — | — | Named | `@()` | Stiltyp zum Filtern (z. B. 'casual', 'formal' usw.). Unterstützt Platzhalter. |
| `-OverallMood` | String[] | — | — | Named | `@()` | Gesamtstimmung zum Filtern (z. B. „ruhig“, „fröhlich“, „traurig“ usw.). Unterstützt Platzhalter. |
| `-MetaCameraMake` | String[] | — | — | Named | `@()` | Filtert nach Kamerahersteller in den EXIF-Metadaten von Bildern. Unterstützt Platzhalter. |
| `-MetaCameraModel` | String[] | — | — | Named | `@()` | Filter nach Kameramodell in den EXIF-Metadaten des Bildes. Unterstützt Platzhalter. |
| `-MetaGPSLatitude` | Double[] | — | — | Named | — | Nach GPS-Breitengradbereich in den EXIF-Metadaten von Bildern filtern. |
| `-MetaGPSLongitude` | Double[] | — | — | Named | — | Nach GPS-Längengradbereich in den EXIF-Metadaten von Bildern filtern. |
| `-MetaGPSAltitude` | Double[] | — | — | Named | — | Nach GPS-Höhenbereich in Bild-EXIF-Metadaten filtern (in Metern). |
| `-GeoLocation` | Double[] | — | — | Named | — | Geografische Koordinaten [Breitengrad, Längengrad] für die Suche in der Nähe. |
| `-GeoDistanceInMeters` | Double | — | — | Named | `1000` | Maximale Entfernung in Metern vom GeoLocation, um nach Bildern zu suchen. |
| `-MetaExposureTime` | Double[] | — | — | Named | — | Nach Belichtungszeitbereich in Bild-EXIF-Metadaten filtern (in Sekunden). |
| `-MetaFNumber` | Double[] | — | — | Named | — | Nach F-Nummer (Blendenbereich) in den EXIF-Metadaten von Bildern filtern. |
| `-MetaISO` | Int32[] | — | — | Named | — | Nach ISO-Empfindlichkeitsbereich in den EXIF-Metadaten von Bildern filtern. |
| `-MetaFocalLength` | Double[] | — | — | Named | — | Nach Brennweitenbereich in den EXIF-Metadaten des Bildes filtern (in mm). |
| `-MetaWidth` | Int32[] | — | — | Named | — | Nach Bildbreitenbereich in Pixeln aus EXIF-Metadaten filtern. |
| `-MetaHeight` | Int32[] | — | — | Named | — | Nach Bildhöhe in Pixeln aus EXIF-Metadaten filtern. |
| `-MetaDateTaken` | DateTime[] | — | — | Named | — | Nach Aufnahmedatum aus EXIF-Metadaten filtern. Kann ein Datumsbereich sein. |
...
| `-KeysToSend` | String[] | — | — | Named | — | 'An das Browserfenster zu sendende Tastenanschläge, ' +
                'siehe Dokumentation zum Cmdlet GenXdev\Send-Key' |
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | Steuerzeichen und Modifikatoren beim Senden von Tasten maskieren |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | Verhindern, dass der Tastaturfokus nach dem Senden von Tasten
wieder an PowerShell zurückgegeben wird |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | Verwenden Sie Umschalt+Eingabe anstelle von Eingabe beim Senden von Tasten |
...
| `-Monitor` | Int32 | — | — | Named | `-2` | Der zu verwendende Monitor, 0 = Standard, -1 = verwerfen, -2 = konfigurierter sekundärer Monitor, Standard ist `Global:DefaultSecondaryMonitor oder 2, falls nicht gefunden |
...
| `-EmbedImages` | SwitchParameter | — | — | Named | — | Betten Sie Bilder als base64 ein. |
| `-NoFallback` | SwitchParameter | — | — | Named | — | Schalter zum Deaktivieren des Fallback-Verhaltens. |
| `-NeverRebuild` | SwitchParameter | — | — | Named | — | Option zum Überspringen der Datenbankinitialisierung und -neuerstellung. |
...
| `-HasNudity` | SwitchParameter | — | — | Named | — | Bilder filtern, die Nacktheit enthalten. |
| `-NoNudity` | SwitchParameter | — | — | Named | — | Filtere Bilder, die KEINE Nacktheit enthalten. |
| `-HasExplicitContent` | SwitchParameter | — | — | Named | — | Filtert Bilder, die explizite Inhalte enthalten. |
| `-NoExplicitContent` | SwitchParameter | — | — | Named | — | Filtert Bilder, die KEINE expliziten Inhalte enthalten. |
| `-ShowInBrowser` | SwitchParameter | — | — | Named | — | Zeigt die Suchergebnisse in einer browserbasierten Bildergalerie an. |
| `-PassThru` | SwitchParameter | — | — | Named | — | Bilddaten als Objekte zurückgeben. Bei Verwendung mit '-ShowInBrowser' wird sowohl die Galerie angezeigt als auch die Objekte zurückgegeben. |
...
| `-Interactive` | SwitchParameter | — | — | Named | — | Stellt eine Verbindung zum Browser her und fügt zusätzliche Schaltflächen wie Bearbeiten und Löschen hinzu. Nur wirksam, wenn mit -ShowInBrowser verwendet. |
| `-Private` | SwitchParameter | — | — | Named | — | Öffnet im Inkognito-/Privatmodus |
| `-Force` | SwitchParameter | — | — | Named | — | Erzwingt das Aktivieren des Debug-Ports und beendet ggf. vorhandene Browser. |
| `-Edge` | SwitchParameter | — | — | Named | — | Öffnet in Microsoft Edge |
| `-Chrome` | SwitchParameter | — | — | Named | — | Öffnet in Google Chrome |
| `-Chromium` | SwitchParameter | — | — | Named | — | Wird in Microsoft Edge oder Google Chrome geöffnet, je nachdem, welcher Browser der Standard ist. |
...
| `-NewWindow` | SwitchParameter | — | — | Named | — | Vorhandenes Browserfenster nicht wiederverwenden, stattdessen ein neues erstellen |
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | Verwenden Sie alternative, in der Sitzung gespeicherte Einstellungen für KI-Präferenzen wie Sprache, Bildsammlungen usw. |
| `-ClearSession` | SwitchParameter | — | — | Named | — | Löscht alternative Einstellungen, die in der Sitzung für KI-Präferenzen wie Sprache, Bildsammlungen usw. gespeichert wurden. |
| `-SkipSession` | SwitchParameter | — | — | Named | — | Keine alternativen Einstellungen aus der Sitzung für KI-Präferenzen wie Sprache, Bildsammlungen usw. verwenden |
...
| `-MinConfidenceRatio` | Double | — | — | Named | — | Mindestkonfidenzverhältnis (0,0-1,0) zum Filtern von Personen, Szenen und Objekten nach Konfidenz. Gibt nur Daten für Personen, Szenen und Objekte zurück, deren Konfidenz größer oder gleich diesem Wert ist. |
...
> Entspricht einem beliebigen aller möglichen Metadatentypen.
...
> Array von Verzeichnispfaden zum Durchsuchen nach Bildern
...
> Sprache für Beschreibungen und Schlüsselwörter.
...
> ('Der zu suchende Beschreibungstext, Platzhalter sind zulässig.')
...
> Die Schlüsselwörter, nach denen gesucht werden soll, Platzhalter sind zulässig.
...
> Personen, nach denen gesucht werden soll, Platzhalter zulässig.
...
> Objekte, nach denen gesucht werden soll, Platzhalter sind erlaubt.
...
> Nach Szenenkategorien suchen, Platzhalter erlaubt.
...
> Bildtyp zum Filtern (z.B. 'Tageslicht', 'Abend', 'Innenraum' usw.). Unterstützt Platzhalter.
...
> Stiltyp zum Filtern (z. B. 'casual', 'formal' usw.). Unterstützt Platzhalter.
...
> Gesamtstimmung zum Filtern (z. B. „ruhig“, „fröhlich“, „traurig“ usw.). Unterstützt Platzhalter.
...
> Filtert nach Kamerahersteller in den EXIF-Metadaten von Bildern. Unterstützt Platzhalter.
...
> Filter nach Kameramodell in den EXIF-Metadaten des Bildes. Unterstützt Platzhalter.
...
> Nach GPS-Breitengradbereich in den EXIF-Metadaten von Bildern filtern.
...
> Nach GPS-Längengradbereich in den EXIF-Metadaten von Bildern filtern.
...
> Nach GPS-Höhenbereich in Bild-EXIF-Metadaten filtern (in Metern).
...
> Geografische Koordinaten [Breitengrad, Längengrad] für die Suche in der Nähe.
...
> Maximale Entfernung in Metern vom GeoLocation, um nach Bildern zu suchen.
...
> Nach Belichtungszeitbereich in Bild-EXIF-Metadaten filtern (in Sekunden).
...
> Nach F-Nummer (Blendenbereich) in den EXIF-Metadaten von Bildern filtern.
...
> Nach ISO-Empfindlichkeitsbereich in den EXIF-Metadaten von Bildern filtern.
...
> Nach Brennweitenbereich in den EXIF-Metadaten des Bildes filtern (in mm).
...
> Nach Bildbreitenbereich in Pixeln aus EXIF-Metadaten filtern.
...
> Nach Bildhöhe in Pixeln aus EXIF-Metadaten filtern.
...
> Nach Aufnahmedatum aus EXIF-Metadaten filtern. Kann ein Datumsbereich sein.
...
> 'An das Browserfenster zu sendende Tastenanschläge, ' +
                'siehe Dokumentation zum Cmdlet GenXdev\Send-Key'
...
> Steuerzeichen und Modifikatoren beim Senden von Tasten maskieren
...
> Verhindern, dass der Tastaturfokus nach dem Senden von Tasten
wieder an PowerShell zurückgegeben wird
...
> Verwenden Sie Umschalt+Eingabe anstelle von Eingabe beim Senden von Tasten
...
> Verzögerung zwischen verschiedenen Eingabezeichenfolgen in Millisekunden beim Senden von Tasten
...
> Der zu verwendende Monitor, 0 = Standard, -1 = verwerfen, -2 = konfigurierter sekundärer Monitor, Standard ist `Global:DefaultSecondaryMonitor oder 2, falls nicht gefunden
...
> Betten Sie Bilder als base64 ein.
...
> Schalter zum Deaktivieren des Fallback-Verhaltens.
...
> Option zum Überspringen der Datenbankinitialisierung und -neuerstellung.
...
> Bilder filtern, die Nacktheit enthalten.
...
> Filtere Bilder, die KEINE Nacktheit enthalten.
...
> Filtert Bilder, die explizite Inhalte enthalten.
...
> Filtert Bilder, die KEINE expliziten Inhalte enthalten.
...
> Zeigt die Suchergebnisse in einer browserbasierten Bildergalerie an.
...
> Bilddaten als Objekte zurückgeben. Bei Verwendung mit '-ShowInBrowser' wird sowohl die Galerie angezeigt als auch die Objekte zurückgegeben.
...
> Stellt eine Verbindung zum Browser her und fügt zusätzliche Schaltflächen wie Bearbeiten und Löschen hinzu. Nur wirksam, wenn mit -ShowInBrowser verwendet.
...
> Erzwingt das Aktivieren des Debug-Ports und beendet ggf. vorhandene Browser.
...
> Wird in Microsoft Edge oder Google Chrome geöffnet, je nachdem, welcher Browser der Standard ist.
...
> Öffnet den von Playwright verwalteten WebKit-Browser. 
Impliziert -PlayWright
...
> Vorhandenes Browserfenster nicht wiederverwenden, stattdessen ein neues erstellen
...
> Nur das generierte HTML zurückgeben, anstatt es in einem Browser anzuzeigen.
...
> Verwenden Sie alternative, in der Sitzung gespeicherte Einstellungen für KI-Präferenzen wie Sprache, Bildsammlungen usw.
...
> Löscht alternative Einstellungen, die in der Sitzung für KI-Präferenzen wie Sprache, Bildsammlungen usw. gespeichert wurden.
...
> Keine alternativen Einstellungen aus der Sitzung für KI-Präferenzen wie Sprache, Bildsammlungen usw. verwenden
...
> Mindestkonfidenzverhältnis (0,0-1,0) zum Filtern von Personen, Szenen und Objekten nach Konfidenz. Gibt nur Daten für Personen, Szenen und Objekte zurück, deren Konfidenz größer oder gleich diesem Wert ist.
