...
* Macht ein Fenster zum Vordergrundfenster, indem mehrere Win32-API-Methoden für
  maximale Zuverlässigkeit verwendet werden.
* Zunächst wird versucht, die SwitchToThisWindow-API zu verwenden, dann wird bei Bedarf
  auf SetForegroundWindow zurückgegriffen.
...
Bringt ein Editor-Fenster mithilfe des Fensterhandles in den Vordergrund.
...
Verwendet Positionsparameter für eine präzise Syntax.

## Parameter Details

### `-WindowHandle <IntPtr>`

> Fensterhandle, das als Vordergrundfenster festgelegt werden soll
