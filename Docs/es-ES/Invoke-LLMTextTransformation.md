...
> Transforma texto mediante procesamiento impulsado por IA.

## Description

Esta función procesa el texto de entrada utilizando modelos de IA para realizar varias transformaciones, como corrección ortográfica, agregar emoticonos o cualquier otra mejora de texto especificada a través de instrucciones. Puede aceptar entrada directamente a través de parámetros, desde la canalización o desde el portapapeles del sistema.
...
| `-Text` | String | — | ✅ (ByValue) | 0 | — | .SYNOPSIS
    Obtiene uno o más conmutadores virtuales.

.DESCRIPTION
    El cmdlet Get-VMSwitch obtiene uno o más conmutadores virtuales Hyper-V.

    Para crear un conmutador virtual, use el cmdlet New-VMSwitch.

.PARÁMETROS Name
    Especifica el nombre de un conmutador virtual. Utilice un asterisco (*) como carácter comodín.

.PARÁMETROS CimSession
    Ejecuta el cmdlet en una sesión remota o en un equipo remoto. Introduzca un nombre de equipo o un objeto de sesión, como la salida del cmdlet New-CimSession o Get-CimSession. El valor predeterminado es la sesión actual en el equipo local.

.PARÁMETROS ComputerName
    Especifica uno o más equipos Hyper-V en los que desea obtener los conmutadores virtuales. Los nombres NetBIOS, las direcciones IP y los nombres de dominio completos (FQDN) son permitidos. El valor predeterminado es el equipo local. Utilice "localhost" o un punto (".") para especificar el equipo local explícitamente.

.PARÁMETROS Credential
    Especifica una cuenta de usuario que tenga permiso para realizar esta acción. El valor predeterminado es el usuario actual.

.PARÁMETROS Id
    Especifica el identificador único de un conmutador virtual.

.PARÁMETROS SwitchType
    Especifica el tipo de un conmutador virtual. Los valores aceptables son:
...
.PARÁMETROS VlanId
    Especifica el ID de VLAN de un conmutador virtual.
...
.EJEMPLO 1
    ...
    Obtiene todos los conmutadores virtuales en el equipo local.

.EJEMPLO 2
    PS C:\> Get-VMSwitch -Name "Servidor Web"

    Obtiene un conmutador virtual llamado "Servidor Web" en el equipo local. |
| `-Instructions` | String | — | — | 1 | `('Check and correct any spelling or grammar ' +
            'errors in the text. Return the corrected text without any ' +
            'additional comments or explanations.')` | A continuación, se muestran los comandos del cmdlet Stop-Service. |
...
> A continuación, se muestran los comandos del cmdlet Stop-Service.
