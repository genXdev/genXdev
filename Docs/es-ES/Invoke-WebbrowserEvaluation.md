...
> Ejecuta código JavaScript en una pestaña seleccionada del navegador web.
...
Puede ejecutar código desde cadenas, archivos o URL.

Esta función proporciona acceso completo a las API del navegador, incluyendo IndexedDB,
localStorage, sessionStorage y otras características de la plataforma web. Incluye
...
privilegiado que elude las restricciones de seguridad estándar de JavaScript en el contexto
de la página actual. Esto permite el acceso a las API de almacenamiento, recursos de origen
cruzado (dentro de la misma pestaña) y otras características del navegador que normalmente
estarían restringidas en contextos web estándar.

Capacidades clave:
- Soporte de async/await y Promise
- Funciones generadoras con soporte de yield
- Sincronización de datos a través de $Global:Data
- Acceso privilegiado a las API de almacenamiento del navegador
- Elude las restricciones de la política de mismo origen para el almacenamiento de la página actual
...
// Cuidado, no todos los sitios web usan indexedDb, podría devolver un conjunto vacío
...
$AllIndexedDbData = Invoke-WebbrowserEvaluation "
    ...
    for (let db of await indexedDB.databases()) {
        ...
        // solicitar abrir la base de datos
        ...
        // iterar sobre los nombres de los almacenes de objetos
...
// NOTA DE SEGURIDAD: Este ejemplo básico funciona porque el módulo usa Playwright
// para la automatización del navegador, lo que omite las restricciones de seguridad normales de JavaScript
// en el contexto de la página actual. Las páginas web estándar no pueden acceder a IndexedDB desde
// otros orígenes, pero Playwright tiene los mismos privilegios que el propio sitio web.
// Vea el ejemplo mejorado a continuación para más detalles sobre consideraciones de seguridad.
    ...
    for (let dbInfo of await indexedDB.databases()) {
        try {
            ...
            let db = await new Promise((resolve, reject) => {
                ...
                setTimeout(() => reject(new Error('Tiempo de espera al abrir la base de datos')), 5000);
            });
            ...
            for (let i = 0; i < db.objectStoreNames.length; i++) {
                ...
                try {
                    ...
                    let count = await new Promise((resolve, reject) => {
                        ...
                        req.onsuccess = () => resolve(req.result);
                        req.onerror = () => reject(req.error);
                        setTimeout(() => reject(new Error('Tiempo de espera al contar')), 3000);
                    });
...
// Diferencias clave con el ejemplo básico:
// 1. Incluye manejo de errores para problemas de acceso a la base de datos
...
// Ambos ejemplos funcionan porque este módulo utiliza automatización del navegador Playwright,
// que elude las restricciones de seguridad estándar de JavaScript en el contexto
// de la página actual:

// Limitaciones de JavaScript estándar:
// - La política del mismo origen restringe el acceso a IndexedDB desde otros orígenes
...
// - Las bases de datos de service workers pueden tener protección adicional

// Cómo este ejemplo elude las restricciones:
// - Utiliza automatización del navegador Playwright para acceso privilegiado
// - Se ejecuta en el contexto de la página real, no en un entorno de pruebas
// - Tiene los mismos permisos que el sitio web para su propio almacenamiento
...
// - Combinar con acceso al sistema de archivos a directorios de perfiles del navegador (cuando sea posible)
...
Soporte para resultados de pipeline generados
Select-WebbrowserTab -Force;
Invoke-WebbrowserEvaluation "
