...
> Elimina los archivos de metadatos de imagen de los directorios de imágenes.

## Description

La función Remove-ImageMetaData elimina los archivos JSON de metadatos asociados a las imágenes. Puede eliminar selectivamente solo las palabras clave (description.json), datos de personas (people.json), datos de objetos (objects.json) o datos de escenas (scenes.json), o eliminar todos los archivos de metadatos si no se proporciona un modificador específico. Los archivos de metadatos específicos de un idioma se pueden eliminar especificando el parámetro Language, y todas las variantes de idioma se pueden eliminar mediante el modificador AllLanguages.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ImageDirectories` | String[] | — | — | 0 | `@(".\")` | Matriz de rutas de directorio para procesar y eliminar metadatos de imágenes. Si no se especifica, se utilizan los directorios predeterminados del sistema. |
| `-Language` | String | — | — | 1 | — | El idioma para eliminar archivos de metadatos específicos del idioma. |
| `-PreferencesDatabasePath` | String | — | — | 2 | — | Ruta de base de datos para archivos de datos de preferencias |
| `-Recurse` | SwitchParameter | — | — | Named | — | Recorrer directorios de forma recursiva. |
| `-OnlyKeywords` | SwitchParameter | — | — | Named | — | Solo elimine archivos description.json (palabras clave/descripciones). |
| `-OnlyPeople` | SwitchParameter | — | — | Named | — | Elimine solo los archivos people.json (datos de reconocimiento facial). |
| `-OnlyObjects` | SwitchParameter | — | — | Named | — | Solo elimina los archivos objects.json (datos de detección de objetos). |
| `-OnlyScenes` | SwitchParameter | — | — | Named | — | Eliminar solo los archivos scenes.json (datos de clasificación de escenas). |
| `-AllLanguages` | SwitchParameter | — | — | Named | — | Eliminar archivos de metadatos para todos los idiomas admitidos. |
...
Elimina todos los archivos de metadatos para imágenes en varios directorios y todos los subdirectorios.
...
Elimina únicamente los archivos description.json de los directorios del sistema predeterminados y sus subdirectorios.
...
Elimina de forma recursiva los archivos de descripción en inglés y español de los directorios predeterminados.
...
Usa alias para eliminar archivos de palabras clave de todos los idiomas compatibles.

## Parameter Details

### `-ImageDirectories <String[]>`

> Matriz de rutas de directorio para procesar y eliminar metadatos de imágenes. Si no se especifica, se utilizan los directorios predeterminados del sistema.
...
> El idioma para eliminar archivos de metadatos específicos del idioma.
...
> Solo elimine archivos description.json (palabras clave/descripciones).
...
> Elimine solo los archivos people.json (datos de reconocimiento facial).
...
> Solo elimina los archivos objects.json (datos de detección de objetos).
...
> Eliminar solo los archivos scenes.json (datos de clasificación de escenas).
...
> Eliminar archivos de metadatos para todos los idiomas admitidos.
