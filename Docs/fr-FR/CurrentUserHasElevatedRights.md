...
Détermine si l'utilisateur Windows actuel dispose de privilèges d'administrateur ou d'opérateur de sauvegarde en vérifiant leurs rôles de principal de sécurité. Renvoie true si l'utilisateur a des droits élevés, false sinon. Implémente une gestion d'erreurs robuste pour les problèmes de sécurité et d'accès.
