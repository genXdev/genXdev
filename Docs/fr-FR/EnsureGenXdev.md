...
> Garantit que tous les modules GenXdev sont correctement chargés en appelant toutes les applets de commande Ensure*.

## Description

Cette fonction récupère toutes les applets de commande GenXdev commençant par « Ensure » ou « Optimize-Ensure » (pour les applets de commande c#) et exécute chacune d'elles pour garantir que tous les modules et dépendances GenXdev requis sont correctement chargés et disponibles. Tous les échecs lors de l'exécution sont capturés et affichés sous forme de messages informatifs.
...
| `-ForceConsent` | SwitchParameter | — | — | Named | — | Forcer une invite de consentement même si la préférence est définie pour l'installation de logiciels tiers. |
| `-ConsentToThirdPartySoftwareInstallation` | SwitchParameter | — | — | Named | — | Accepter automatiquement l'installation de logiciels tiers et définir l'indicateur persistant pour tous les packages. |
...
Cette commande exécute toutes les cmdlets Ensure* disponibles pour initialiser l'environnement GenXdev.
...
Cette commande exécute toutes les applets de commande Ensure* et télécharge et charge également tous les packages NuGet définis dans le fichier manifeste packages.json.
...
> Force l'exécution des opérations ensure même si elles semblent déjà terminées
...
> Télécharge et charge tous les packages NuGet définis dans le fichier manifeste packages.json
...
> Forcer une invite de consentement même si la préférence est définie pour l'installation de logiciels tiers.
...
> Accepter automatiquement l'installation de logiciels tiers et définir l'indicateur persistant pour tous les packages.
