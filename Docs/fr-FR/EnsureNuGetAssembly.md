...
> Télécharge et charge les assemblies .NET à partir de packages NuGet basés sur la clé ou l'ID du package.

## Description

Cette fonction garantit que les assemblys du package NuGet spécifié sont téléchargés et chargés dans la session PowerShell active. Elle prend en charge la détection et le chargement automatiques avec un repli vers un manifeste JSON pour les remplacements de configuration, en gérant les cas particuliers comme les dépendances factices (par exemple, SQLite). L'installation utilise exclusivement l'interface CLI dotnet pour une gestion fiable des packages et charge les assemblys directement depuis le cache NuGet global sans les copier. Le chargement différé vérifie si le type est déjà disponible avant de procéder.
...
| `-ManifestPath` | String | — | — | Named | — | Chemin vers packages.json ; par défaut, racine du module si omis. |
| `-Version` | String | — | — | Named | — | Version spécifique ; si omise, utilise la plus élevée du JSON ou la plus récente. |
| `-TypeName` | String | — | — | Named | — | Nom de type pour vérifier le chargement. |
| `-ForceLatest` | SwitchParameter | — | — | Named | — | Rétrograder vers la version la plus récente si la version exacte échoue. |
| `-Destination` | String | — | — | Named | — | Destination d'installation personnalisée ; par défaut, cache persistant ou global local. |
| `-Description` | String | — | — | Named | — | Description facultative du logiciel et de son objectif pour le consentement. |
| `-Publisher` | String | — | — | Named | — | Éditeur ou fournisseur facultatif du logiciel pour le consentement. |
| `-ForceConsent` | SwitchParameter | — | — | Named | — | Forcer une invite même si la préférence de consentement est définie. |
| `-ConsentToThirdPartySoftwareInstallation` | SwitchParameter | — | — | Named | — | Accepter automatiquement l'installation de logiciels tiers et définir le drapeau persistant. |
...
> Clé du package à partir de packages.json ou PackageId NuGet direct.
...
> Chemin vers packages.json ; par défaut, racine du module si omis.
...
> Version spécifique ; si omise, utilise la plus élevée du JSON ou la plus récente.
...
> Nom de type pour vérifier le chargement.
...
> Rétrograder vers la version la plus récente si la version exacte échoue.
...
> Destination d'installation personnalisée ; par défaut, cache persistant ou global local.
...
> Description facultative du logiciel et de son objectif pour le consentement.
...
> Éditeur ou fournisseur facultatif du logiciel pour le consentement.
...
> Forcer une invite même si la préférence de consentement est définie.
...
> Accepter automatiquement l'installation de logiciels tiers et définir le drapeau persistant.
