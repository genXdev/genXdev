...
> Rechercher les fichiers en double dans plusieurs répertoires en fonction de critères configurables.

## Description

Recherche récursivement des fichiers en double dans les répertoires spécifiés. Les fichiers sont considérés comme en double si le contenu est exactement identique ou commence par le même contenu (pour une comparaison partielle à l'aide du commutateur -ComparePartial). L'applet de commande utilise un hachage rapide des premiers octets du fichier pour un regroupement efficace avant d'effectuer des comparaisons de contenu complètes. La taille de ce hachage rapide peut être configurée avec le paramètre -CompareByteLength, ce qui permet un équilibre entre performances et précision dans la détection des doublons. Seuls les fichiers ayant une longueur minimale correspondant à cette valeur CompareByteLength seront pris en compte pour la détection des doublons ; les fichiers plus petits seront regroupés uniquement en fonction du nom de fichier.
...
| `-ComparePartial` | SwitchParameter | — | — | Named | — | Renverra les fichiers de tailles différentes tant qu'ils commencent par le même nom de fichier et contenu, cela détecte les téléchargements/copies interrompues |
| `-CompareByteLength` | Int32 | — | — | Named | `65536` | Compare rapidement la taille en octets des fichiers, utilisé pour l'indexation rapide et la recherche de doublons, cela ne compare pas le contenu des fichiers |
...
> Renverra les fichiers de tailles différentes tant qu'ils commencent par le même nom de fichier et contenu, cela détecte les téléchargements/copies interrompues
...
> Compare rapidement la taille en octets des fichiers, utilisé pour l'indexation rapide et la recherche de doublons, cela ne compare pas le contenu des fichiers
