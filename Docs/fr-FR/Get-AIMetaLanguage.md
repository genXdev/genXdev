...
> Obtient la langue par défaut configurée pour les opérations de métadonnées d'image.

## Description

* Cette applet de commande récupère la langue par défaut utilisée par le module GenXdev.AI
  pour les opérations de métadonnées d'image.
* Elle vérifie d'abord les variables globales (sauf si SkipSession est spécifié), puis
...
| `-SessionOnly` | SwitchParameter | — | — | Named | `False` | Utilisez des paramètres alternatifs stockés dans la session pour les préférences IA telles que la langue, les collections d'images, etc. |
| `-ClearSession` | SwitchParameter | — | — | Named | `False` | Effacer le paramètre de session (variable globale) avant de récupérer |
| `-SkipSession` | SwitchParameter | — | — | Named | `False` | N'utilisez pas de paramètres alternatifs stockés dans la session pour les préférences IA telles que la langue, les collections d'images, etc. |
...
Obtenir la langue actuellement configurée à partir des variables globales ou des préférences.
...
Obtenir la langue configurée à partir des préférences persistantes, en ignorant la session.
...
Effacer le paramètre de session et obtenir la langue à partir des préférences persistantes.
