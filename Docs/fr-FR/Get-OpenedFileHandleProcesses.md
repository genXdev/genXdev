...
> Récupère les processus qui ont des descripteurs de fichier ouverts sur les fichiers spécifiés.
...
* Utile pour déterminer quels processus empêchent les opérations sur les fichiers
  telles que la suppression ou la modification.
...
Vérifie plusieurs fichiers via l'entrée pipeline pour les processus ayant des handles ouverts.

## Parameter Details

### `-FilePath <String[]>`

> Le chemin vers le(s) fichier(s) à vérifier pour les handles ouverts
