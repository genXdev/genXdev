...
> Récupère les définitions de refactorisation à partir des préférences de GenXdev en fonction des modèles de noms.

## Description

Recherche et charge les ensembles de définitions de refactorisation stockés dans les préférences GenXdev. Chaque ensemble de refactorisation est stocké sous forme de chaîne JSON dans une préférence dont le nom commence par 'refactor_set_'. La fonction désérialise les ensembles correspondants en objets.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Name` | String[] | — | ✅ (ByValue, ByPropertyName) | 0 | `@('*')` | Modèle(s) à rechercher pour les définitions de refactorisation 🌐 *Supports wildcards* |
| `-PreferencesDatabasePath` | String | — | — | Named | — | Chemin de la base de données pour les fichiers de données de préférences |
| `-DefaultValue` | String | — | — | Named | — | La valeur par défaut si la préférence n'est pas trouvée |
...
> Modèle(s) à rechercher pour les définitions de refactorisation
