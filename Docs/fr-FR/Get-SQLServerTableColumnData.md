...
> Récupère les données d'une colonne spécifique dans une table de base de données SQL.

## Description

Cette fonction fournit un moyen pratique d'extraire des données d'une seule colonne dans une table de base de données SQL. Elle prend en charge deux méthodes de connexion : le chemin direct du fichier de base de données ou la chaîne de connexion. La fonction inclut des options pour limiter le nombre d'enregistrements renvoyés et utilise une construction de requête SQL appropriée pour des performances optimales.
...
| `-Count` | Int32 | — | — | 4 | `100` | Nombre d'enregistrements à retourner. 100 par défaut. Utilisez -1 pour tous. |
...
> Nombre d'enregistrements à retourner. 100 par défaut. Utilisez -1 pour tous.
