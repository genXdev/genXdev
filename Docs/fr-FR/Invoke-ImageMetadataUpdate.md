...
Elle traite chaque image pour extraire les métadonnées EXIF détaillées, y compris les détails de l'appareil photo,
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ImageDirectories` | String[] | — | — | 0 | `@('.\')` | Tableau des chemins de répertoire à traiter pour les mises à jour des métadonnées d'images |
| `-RetryFailed` | SwitchParameter | — | — | Named | — | Réessaiera les mises à jour des métadonnées d'image ayant précédemment échoué |
| `-OnlyNew` | SwitchParameter | — | — | Named | — | Traitez uniquement les images qui n'ont pas encore de fichier de métadonnées |
...
| `-PassThru` | SwitchParameter | — | — | Named | — | Renvoie des objets structurés au lieu d'afficher dans la console |
...
> Tableau des chemins de répertoire à traiter pour les mises à jour des métadonnées d'images
...
> Réessaiera les mises à jour des métadonnées d'image ayant précédemment échoué
...
> Traitez uniquement les images qui n'ont pas encore de fichier de métadonnées
...
> Si spécifié, traite les images dans les sous-répertoires de manière récursive
...
> Forcer la reconstruction des métadonnées même si elles existent déjà
...
> Renvoie des objets structurés au lieu d'afficher dans la console
...
> Ne pas utiliser les paramètres alternatifs stockés dans la session pour les préférences IA comme la Langue, les collections d'images, etc.
