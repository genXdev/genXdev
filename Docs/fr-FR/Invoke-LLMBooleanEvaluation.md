...
> Évalue une instruction à l'aide de l'IA pour déterminer si elle est vraie ou fausse.

## Description

Cette fonction utilise des modèles d'IA pour évaluer des affirmations et déterminer leur valeur de vérité. Elle peut accepter une entrée directement via des paramètres, à partir du pipeline, ou du presse-papiers système. La fonction renvoie un résultat booléen accompagné du niveau de confiance et du raisonnement du modèle d'IA.
...
| `-DontAddThoughtsToHistory` | SwitchParameter | — | — | Named | — | Ne pas ajouter de réflexions du modèle à l'historique des conversations |
...
| `-OutputMarkdownBlocksOnly` | SwitchParameter | — | — | Named | — | Convertit les notifications push de l'appareil en notifications toast pour l'application actuelle. Par défaut, empêche les notifications toast de déclencher un son et d'être affichées en mode Ne pas déranger. |
...
| `-SuppressRegex` | String | — | — | Named | — | Expression régulière à supprimer de la sortie |
...
| `-LengthPenalty` | Double | — | — | Named | — | Pénalité de longueur pour la génération de séquences |
...
| `-NoVOX` | SwitchParameter | — | — | Named | — | Désactiver la VOX (activation vocale) |
...
| `-OnlyResponses` | SwitchParameter | — | — | Named | — | Je suis un assistant utile conçu pour produire du JSON. |
...
> Instructions pour le modèle d'IA sur la façon d'évaluer la déclaration
...
> Ne pas ajouter de réflexions du modèle à l'historique des conversations
...
> Autoriser l'IA à utiliser les outils et capacités par défaut
...
> Convertit les notifications push de l'appareil en notifications toast pour l'application actuelle. Par défaut, empêche les notifications toast de déclencher un son et d'être affichées en mode Ne pas déranger.
...
> Expression régulière à supprimer de la sortie
...
> N'utilisez pas le contexte dans la requête
...
> Je suis un assistant utile conçu pour produire du JSON.
