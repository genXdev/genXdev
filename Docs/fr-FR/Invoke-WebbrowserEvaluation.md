...
Cette fonction offre un accès complet aux API du navigateur, y compris IndexedDB, localStorage, sessionStorage et d'autres fonctionnalités de la plateforme web. Elle inclut une gestion des erreurs intégrée, une gestion du temps d'attente et la prise en charge de plusieurs résultats issus de fonctions génératrices.

La fonction utilise Playwright pour l'automatisation du navigateur, ce qui fournit un accès privilégié contournant les restrictions de sécurité JavaScript standard dans le contexte de la page actuelle. Cela permet d'accéder aux API de stockage, aux ressources inter-origines (dans le même onglet) et à d'autres fonctionnalités du navigateur normalement restreintes dans les contextes web standard.
...
- Énumération et extraction des données d'IndexedDB
...
// cette fonction retourne toutes les lignes de toutes les tables/datastores de toutes les bases de données indexedDb de l'onglet sélectionné
// attention, tous les sites web n'utilisent pas indexedDb, cela pourrait retourner un ensemble vide
...
$AllIndexedDbData = Invoke-WebbrowserEvaluation "
    ...
    for (let db of await indexedDB.databases()) {
        ...
        // itérer sur les noms des magasins d'objets
        for (let i = 0; i < openedDb.objectStoreNames.length; i++) {
            ...
            // obtenir le gestionnaire du magasin d'objets
...
// NOTE DE SÉCURITÉ : Cet exemple de base fonctionne car le module utilise l'automatisation de navigateur Playwright,
// qui contourne les restrictions de sécurité JavaScript normales dans le contexte de la page actuelle.
// Les pages web standard ne peuvent pas accéder à IndexedDB depuis d'autres origines, mais Playwright a les mêmes privilèges que le site web lui-même.
...
// les comptes de bases de données, la gestion des erreurs et les métadonnées. Contrairement à l'exemple
// de base ci-dessus, cette version gère les restrictions de sécurité, fournit des informations
// détaillées sur les magasins et inclut les comptes d'enregistrements sans nécessairement récupérer toutes les données.
...
$EnhancedIndexedDbData = Invoke-WebbrowserEvaluation "
    ...
    for (let dbInfo of await indexedDB.databases()) {
        try {
            // Ouvrir la base de données avec délai d'attente
            let db = await new Promise((resolve, reject) => {
                let req = indexedDB.open(dbInfo.name);
                req.onsuccess = () => resolve(req.result);
                req.onerror = () => reject(req.error);
                setTimeout(() => reject(new Error('Délai d\'ouverture de la base de données dépassé')), 5000);
            });
            ...
            for (let i = 0; i < db.objectStoreNames.length; i++) {
                ...
                try {
                    ...
                    let count = await new Promise((resolve, reject) => {
                        ...
                        req.onsuccess = () => resolve(req.result);
                        req.onerror = () => reject(req.error);
                        setTimeout(() => reject(new Error('Délai de comptage dépassé')), 3000);
                    });
...
// Différences clés par rapport à l'exemple de base :
...
// 3. Obtient les comptes d'enregistrements sans récupérer toutes les données (plus efficace)
...
// - La politique de même origine limite l'accès à IndexedDB depuis d'autres origines
// - Certaines bases de données peuvent être cachées ou protégées par les fonctions de sécurité du navigateur
// - L'accès à une base de données inter-origine est généralement bloqué
...
// - S'exécute dans le contexte de la page réelle, pas dans un environnement isolé
// - A les mêmes permissions que le site web lui-même pour son propre stockage
// - Peut accéder à toutes les bases de données créées par l'origine/domaine actuelle
...
// - Certaines extensions de navigateur peuvent créer un stockage isolé non accessible via JavaScript

// Approches alternatives pour un accès maximal :
// - Utiliser GenXdev.Webbrowser avec plusieurs onglets de différentes origines
// - Combiner avec l'accès au système de fichiers pour les répertoires de profil du navigateur (lorsque possible)
...
Support pour les résultats de pipeline générés
Select-WebbrowserTab -Force;
Invoke-WebbrowserEvaluation "
