...
Recherche des fichiers en utilisant le même ensemble de paramètres que `Find-Item` et déplace chaque fichier correspondant dans un sous-dossier de `TargetFolder` en fonction de la date de création ou de la date média du fichier. L'applet de commande prend en charge la correspondance de contenu, les recherches sur l'ensemble du lecteur et de nombreux filtres. Il peut éventuellement supprimer les dossiers sources vides après le déplacement.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Dossier racine cible dans lequel les fichiers correspondants seront déplacés dans des sous-dossiers basés sur la date |
...
> Dossier racine cible dans lequel les fichiers correspondants seront déplacés dans des sous-dossiers basés sur la date
