...
Ouvre une ou plusieurs requêtes de recherche Bing dans un navigateur Web, en utilisant des paramètres de navigateur et des commutateurs de ligne de commande configurables. Cette fonction fournit une interface complète pour effectuer des recherches Bing avec des options de personnalisation étendues du navigateur, y compris le positionnement de la fenêtre, la sélection du moniteur, la navigation privée et l'automatisation des frappes.

La fonction encode automatiquement les requêtes au format URL et les formate pour l'API de recherche Bing. Elle prend en charge tous les navigateurs majeurs, y compris Edge, Chrome et Firefox, avec des fonctionnalités telles que la navigation privée, le mode application, la gestion précise des fenêtres et l'automatisation des frappes.
...
Recherche plusieurs termes via le pipeline et ouvre les résultats dans tous les navigateurs installés, en positionnant les fenêtres sur le côté gauche de l'écran.
