...
> Ouvre une interface de chat LLM cloud pour les requêtes IA.

## Description

Cette fonction fournit une interface unifiée pour ouvrir divers services de chat IA basés sur le cloud dans un navigateur Web. Elle prend en charge plusieurs plateformes d'IA, notamment ChatGPT, BingCopilot, Google Gemini, X Grok, DeepSearch et GitHub Copilot. La fonction sélectionne automatiquement la fonction spécifique au point de terminaison approprié et transmet tous les paramètres pertinents pour la configuration du navigateur et le positionnement de la fenêtre.
...
| `-EndPoint` | String | — | — | 1 | `'XGrok'` | Le point de terminaison pour appeler la requête |
...
Ouvre ChatGPT et soumet la requête concernant les fonctions PowerShell.
...
Utilise l'alias pour interroger Google Gemini sur l'apprentissage automatique.
...
Traite plusieurs requêtes via X Grok sur le moniteur par défaut.
