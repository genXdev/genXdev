...
> Ouvre une requête Github CoPilot dans un navigateur web

## Description

Ouvre une requête Github Copilot dans le navigateur Web et saisit automatiquement la requête spécifiée. La fonction attend que la page se charge avant de taper la requête. Cette fonction utilise la fonction sous-jacente Open-WebsiteAndPerformQuery pour gérer l'automatisation du navigateur et offre un moyen pratique d'interagir avec GitHub Copilot depuis PowerShell avec des options de configuration étendues du navigateur.
