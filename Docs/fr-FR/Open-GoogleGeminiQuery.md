...
> Ouvre une requête Google Gemini dans un navigateur web

## Description

Ouvre une requête Google Gemini dans le navigateur web, saisit automatiquement la requête et la soumet. Cela offre un moyen rapide d'interagir avec l'IA Gemini de Google. La fonction prend en charge un contrôle étendu du navigateur, y compris la sélection du moniteur, le positionnement des fenêtres et les fonctions d'automatisation du clavier.
...
Ouvre Google Gemini et soumet la requête "Qu'est-ce que PowerShell ?".
...
Utilise l'alias pour soumettre une requête sur les tableaux via l'entrée du pipeline.
