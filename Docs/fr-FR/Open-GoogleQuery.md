...
Ouvre une ou plusieurs requêtes Google dans un navigateur Web avec une prise en charge complète pour plusieurs langues, la sélection de moniteur, le positionnement de la fenêtre, le choix du navigateur et l'automatisation du clavier. La fonction fournit une interface riche en fonctionnalités pour effectuer des recherches Google avec un contrôle précis du comportement et de l'apparence du navigateur.
...
- Traitement par lots de requêtes avec prise en charge du pipeline
- Automatisation des frappes clavier vers les fenêtres du navigateur
- Passage intelligent des paramètres aux fonctions de navigateur sous-jacentes

La fonction formate automatiquement les requêtes de recherche pour le moteur de recherche Google et applique le filtrage linguistique lorsque spécifié. Toutes les fonctionnalités de positionnement et de gestion du navigateur sont héritées de la fonction Open-Webbrowser sous-jacente.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Les termes de recherche pour interroger Google |
...
| `-Monitor` | Int32 | — | — | Named | `-1` | Le moniteur à utiliser, 0 = par défaut, -1 = ignorer, -2 = moniteur secondaire configuré, par défaut -1 |
...
Ouvre une recherche Google pour « PowerShell scripting » en anglais sur le moniteur principal.
...
Ouvre une recherche Google pour « algorithmes d'apprentissage automatique » en mode plein écran sur le moniteur 2 à l'aide d'alias.
...
Ouvre plusieurs recherches Google dans Chrome via l'entrée du pipeline, chacune en anglais.
...
Ouvre une recherche de navigation privée pour « intelligence artificielle » dans une nouvelle fenêtre.
...
> Les termes de recherche pour interroger Google
...
> La langue des résultats de recherche Google
...
> Le moniteur à utiliser, 0 = par défaut, -1 = ignorer, -2 = moniteur secondaire configuré, par défaut -1
