...
Ouvre les requêtes InstantStreetView de manière configurable en utilisant les commutateurs de ligne de commande du navigateur web. Prend en charge plusieurs requêtes et la sélection de moniteurs. Offre un contrôle complet du navigateur, y compris le positionnement des fenêtres, les paramètres de langue et les fonctionnalités spécifiques au navigateur.

La fonction construit automatiquement les URL InstantStreetView en encodant les requêtes de localisation et en les ouvrant dans le navigateur spécifié. Prend en charge tous les principaux navigateurs, y compris Edge, Chrome et Firefox, avec des options de personnalisation étendues.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Les requêtes de localisation à afficher dans InstantStreetView |
...
Ouvre InstantStreetView pour Big Ben en anglais en utilisant le mode de navigation privée.
...
Ouvre plusieurs requêtes InstantStreetView via le pipeline dans le navigateur Chrome, positionnées sur le côté gauche de l'écran.

## Parameter Details

### `-Queries <String[]>`

> Les requêtes de localisation à afficher dans InstantStreetView
