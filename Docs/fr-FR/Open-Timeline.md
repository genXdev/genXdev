...
Ouvre une timeline interactive en ligne.
...
| `-Language` | String | — | — | 1 | — | Remplacez la langue par défaut du navigateur, ou sélectionnez [Toutes] pour faire tourner toutes les langues chaque minute |
| `-DragedNodeBackground` | String | — | — | Named | `$null` | Écraser le style de couleur CSS pour peindre l'arrière-plan du nœud actuellement glissé |
| `-FocusedNodeBackground` | String | — | — | Named | `$null` | Override the CSS color style for painting the focused node's (in center) background |
| `-FocusedNodeForeground` | String | — | — | Named | `$null` | Override the CSS color style for painting the focused node's (in center) foreground text |
| `-UnFocusedNodeBackground` | String | — | — | Named | `$null` | Override the CSS color style for painting the focused node's (in center) background |
| `-UnFocusedNodeForeground` | String | — | — | Named | `$null` | Overwrite the css-color-style for painting the unfocused node's (centered) foreground text |
| `-BorderLightColor` | String | — | — | Named | `$null` | Overwrite the default CSS border-light-color color style |
| `-BorderDarkColor` | String | — | — | Named | `$null` | Override the default CSS border-dark-color color style |
| `-BorderWidth` | Int32 | — | — | Named | `1` | Remplace la largeur par défaut pour le dessin des bordures de nœuds |
| `-RotationDelaySeconds` | Int32 | — | — | Named | `15` | Écraser la largeur par défaut pour le délai de rotation de fond |
...
Ouvre une chronologie interactive avec des visuels inspirés de Van Gogh en français.
...
Ouvre la chronologie sur le moniteur 2 en utilisant des alias.
...
Ouvre la timeline dans le mode incognito de Chrome en plein écran.
