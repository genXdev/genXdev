...
Ouvre une ou plusieurs requêtes Wolfram Alpha dans un navigateur web. Prend en charge la sélection configurable de moniteur et les options de navigateur via des paramètres dynamiques. Offre un contrôle complet du navigateur, y compris le positionnement, le redimensionnement et les capacités d'automatisation pour améliorer les workflows de calcul scientifique.
...
Ouvre une requête Wolfram Alpha pour la masse du soleil sur l'écran par défaut.
...
Ouvre une requête de calcul en mode plein écran avec la préférence de langue anglaise via l'entrée du pipeline.
