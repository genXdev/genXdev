...
> Ouvre le jeu de combat YabAI dans un navigateur web.

## Description

Cette fonction lance le jeu YabAI Battle dans un navigateur Web avec des options de personnalisation étendues pour le placement de la fenêtre, la sélection du navigateur et le mode d'affichage. Elle fournit un wrapper autour de la fonction GenXdev\Open-Webbrowser avec la construction d'URL spécifique au jeu et la gestion des paramètres.

La fonction prend en charge deux modes de jeu :
- Mode Battle : Lancer une nouvelle bataille IA contre IA
- Mode Spectateur : Regarder des batailles IA existantes

Les options de positionnement du navigateur incluent les modes gauche, droite, haut, bas, centré et plein écran avec la prise en charge multi-moniteur. La fonction gère automatiquement les paramètres du mode application et la gestion des extensions du navigateur pour une expérience de jeu optimale.
...
| `-Monitor` | Int32 | — | — | Named | `-2` | L'écran à utiliser, 0 = défaut, -1 = ignorer, -2 = écran secondaire configuré, par défaut sur $Global:DefaultSecondaryMonitor ou 2 si non trouvé |
| `-ShowWindow` | SwitchParameter | — | — | Named | — | Ouvre en mode plein écran |
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | Échappez les caractères de contrôle lors de l'envoi de touches. |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | Empêche le retour du focus clavier à PowerShell après l'envoi de touches. |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | Envoyer Maj+Entrée au lieu de la touche Entrée standard pour les sauts de ligne lors de l'envoi des touches. |
...
| `-RestoreFocus` | SwitchParameter | — | — | Named | — | Restaurer le focus sur la fenêtre précédente après l'ouverture du navigateur. |
...
Ouvre le jeu YabAI Battle dans Microsoft Edge en mode navigation privée
sans plein écran.
...
Ouvre le jeu en utilisant des alias pour Edge, le mode incognito et sans plein écran.
...
Ouvre le jeu en mode spectateur en utilisant Google Chrome pour regarder les batailles d'IA.
...
Ouvre le jeu avec l'interface en espagnol positionnée sur le côté gauche de l'écran 1.

## Parameter Details

### `-Language <String>`

> La langue de l'interface du jeu
...
> Envoyer Maj+Entrée au lieu de la touche Entrée standard pour les sauts de ligne lors de l'envoi des touches.
...
> Délai en millisecondes entre l'envoi de chaque touche.
...
> Restaurer le focus sur la fenêtre précédente après l'ouverture du navigateur.
