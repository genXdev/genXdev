...
Cette fonction envoie une entrée clavier à une fenêtre ou un processus cible à l'aide de l'objet Windows Script Host Shell. Elle peut cibler des fenêtres par nom de processus, identifiant de processus ou handle de fenêtre. La fonction prend en charge les séquences de touches spéciales, les caractères d'échappement et diverses options de temporisation pour une livraison fiable des touches.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-KeysToSend` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | L'entrée clavier à envoyer sous forme d'un tableau de chaînes |
| `-ProcessName` | String | — | ✅ (ByPropertyName) | Named | — | Nom du processus auquel envoyer les touches (supporte les caractères génériques) *(Parameter set: )* 🌐 *Supports wildcards* |
| `-ProcessId` | Int32 | — | ✅ (ByPropertyName) | Named | — | ID du processus auquel envoyer les touches *(Parameter set: )* |
| `-WindowHandle` | Int64 | — | ✅ (ByPropertyName) | Named | — | Handle de fenêtre pour envoyer les touches *(Parameter set: )* |
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | Échapper les caractères de contrôle et les modificateurs dans le texte d'entrée |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | Conserver le focus clavier sur la fenêtre cible après l'envoi des touches |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | Utilisez Maj+Entrée au lieu d'Entrée pour les sauts de ligne |
...
> L'entrée clavier à envoyer sous forme d'un tableau de chaînes
...
> Nom du processus auquel envoyer les touches (supporte les caractères génériques)
...
> ID du processus auquel envoyer les touches
...
> Handle de fenêtre pour envoyer les touches
...
> Échapper les caractères de contrôle et les modificateurs dans le texte d'entrée
...
> Conserver le focus clavier sur la fenêtre cible après l'envoi des touches
...
> Utilisez Maj+Entrée au lieu d'Entrée pour les sauts de ligne
