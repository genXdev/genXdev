...
> Envoie un paquet magique Wake-on-LAN pour réveiller des ordinateurs distants sur le réseau.

## Description

Construit et diffuse un paquet magique Wake-on-LAN (WOL) vers la ou les adresses MAC spécifiées. Le paquet magique est une diffusion UDP contenant l'adresse MAC cible répétée 16 fois, ce qui déclenche la mise sous tension de l'ordinateur sur les interfaces réseau prises en charge. Prend en charge les adresses de diffusion et les ports personnalisés.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-MacAddress` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Une ou plusieurs adresses MAC vers lesquelles envoyer le paquet magique (par exemple, 00:11:22:33:44:55) |
| `-BroadcastAddress` | String | — | — | 1 | `"255.255.255.255"` | L'adresse IP de diffusion à laquelle envoyer le paquet magique (par défaut : 255.255.255.255) |
| `-Port` | Int32 | — | — | 2 | `4000` | Le port UDP pour envoyer le paquet magique (par défaut : 4000) |
...
Envoie des paquets magiques à plusieurs ordinateurs sur le port 9 via l'entrée de pipeline.

## Parameter Details

### `-MacAddress <String[]>`

> Une ou plusieurs adresses MAC vers lesquelles envoyer le paquet magique (par exemple, 00:11:22:33:44:55)
...
> L'adresse IP de diffusion à laquelle envoyer le paquet magique (par défaut : 255.255.255.255)
...
> Le port UDP pour envoyer le paquet magique (par défaut : 4000)
