...
> Temukan file duplikat di beberapa direktori berdasarkan kriteria yang dapat dikonfigurasi.

## Description

Mencari direktori yang ditentukan secara rekursif untuk file duplikat. File dianggap duplikat jika kontennya sama persis atau dimulai dengan konten yang sama (untuk perbandingan parsial menggunakan parameter -ComparePartial). Cmdlet menggunakan hash cepat dari byte awal file untuk pengelompokan yang efisien sebelum melakukan perbandingan konten penuh. Ukuran hash cepat ini dapat dikonfigurasi dengan parameter -CompareByteLength, memungkinkan keseimbangan antara kinerja dan akurasi dalam deteksi duplikat, hanya file yang memiliki panjang minimum dari nilai CompareByteLength ini yang akan dipertimbangkan untuk deteksi duplikat, file yang lebih kecil akan dikelompokkan berdasarkan nama file saja.
...
| `-ComparePartial` | SwitchParameter | — | — | Named | — | Akan mengembalikan file dengan ukuran berbeda selama file tersebut memiliki nama dan konten yang sama, ini mendeteksi unduhan/salinan yang gagal |
| `-CompareByteLength` | Int32 | — | — | Named | `65536` | Membandingkan panjang byte file dengan cepat, digunakan untuk pengindeksan cepat dan pencarian duplikat, ini tidak akan membandingkan konten file |
...
> Akan mengembalikan file dengan ukuran berbeda selama file tersebut memiliki nama dan konten yang sama, ini mendeteksi unduhan/salinan yang gagal
...
> Membandingkan panjang byte file dengan cepat, digunakan untuk pengindeksan cepat dan pencarian duplikat, ini tidak akan membandingkan konten file
