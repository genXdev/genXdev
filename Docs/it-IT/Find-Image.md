...
> Cerca file immagine e metadati in directory specificate con funzionalità di filtro e visualizzazione opzionale in galleria basata su browser.

## Description

Cerca file immagine (jpg, jpeg, png, gif, bmp, webp, tiff, tif) nella directory specificata e nelle sue sottodirectory. Per ogni immagine, controlla i file description.json, keywords.json, people.json e objects.json associati per i metadati. Può filtrare le immagini in base a corrispondenze di parole chiave, riconoscimento di persone e rilevamento di oggetti, quindi restituire i risultati come oggetti. Utilizza -ShowInBrowser per visualizzare i risultati in un layout a muratura basato su browser.

Logica dei parametri:
- All'interno di ciascun tipo di parametro (Keywords, People, Objects, ecc.): utilizza la logica OR
  Esempio: -Keywords "gatto","cane" trova immagini con gatto O cane
- Tra diversi tipi di parametro: utilizza la logica AND
  Esempio: -Keywords "gatto" -People "Mario" trova immagini con gatto E Mario
- Parametri di intervallo EXIF: fornisci valori [min, max] per filtrare gli intervalli
- Parametri stringa: supportano corrispondenza con caratteri jolly * e ?

La funzione cerca nelle directory delle immagini ed esamina i flussi di dati alternativi contenenti metadati in formato JSON. Può abbinare parole chiave utilizzando pattern con caratteri jolly, filtrare per persone specifiche e cercare oggetti rilevati. Per impostazione predefinita, restituisce oggetti di dati immagine. Utilizza -ShowInBrowser per visualizzare in un browser web.
...
| `-Name` | String[] | — | — | 1 | `@('.\')` | Matrice di percorsi di directory per cercare immagini |
...
| `-Scenes` | String[] | — | — | Named | `@()` | Categorie di scena da cercare, caratteri jolly consentiti. |
| `-PictureType` | String[] | — | — | Named | `@()` | Tipo di immagine per cui filtrare (ad es., 'diurno', 'serale', 'interno', ecc.). Supporta caratteri jolly. |
| `-StyleType` | String[] | — | — | Named | `@()` | Tipo di stile per filtrare (ad es. 'casual', 'formale', ecc.). Supporta caratteri jolly. |
| `-OverallMood` | String[] | — | — | Named | `@()` | Umore generale per il filtro (ad es. 'calmo', 'allegro', 'triste', ecc.). Supporta i caratteri jolly. |
| `-MetaCameraMake` | String[] | — | — | Named | `@()` | Filtra per produttore della fotocamera nei metadati EXIF delle immagini. Supporta caratteri jolly. |
| `-MetaCameraModel` | String[] | — | — | Named | `@()` | Filtra per modello di fotocamera nei metadati EXIF dell'immagine. Supporta i caratteri jolly. |
| `-MetaGPSLatitude` | Double[] | — | — | Named | — | Filtra per intervallo di latitudine GPS nei metadati EXIF delle immagini. |
| `-MetaGPSLongitude` | Double[] | — | — | Named | — | Filtra per intervallo di longitudine GPS nei metadati EXIF dell'immagine. |
| `-MetaGPSAltitude` | Double[] | — | — | Named | — | Filtra per intervallo di altitudine GPS nei metadati EXIF dell'immagine (in metri). |
...
| `-MetaExposureTime` | Double[] | — | — | Named | — | Filtra per intervallo di tempo di esposizione nei metadati EXIF dell'immagine (in secondi). |
| `-MetaFNumber` | Double[] | — | — | Named | — | Filtra per intervallo di numero F (apertura) nei metadati EXIF dell'immagine. |
| `-MetaISO` | Int32[] | — | — | Named | — | Filtra per intervallo di sensibilità ISO nei metadati EXIF dell'immagine. |
| `-MetaFocalLength` | Double[] | — | — | Named | — | Filtra per intervallo di lunghezza focale nei metadati EXIF dell'immagine (in mm). |
| `-MetaWidth` | Int32[] | — | — | Named | — | Filtra per intervallo di larghezza immagine in pixel dai metadati EXIF. |
| `-MetaHeight` | Int32[] | — | — | Named | — | Filtra per intervallo di altezza immagine in pixel dai metadati EXIF. |
| `-MetaDateTaken` | DateTime[] | — | — | Named | — | Filtra per data di scatto dai metadati EXIF. Può essere un intervallo di date. |
...
| `-Description` | String | — | — | Named | `('Hover over images to see face recognition ' +
            'and object detection data')` | Descrizione per la galleria |
| `-AcceptLang` | String | — | — | Named | `$null` | Imposta l'intestazione http accept-lang del browser |
| `-KeysToSend` | String[] | — | — | Named | — | Sequenze di tasti da inviare alla finestra del browser, vedere la documentazione per il cmdlet GenXdev\Send-Key |
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | Escape dei caratteri di controllo e dei modificatori durante l'invio dei tasti |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | Impedisce il ritorno del focus della tastiera a PowerShell dopo l'invio dei tasti |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | Usa Shift+Invio invece di Invio per inviare i tasti |
| `-SendKeyDelayMilliSeconds` | Int32 | — | — | Named | — | Ritardo tra diverse stringhe di input in millisecondi durante l'invio dei tasti |
...
| `-Monitor` | Int32 | — | — | Named | `-2` | ('Il monitor da utilizzare, 0 = predefinito, -1 = ignora, ' +
                '-2 = Monitor secondario configurato, per impostazione predefinita ' +
                "`Global:DefaultSecondaryMonitor o 2 se non trovato") |
...
| `-EmbedImages` | SwitchParameter | — | — | Named | — | Incorporare le immagini come base64. |
| `-NoFallback` | SwitchParameter | — | — | Named | — | Opzione per disabilitare il comportamento di fallback. |
| `-NeverRebuild` | SwitchParameter | — | — | Named | — | Opzione per saltare l'inizializzazione e la ricostruzione del database. |
...
| `-ShowInBrowser` | SwitchParameter | — | — | Named | — | Mostra i risultati della ricerca in una galleria di immagini basata su browser. |
| `-PassThru` | SwitchParameter | — | — | Named | — | Restituisci i dati immagine come oggetti. Se utilizzato con -ShowInBrowser, mostra la galleria e restituisce gli oggetti. |
...
| `-Interactive` | SwitchParameter | — | — | Named | — | Si connetterà al browser e aggiungerà pulsanti aggiuntivi come Modifica ed Elimina. Efficace solo se utilizzato con -ShowInBrowser. |
| `-Private` | SwitchParameter | — | — | Named | — | Apre in modalità di navigazione in incognito/privata |
| `-Force` | SwitchParameter | — | — | Named | — | Abilita forzatamente la porta di debug, fermando i browser esistenti se necessario |
| `-Edge` | SwitchParameter | — | — | Named | — | Si apre in Microsoft Edge |
| `-Chrome` | SwitchParameter | — | — | Named | — | Si apre in Google Chrome |
| `-Chromium` | SwitchParameter | — | — | Named | — | Si apre in Microsoft Edge o Google Chrome, a seconda del browser predefinito |
| `-Firefox` | SwitchParameter | — | — | Named | — | Si apre in Firefox |
| `-PlayWright` | SwitchParameter | — | — | Named | — | Utilizza il browser gestito da Playwright invece del browser installato nel sistema operativo |
...
| `-NewWindow` | SwitchParameter | — | — | Named | — | Non riutilizzare la finestra del browser esistente, ma crearne una nuova |
| `-OnlyReturnHtml` | SwitchParameter | — | — | Named | — | ('Restituisci solo il codice HTML generato invece di visualizzarlo in un browser.') |
| `-ShowOnlyPictures` | SwitchParameter | — | — | Named | — | Mostra solo le immagini in un rettangolo arrotondato, nessun testo sotto. |
| `-SessionOnly` | SwitchParameter | — | — | Named | — | Utilizza impostazioni alternative memorizzate nella sessione per le preferenze AI come Lingua, Raccolte di immagini, ecc. |
| `-ClearSession` | SwitchParameter | — | — | Named | — | Cancella le impostazioni alternative memorizzate nella sessione per le preferenze AI come Lingua, Raccolte di immagini, ecc. |
| `-SkipSession` | SwitchParameter | — | — | Named | — | Non utilizzare le impostazioni alternative memorizzate nella sessione per le preferenze AI come Lingua, Raccolte di immagini, ecc. |
...
| `-MinConfidenceRatio` | Double | — | — | Named | — | Rapporto di confidenza minimo (0.0-1.0) per filtrare persone, scene e oggetti in base alla confidenza. Restituisce solo dati per persone, scene e oggetti con confidenza maggiore o uguale a questo valore. |
...
> Matrice di percorsi di directory per cercare immagini
...
> Categorie di scena da cercare, caratteri jolly consentiti.
...
> Tipo di immagine per cui filtrare (ad es., 'diurno', 'serale', 'interno', ecc.). Supporta caratteri jolly.
...
> Tipo di stile per filtrare (ad es. 'casual', 'formale', ecc.). Supporta caratteri jolly.
...
> Umore generale per il filtro (ad es. 'calmo', 'allegro', 'triste', ecc.). Supporta i caratteri jolly.
...
> Filtra per produttore della fotocamera nei metadati EXIF delle immagini. Supporta caratteri jolly.
...
> Filtra per modello di fotocamera nei metadati EXIF dell'immagine. Supporta i caratteri jolly.
...
> Filtra per intervallo di latitudine GPS nei metadati EXIF delle immagini.
...
> Filtra per intervallo di longitudine GPS nei metadati EXIF dell'immagine.
...
> Filtra per intervallo di altitudine GPS nei metadati EXIF dell'immagine (in metri).
...
> Filtra per intervallo di tempo di esposizione nei metadati EXIF dell'immagine (in secondi).
...
> Filtra per intervallo di numero F (apertura) nei metadati EXIF dell'immagine.
...
> Filtra per intervallo di sensibilità ISO nei metadati EXIF dell'immagine.
...
> Filtra per intervallo di lunghezza focale nei metadati EXIF dell'immagine (in mm).
...
> Filtra per intervallo di larghezza immagine in pixel dai metadati EXIF.
...
> Filtra per intervallo di altezza immagine in pixel dai metadati EXIF.
...
> Filtra per data di scatto dai metadati EXIF. Può essere un intervallo di date.
...
> Sequenze di tasti da inviare alla finestra del browser, vedere la documentazione per il cmdlet GenXdev\Send-Key
...
> Escape dei caratteri di controllo e dei modificatori durante l'invio dei tasti
...
> Impedisce il ritorno del focus della tastiera a PowerShell dopo l'invio dei tasti
...
> Usa Shift+Invio invece di Invio per inviare i tasti
...
> Ritardo tra diverse stringhe di input in millisecondi durante l'invio dei tasti
...
> ('Il monitor da utilizzare, 0 = predefinito, -1 = ignora, ' +
                '-2 = Monitor secondario configurato, per impostazione predefinita ' +
                "`Global:DefaultSecondaryMonitor o 2 se non trovato")
...
> Incorporare le immagini come base64.
...
> Opzione per disabilitare il comportamento di fallback.
...
> Opzione per saltare l'inizializzazione e la ricostruzione del database.
...
> Mostra i risultati della ricerca in una galleria di immagini basata su browser.
...
> Restituisci i dati immagine come oggetti. Se utilizzato con -ShowInBrowser, mostra la galleria e restituisce gli oggetti.
...
> Si connetterà al browser e aggiungerà pulsanti aggiuntivi come Modifica ed Elimina. Efficace solo se utilizzato con -ShowInBrowser.
...
> Abilita forzatamente la porta di debug, fermando i browser esistenti se necessario
...
> Si apre in Microsoft Edge o Google Chrome, a seconda del browser predefinito
...
> Utilizza il browser gestito da Playwright invece del browser installato nel sistema operativo
...
> Non riutilizzare la finestra del browser esistente, ma crearne una nuova
...
> ('Restituisci solo il codice HTML generato invece di visualizzarlo in un browser.')
...
> Mostra solo le immagini in un rettangolo arrotondato, nessun testo sotto.
...
> Utilizza impostazioni alternative memorizzate nella sessione per le preferenze AI come Lingua, Raccolte di immagini, ecc.
...
> Cancella le impostazioni alternative memorizzate nella sessione per le preferenze AI come Lingua, Raccolte di immagini, ecc.
...
> Non utilizzare le impostazioni alternative memorizzate nella sessione per le preferenze AI come Lingua, Raccolte di immagini, ecc.
...
> Rapporto di confidenza minimo (0.0-1.0) per filtrare persone, scene e oggetti in base alla confidenza. Restituisce solo dati per persone, scene e oggetti con confidenza maggiore o uguale a questo valore.
