...
> Restituisce l'oggetto processo per la finestra che ospita il terminale PowerShell.

## Description

* Attraversa la gerarchia dei processi a partire dal processo PowerShell corrente
  per individuare la finestra padre responsabile dell'hosting del terminale.
* Se il processo padre immediato non ha un handle di finestra principale,
  cerca processi simili che hanno finestre principali.
* Utile per identificare il processo della finestra del terminale effettivo (come
  Windows Terminal, ConHost, ecc.) che contiene la sessione PowerShell.
...
Restituisce il processo che ospita la sessione PowerShell corrente e ne mostra il nome.
