...
> Valuta un'affermazione usando l'IA per determinare se è vera o falsa.

## Description

Questa funzione utilizza modelli di intelligenza artificiale per valutare affermazioni e determinarne il valore di verità. Può accettare input direttamente tramite parametri, dalla pipeline o dagli appunti di sistema. La funzione restituisce un risultato booleano insieme al livello di confidenza e al ragionamento del modello di intelligenza artificiale.
...
| `-Text` | String | — | ✅ (ByValue) | 0 | — | .SYNOPSIS
    Elenca le voci che corrispondono alla query di Windows Search.
.DESCRIPTION
    Il cmdlet `Invoke-WqlSearch` esegue la query specificata utilizzando Windows Search (WDS/Windows Search Service) tramite WQL e restituisce i risultati.
    Le voci possono limitarsi a un determinato percorso specificando l'argomento `-Path`.
    Per evitare di restituire tutti i file, utilizzare l'argomento `-Limit`.
    Questo cmdlet restituisce oggetti con le proprietà recuperate dal database di Windows Search. Le proprietà disponibili dipendono dalla query.
    Nota: questo cmdlet interroga l'indice di Windows Search. I file non indicizzati non verranno restituiti.
.PARAMETER Query
    La query WQL da eseguire. Deve essere una stringa di query WQL valida.
.PARAMETER Path
    Il percorso in cui cercare. Se non specificato, la ricerca viene eseguita su tutto l'ambito.
.PARAMETER Limit
    Il numero massimo di risultati da restituire. Il valore predefinito è 100.
.PARAMETER ComputerName
    Nome del computer su cui eseguire la ricerca. Se non specificato, viene utilizzato il computer locale.
.PARAMETER Credential
    Credenziali da utilizzare per la connessione.
.EXAMPLE
    PS C:\> Invoke-WqlSearch -Query "SELECT System.ItemName FROM SYSTEMINDEX" -Limit 10

    Recupera i primi 10 nomi di elemento dall'indice di sistema.
.EXAMPLE
    PS C:\> Invoke-WqlSearch -Query "SELECT System.ItemName, System.ItemPathDisplay FROM SYSTEMINDEX WHERE System.ItemType = '.ps1'" -Path "C:\Scripts"

    Recupera i nomi e i percorsi dei file .ps1 nella directory C:\Scripts.
.EXAMPLE
    PS C:\> Invoke-WqlSearch -Query "SELECT System.ItemName, System.FileOwner FROM SYSTEMINDEX" -Limit 20 -ComputerName PC01

    Recupera i primi 20 nomi di elemento e proprietari dal computer remoto PC01.
.NOTES
    Autore: Zachary Loeber
    Sito: http://www.the-little-things.net/
.LINK
    https://github.com/zloeber/PSUtil |
| `-Instructions` | String | — | — | 1 | `''` | Istruzioni per il modello AI su come valutare l'affermazione |
...
| `-DontAddThoughtsToHistory` | SwitchParameter | — | — | Named | — | Non aggiungere pensieri del modello alla cronologia della conversazione |
...
| `-AllowDefaultTools` | SwitchParameter | — | — | Named | — | Consenti all'IA di utilizzare strumenti e funzionalità predefiniti |
...
| `-OutputMarkdownBlocksOnly` | SwitchParameter | — | — | Named | — | .PARAMETER <NomeParametro>
...
| `-Language` | String | — | — | Named | — | it |
| `-CpuThreads` | Int32 | — | — | Named | — | Numero di thread della CPU da utilizzare per l'elaborazione |
| `-SuppressRegex` | String | — | — | Named | — | Espressione regolare da sopprimere dall'output |
| `-AudioContextSize` | Int32 | — | — | Named | — | Dimensioni del contesto audio per l'elaborazione |
...
| `-EntropyThreshold` | Double | — | — | Named | — | Soglia di entropia per il filtro dell'output |
| `-LogProbThreshold` | Double | — | — | Named | — | Soglia di probabilità logaritmica per il filtraggio dell'output |
| `-NoSpeechThreshold` | Double | — | — | Named | — | Nessuna soglia di discorso per il rilevamento audio |
| `-DontSpeak` | SwitchParameter | — | — | Named | — | Disattiva l'uscita vocale |
| `-DontSpeakThoughts` | SwitchParameter | — | — | Named | — | Disabilita l'output vocale per i pensieri |
| `-NoVOX` | SwitchParameter | — | — | Named | — | Disattiva VOX (attivazione vocale) |
| `-UseDesktopAudioCapture` | SwitchParameter | — | — | Named | — | Utilizza la cattura audio del desktop come input |
| `-NoContext` | SwitchParameter | — | — | Named | — | Non utilizzare contesto nella query |
...
Il cmdlet Get-EC2Instance offre una varietà di modi per recuperare informazioni su un'istanza EC2. Fornisce proprietà come l'indirizzo IP privato, l'indirizzo IP pubblico, lo stato dell'istanza, il nome del tag, la zona di disponibilità e altro ancora. Puoi filtrare i risultati in modo da ottenere solo le istanze che corrispondono a criteri specifici. Puoi anche utilizzare questo cmdlet per avviare e arrestare istanze utilizzando il cmdlet Start-EC2Instance. |
...
> Istruzioni per il modello AI su come valutare l'affermazione
...
> Non aggiungere pensieri del modello alla cronologia della conversazione
...
> Consenti all'IA di utilizzare strumenti e funzionalità predefiniti
...
> Espressione regolare da sopprimere dall'output
...
> Utilizza la cattura audio del desktop come input
...
> Non utilizzare contesto nella query
