...
> Sposta i file che corrispondono ai criteri di ricerca in sottocartelle basate sulla data.

## Description

Cerca file utilizzando lo stesso set di parametri di `Find-Item` e sposta ogni file trovato in una sottocartella di `TargetFolder` in base alla data di creazione o data multimediale del file. Il cmdlet supporta la corrispondenza del contenuto, ricerche sull'intero drive e molti filtri. Opzionalmente può eliminare le cartelle di origine vuote dopo lo spostamento.

Tenta diverse strategie per determinare una data di creazione o acquisizione accurata per il file specificato. Le strategie includono la lettura dei metadati EXIF delle immagini, l'analisi delle informazioni data/ora dai nomi dei file e il fallback all'ora dell'ultima scrittura del file quando non sono disponibili altre informazioni affidabili.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Cartella radice di destinazione in cui i file corrispondenti verranno spostati in sottocartelle basate sulla data |
...
Move-FilesToDateFolder -TargetFolder 'D:\Archivio' -Name '*.jpg' -WhatIf
...
> Cartella radice di destinazione in cui i file corrispondenti verranno spostati in sottocartelle basate sulla data
