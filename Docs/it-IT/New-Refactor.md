...
> Crea un nuovo set di refactoring per attività di trasformazione del codice.

## Description

Crea e configura una nuova definizione di refactoring con impostazioni specificate per trasformazioni di codice basate su LLM. La funzione gestisce:
- Configurazione del refactoring
- Configurazione dei criteri di selezione e dei prompt
- Gestione delle impostazioni del modello LLM
- Integrazione con ambienti di sviluppo
- Persistenza delle definizioni di refactoring
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Name` | String | ✅ | — | 0 | — | Il nome di questo nuovo set di refactoring |
| `-PromptKey` | String | ✅ | — | 1 | — | La chiave del prompt indica quale script del prompt utilizzare |
| `-Prompt` | String | — | — | 2 | `''` | Testo del prompt personalizzato per sovrascrivere il modello |
| `-SelectionScript` | String | — | — | 3 | — | Script PowerShell per selezionare elementi da rifattorizzare |
| `-SelectionPrompt` | String | — | — | 4 | — | Prompt per la selezione del LLM |
| `-LLMQueryType` | String | — | — | 5 | `'Coding'` | Il tipo di query LLM |
...
| `-Priority` | Int32 | — | — | Named | `0` | Priorità per questo refactoring impostata |
| `-ExposedCmdLets` | GenXdev.Helpers.ExposedCmdletDefinition[] | — | — | Named | `@()` | Matrice di definizioni dei comandi PowerShell per strumenti LLM |
| `-FilesToAdd` | IO.FileInfo[] | — | — | Named | `@()` | Array di file da elaborare |
| `-PreferencesDatabasePath` | String | — | — | Named | — | Percorso del database per i file dei dati delle preferenze |
| `-AutoAddModifiedFiles` | SwitchParameter | — | — | Named | — | Aggiungerà automaticamente i file modificati alla coda |
...
| `-Code` | SwitchParameter | — | — | Named | — | Aprire file in Visual Studio Code |
| `-VisualStudio` | SwitchParameter | — | — | Named | — | Apri in Visual Studio |
| `-KeysToSend` | String[] | — | — | Named | `@()` | Tasti premuti da inviare dopo l'apertura dei file |
...
> Il nome di questo nuovo set di refactoring
...
> Testo del prompt personalizzato per sovrascrivere il modello
...
> Script PowerShell per selezionare elementi da rifattorizzare
...
> Prompt per la selezione del LLM
...
> Matrice di definizioni dei comandi PowerShell per strumenti LLM
...
> Aggiungerà automaticamente i file modificati alla coda
...
> Aprire file in Visual Studio Code
...
> Tasti premuti da inviare dopo l'apertura dei file
