...
> Apre una query di ChatGPT in un browser web.

## Description

Apre ChatGPT in un browser web, inserisce automaticamente la query specificata e la invia. Supporta più query che verranno eseguite utilizzando la funzione sottostante Open-WebsiteAndPerformQuery. Questa funzione fornisce un modo conveniente per interagire con ChatGPT di OpenAI da PowerShell con opzioni estese di configurazione del browser.
...
| `-ShowWindow` | SwitchParameter | — | — | Named | — | Mostra la finestra del browser |
...
Invia più query utilizzando il nome completo del parametro.
...
Invia una query utilizzando alias e pipeline.
...
> Mostra la finestra del browser
