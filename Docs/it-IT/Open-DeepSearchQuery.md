...
> Apre una query di DeepSeek in un browser web

## Description

Apre una query di DeepSeek nel browser web, digita la query e preme Invio per eseguire la ricerca. Questa funzione sfrutta il motore di ricerca AI di DeepSeek per fornire risposte intelligenti e approfondimenti per le query fornite.
...
Apre DeepSeek e cerca "powershell function" utilizzando le impostazioni predefinite.
...
Utilizza l'alias per cercare "funzione di PowerShell" con parametri predefiniti.
...
Apre DeepSeek in modalità schermo intero sul monitor principale per cercare "nozioni di base sul machine learning".
