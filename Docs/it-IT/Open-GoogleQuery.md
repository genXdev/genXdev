...
> Apre le query di Google in un browser web con impostazioni configurabili e ampie opzioni di personalizzazione.

## Description

Apre una o più query di Google in un browser web con supporto completo per più lingue, selezione del monitor, posizionamento della finestra, selezione del browser e automazione della tastiera. La funzione fornisce un'interfaccia ricca di funzionalità per eseguire ricerche Google con controllo preciso sul comportamento e l'aspetto del browser.

Le caratteristiche principali includono:
- Supporto per oltre 100 lingue tramite filtro linguistico di Google
- Supporto multi-monitor con selezione automatica o manuale del monitor
- Posizionamento della finestra (sinistra, destra, superiore, inferiore, centrato, schermo intero)
- Supporto per modalità di navigazione privata/incognito
...
- Elaborazione batch di query con supporto pipeline
- Automazione della pressione dei tasti nelle finestre del browser
- Passaggio intelligente dei parametri alle funzioni del browser sottostanti

La funzione formatta automaticamente le query di ricerca per il motore di ricerca Google e applica il filtro linguistico quando specificato. Tutte le funzionalità di posizionamento e gestione del browser sono ereditate dalla funzione Open-Webbrowser sottostante.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Le parole chiave da cercare su Google |
| `-Language` | String | — | — | 1 | — | La lingua per i risultati di ricerca di Google |
...
| `-Monitor` | Int32 | — | — | Named | `-1` | Il monitor da utilizzare, 0 = predefinito, -1 = scarta, -2 = monitor secondario configurato, il valore predefinito è -1 |
...
Apre una ricerca Google per "PowerShell scripting" in inglese sul monitor principale.
...
Apre una ricerca Google per "algoritmi di machine learning" in modalità schermo intero sul monitor 2 utilizzando alias.
...
Apre più ricerche di Google in Chrome tramite input pipeline, ciascuna in inglese.
...
Apre una ricerca in navigazione privata per "intelligenza artificiale" in una nuova finestra.
...
Apre una ricerca in modalità applicazione (senza controlli del browser) centrata sullo schermo.

## Parameter Details

### `-Queries <String[]>`

> Le parole chiave da cercare su Google
...
> La lingua per i risultati di ricerca di Google
...
> Il monitor da utilizzare, 0 = predefinito, -1 = scarta, -2 = monitor secondario configurato, il valore predefinito è -1
