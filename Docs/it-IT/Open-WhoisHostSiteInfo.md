...
Apre le query di informazioni sull'host Whois in un browser web per nomi di dominio o indirizzi IP. Supporta la selezione configurabile del monitor e query multiple con ampie opzioni di configurazione del browser, inclusi posizionamento della finestra, selezione del browser e funzionalità di automazione.
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | Utilizza impostazioni alternative memorizzate nella sessione per le preferenze AI come Lingua, Raccolte immagini, ecc. |
...
| `-SkipSession` | SwitchParameter | — | — | Named | — | Memorizza le impostazioni solo nelle preferenze persistenti senza influenzare la sessione per le preferenze AI come lingua, raccolte immagini, ecc. |
...
Apre le informazioni Whois per più domini sul monitor predefinito.
...
Apre le informazioni Whois per i domini dalla pipeline in modalità privata di Chrome.
