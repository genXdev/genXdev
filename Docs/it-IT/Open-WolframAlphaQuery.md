...
> Apre una query di Wolfram Alpha in un browser web.

## Description

Apre una o più query di Wolfram Alpha in un browser web. Supporta la selezione configurabile del monitor e le opzioni del browser tramite parametri dinamici. Fornisce un controllo completo del browser, inclusi posizionamento, dimensionamento e funzionalità di automazione per flussi di lavoro di calcolo scientifico avanzati.
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | Escape dei caratteri di controllo durante l'invio dei tasti. |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | Impedisci di restituire lo stato attivo a PowerShell dopo l'invio dei tasti. |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | Invia Maiusc+Invio invece del normale Invio per andare a capo quando si inviano i tasti. |
| `-SendKeyDelayMilliSeconds` | Int32 | — | — | Named | — | Ritardo tra l'invio di diverse sequenze di tasti in millisecondi. |
...
Apre una query di Wolfram Alpha per la massa del sole sul monitor predefinito.
...
Apre più query di Wolfram Alpha utilizzando l'alias per calcoli scientifici rapidi.
...
Apre una query di calcolo in modalità schermo intero con preferenza per la lingua inglese tramite input pipeline.
...
> Invia Maiusc+Invio invece del normale Invio per andare a capo quando si inviano i tasti.
