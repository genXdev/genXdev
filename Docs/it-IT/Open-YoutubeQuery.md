...
> Apre le query di ricerca di YouTube in un browser web.

## Description

Apre una o più query di ricerca di YouTube in un browser web con ampie opzioni di posizionamento e configurazione. Supporta la selezione configurabile del monitor, le impostazioni del browser e l'automazione delle sequenze di tasti tramite parametri dinamici ereditati dal cmdlet Open-Webbrowser. Ogni query viene aperta in un'istanza separata del browser con pieno controllo sul posizionamento della finestra, sul tipo di browser e sulla lingua di ricerca.
...
Apre una ricerca su YouTube per "esercitazione PowerShell" sul monitor predefinito.
...
Apre una ricerca su YouTube per "come programmare" sul monitor secondario configurato utilizzando l'alias della funzione.
