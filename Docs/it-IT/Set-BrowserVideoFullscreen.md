...
> Ingrandisce al massimo il primo elemento video trovato nella scheda del browser corrente.

## Description

Esegue codice JavaScript per individuare e massimizzare il primo elemento video sulla pagina web corrente. Il video viene impostato per coprire l'intero viewport con il massimo z-index per garantire visibilità. Le barre di scorrimento della pagina vengono nascoste per un'esperienza a schermo intero pulita.
