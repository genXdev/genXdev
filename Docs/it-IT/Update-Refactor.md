...
> Aggiorna e gestisce set di refactoring inclusi la selezione dei file e l'elaborazione.

## Description

Fornisce la gestione completa dei set di refactoring:
- Aggiunta o rimozione di file dalle code di elaborazione
- Pulizia dei file cancellati dal set
- Gestione delle informazioni di stato e monitoraggio del progresso
- Gestione della selezione e elaborazione dei file basata su LLM
- Supporto per la gestione automatica e manuale dei file
- Mantenimento di log dettagliati di tutte le operazioni
- Gestione corretta dei file cancellati (saltati a meno che non venga usato CleanUpDeletedFiles)
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Name` | String[] | — | ✅ (ByValue, ByPropertyName) | 0 | `@('*')` | Il nome del refactoring, accetta caratteri jolly *(Parameter set: )* 🌐 *Supports wildcards* |
| `-Refactor` | GenXdev.Helpers.RefactorDefinition[] | — | — | 1 | — | Il set di refactoring da aggiornare *(Parameter set: )* |
| `-FilesToAdd` | IO.FileInfo[] | — | — | 2 | `@()` | Nomi di file da aggiungere |
| `-FilesToRemove` | IO.FileInfo[] | — | — | 3 | `@()` | Nomi file da rimuovere |
| `-SelectByModifiedDateFrom` | DateTime | — | — | Named | — | Selezionare i file in base alla data di modifica da |
| `-SelectByModifiedDateTo` | DateTime | — | — | Named | — | Seleziona i file per data di modifica |
| `-SelectByCreationDateFrom` | DateTime | — | — | Named | — | Seleziona i file per data di creazione da |
| `-SelectByCreationDateTo` | DateTime | — | — | Named | — | Selezionare i file per data di creazione |
| `-PromptKey` | String | — | — | Named | — | La chiave del prompt indica quale script del prompt utilizzare |
| `-Prompt` | String | — | — | Named | `''` | La chiave del prompt indica quale script del prompt utilizzare |
| `-SelectionScript` | String | — | — | Named | — | Script PowerShell per funzione per selezionare elementi da rifattorizzare |
| `-SelectionPrompt` | String | — | — | Named | — | Se fornito, invocherà LLM per effettuare la selezione in base al contenuto dello script |
...
| `-Priority` | Int32 | — | — | Named | — | Priorità per questo refactoring impostata |
| `-ExposedCmdLets` | GenXdev.Helpers.ExposedCmdletDefinition[] | — | — | Named | `@()` | Array di definizioni di comandi PowerShell da utilizzare come strumenti durante la selezione LLM |
| `-KeysToSend` | String[] | — | — | Named | `@()` | I tasti da premere come sequenza di tasti dopo aver aperto il file |
...
| `-AskBeforeLLMSelection` | SwitchParameter | — | — | Named | — | Passa per sopprimere l'interazione utente |
| `-PerformAutoSelections` | SwitchParameter | — | — | Named | — | Attiva per abilitare l'elaborazione della selezione file basata su LLM |
| `-PerformAISelections` | SwitchParameter | — | — | Named | — | Passa a elaborare tutti i file nel set di refactoring |
| `-RetryFailedLLMSelections` | SwitchParameter | — | — | Named | — | Passa alle selezioni LLM non riuscite in modalità riprova |
| `-Clear` | SwitchParameter | — | — | Named | — | Elimina tutti i file dal set di refactoring |
| `-ClearLog` | SwitchParameter | — | — | Named | — | Cancella il log del set di refactoring |
| `-Reset` | SwitchParameter | — | — | Named | — | Iniziare dall'inizio del set di refactoring |
| `-ResetLMSelections` | SwitchParameter | — | — | Named | — | Riavvia tutte le LLMSelections |
...
| `-ReprocessModifiedFiles` | SwitchParameter | — | — | Named | — | Rielabora automaticamente i file modificati dall'ultimo aggiornamento |
...
| `-SelectByFreeRam` | SwitchParameter | — | — | Named | — | Seleziona la configurazione in base alla RAM di sistema disponibile |
| `-SelectByFreeGpuRam` | SwitchParameter | — | — | Named | — | Seleziona la configurazione in base alla RAM GPU disponibile |
...
> Il set di refactoring da aggiornare
...
> Selezionare i file in base alla data di modifica da
...
> Seleziona i file per data di modifica
...
> Seleziona i file per data di creazione da
...
> Selezionare i file per data di creazione
...
> La chiave del prompt indica quale script del prompt utilizzare
...
> Script PowerShell per funzione per selezionare elementi da rifattorizzare
...
> Se fornito, invocherà LLM per effettuare la selezione in base al contenuto dello script
...
> Priorità per questo refactoring impostata
...
> Array di definizioni di comandi PowerShell da utilizzare come strumenti durante la selezione LLM
...
> I tasti da premere come sequenza di tasti dopo aver aperto il file
...
> Passa per sopprimere l'interazione utente
...
> Attiva per abilitare l'elaborazione della selezione file basata su LLM
...
> Passa a elaborare tutti i file nel set di refactoring
...
> Passa alle selezioni LLM non riuscite in modalità riprova
...
> Elimina tutti i file dal set di refactoring
...
> Cancella il log del set di refactoring
...
> Riavvia tutte le LLMSelections
...
> Rielabora automaticamente i file modificati dall'ultimo aggiornamento
...
> Seleziona la configurazione in base alla RAM di sistema disponibile
...
> Seleziona la configurazione in base alla RAM GPU disponibile
