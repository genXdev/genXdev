...
Webブラウザインスタンスを閉じる際の細かい制御を提供します。特定のブラウザ（Edge、Chrome、Firefox）を対象とするか、すべてのブラウザを閉じることができます。メインウィンドウとバックグラウンドプロセスの両方を閉じることをサポートします。
...
| `-Chrome` | SwitchParameter | — | — | Named | — | Google Chrome ブラウザのインスタンスを閉じます *(Parameter set: )* |
| `-Chromium` | SwitchParameter | — | — | Named | — | デフォルトのChromiumベースのブラウザを閉じます *(Parameter set: )* |
...
> Microsoft Edge ブラウザのインスタンスを閉じます
...
> Google Chrome ブラウザのインスタンスを閉じます
