...
> 直接的な表現や率直な内容を、AIを用いて丁寧で専門的な企業向け表現に変換します。

## Description

この関数は、直接的なまたは威圧的な言葉遣いを、外交的でプロフェッショナルな企業コミュニケーションに変換するように入力テキストを処理します。パラメーター、パイプライン、またはシステムクリップボードから直接入力を受け付けることができます。この関数はAIモデルを活用して、元の意図を保持しながらテキストを分析し言い換えます。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Text` | String | — | ✅ (ByValue) | 0 | — | 私たちは、このテキストをコーポレートスピークに変換する必要があります。 |
...
> 私たちは、このテキストをコーポレートスピークに変換する必要があります。
