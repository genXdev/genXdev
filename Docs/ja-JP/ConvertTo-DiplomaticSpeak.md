...
> 直接的な表現や率直な文章を、丁寧で tactful な外交的表現に変換します。

## Description

この関数は、ユーザーの入力を直接的または率直な表現から、外交的で tact のある言葉遣いに変換し、ハイレベルな議論、交渉、または正式なコミュニケーションに適したものにします。この関数はAI言語モデルを使用して、元の意図を維持しながら、口調を和らげ、メッセージをより外交的かつプロフェッショナルにします。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Text` | String | — | ✅ (ByValue) | 0 | — | 御覧のテキストを外交的な表現に変換します。 |
