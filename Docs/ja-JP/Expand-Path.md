...
> 指定されたファイル参照を完全なパス名に展開します。

## Description

指定されたファイル参照を、ユーザーの現在のディレクトリを基準とした完全なパス名に展開します。オプションで、ディレクトリやファイルが存在することを確認できます。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-FilePath` | String | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 展開するパス |
| `-CreateDirectory` | SwitchParameter | — | — | Named | — | ディレクトリが存在しない場合は作成します |
| `-CreateFile` | SwitchParameter | — | — | Named | — | 存在しない場合は空のファイルを作成します |
| `-DeleteExistingFile` | SwitchParameter | — | — | Named | — | ファイルが既に存在する場合は削除します |
| `-ForceDrive` | Char | — | — | Named | `'*'` | 特定のドライブを強制的に使用します。 |
| `-FileMustExist` | SwitchParameter | — | — | Named | — | ファイルが存在しない場合、スローされます。 |
| `-DirectoryMustExist` | SwitchParameter | — | — | Named | — | ディレクトリが存在しない場合はエラーになります。 |
