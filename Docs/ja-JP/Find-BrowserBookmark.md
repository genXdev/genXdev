...
> 1つ以上のWebブラウザからブックマークを検索します。

## Description

Microsoft Edge、Google Chrome、またはMozilla Firefoxのブックマークを検索します。
名前、URL、またはフォルダパスが1つ以上の検索クエリに一致するブックマークを返します。
クエリが指定されていない場合は、選択されたブラウザのすべてのブックマークを返します。
...
| `-Firefox` | SwitchParameter | — | — | Named | — | Firefoxのブックマークを検索 |
| `-Count` | Int32 | — | — | Named | `99999999` | 返す結果の最大数 |
| `-PassThru` | SwitchParameter | — | — | Named | — | URLではなくブックマークオブジェクトを返してください |
