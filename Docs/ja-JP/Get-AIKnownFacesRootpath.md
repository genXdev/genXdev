...
> GenXdev.AI 操作で使用される顔画像ファイル用の構成済みディレクトリを取得します。

## Description

この関数は、GenXdev.AIモジュールが顔認識やAI操作で使用するグローバル顔ディレクトリを取得します。最初にグローバル変数を確認し（SkipSessionが指定されていない場合）、次に永続的な設定をフォールバックし、最後にシステムのデフォルトを使用します。
...
現在構成されている顔ディレクトリをグローバル変数または設定から取得します。
...
永続的な設定から構成された顔ディレクトリのみを取得し、セッション設定は無視します。
...
セッションの面ディレクトリ設定をクリアしてから、永続的な設定からディレクトリを取得します。
