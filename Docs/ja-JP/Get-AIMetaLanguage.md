...
> イメージメタデータ操作に対して構成されている既定の言語を取得します。

## Description

* このコマンドレットは、GenXdev.AI モジュールが画像メタデータ操作に使用する既定の言語を取得します。
* まずグローバル変数を確認し (SkipSession が指定されていない場合)、次に永続的な設定にフォールバックし、最後にシステムの既定値を使用します。
...
| `-SessionOnly` | SwitchParameter | — | — | Named | `False` | 言語、画像コレクションなどのAI設定のためにセッションに保存された代替設定を使用する |
| `-ClearSession` | SwitchParameter | — | — | Named | `False` | セッション設定（グローバル変数）をクリアしてから取得する |
| `-SkipSession` | SwitchParameter | — | — | Named | `False` | AIの設定（言語、画像コレクションなど）のためにセッションに保存された代替設定を使用しない |
...
グローバル変数またはユーザー設定から現在構成されている言語を取得します。
...
永続的な設定から構成された言語を取得します。セッションは無視します。
