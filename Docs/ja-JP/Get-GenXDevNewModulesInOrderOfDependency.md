...
> GenXDev モジュールを依存関係の順序で取得します。

## Description

この関数は、正しい依存関係の順序で配置されたGenXDevモジュールのリストを返し、適切なモジュールの読み込みを保証します。まずすべてのモジュール情報を取得し、次に依存関係に基づいてモジュールを並べ替えます。コアモジュールから始まり、依存モジュールで終了します。これにより、モジュールが正しい順序で読み込まれます。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ModuleName` | String[] | — | ✅ (ByValue, ByPropertyName) | 0 | `@('GenXdev*')` | フィルター処理する1つ以上のモジュール名 |
