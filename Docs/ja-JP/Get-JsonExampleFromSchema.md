...
> JSON スキーマ定義から人間が読めるサンプル JSON 文字列を生成します。

## Description

任意のJSONスキーマ（OpenAI json_schema形式または生のスキーマ）を、LLMシステム指示に注入するのに適したコンパクトなJSON文字列例に変換します。ネストされたオブジェクト、配列、スカラー型、列挙型、共用型、oneOf/anyOf/allOf合成、date-timeやemailなどの形式ヒントを処理します。

この関数は、スキーマを再帰的に走査してPowerShellオブジェクトツリーを構築し、ConvertTo-Jsonを介してコンパクトなJSON文字列にシリアル化します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Schema` | Object | — | — | 0 | — | JSONスキーマ定義（JSON文字列またはPSCustomObject）。OpenAIのjson_schemaラッパーフォーマットと生のJSONスキーマオブジェクトをサポートします。 |
...
@{response=生成されます: {"items":[{"id":0,"label":"..."}]}}

## Parameter Details

### `-Schema <Object>`

> JSONスキーマ定義（JSON文字列またはPSCustomObject）。OpenAIのjson_schemaラッパーフォーマットと生のJSONスキーマオブジェクトをサポートします。
