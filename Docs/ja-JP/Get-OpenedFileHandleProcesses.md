...
> 指定されたファイルへのファイルハンドルを開いているプロセスを取得します。

## Description

* Sysinternals の handle.exe ツールを使用して、指定された 1 つ以上のファイルに対して現在開いているハンドルを持つプロセスを識別します。
* 削除や変更などのファイル操作を妨げているプロセスを特定するのに役立ちます。
* Sysinternals スイートの handle.exe がインストールされ、システムパスで使用可能である必要があります。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-FilePath` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 開いているハンドルを確認するファイルへのパス |
...
パイプライン入力を介して複数のファイルをチェックし、開いているハンドルを持つプロセスを検出します。

## Parameter Details

### `-FilePath <String[]>`

> 開いているハンドルを確認するファイルへのパス
