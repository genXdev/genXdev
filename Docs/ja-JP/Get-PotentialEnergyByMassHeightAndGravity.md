...
| `-HeightInMeters` | Double | ✅ | — | 1 | `0` | 高さ（メートル） |
| `-GravityInMetersPerSecondSquared` | Double | — | — | 2 | `0` | 重力 (m/s²) (デフォルト: 9.81) |
| `-As` | String | — | — | 3 | — | エネルギーの出力単位 |
...
10kgの質量が5メートルの高さにある場合の重力ポテンシャルエネルギーを計算し、結果をカロリーで出力します。
...
位置パラメーターを使用して位置エネルギーを計算する方法を示します。
