...
> SQL Server データベースからビューの一覧を取得します。

## Description

指定されたSQL Serverデータベースからすべてのビュー名を取得します。さらなるデータベース操作に使用できるビュー名の配列を返します。この関数は柔軟性のために2つのパラメーターセットをサポートしています。接続文字列を指定するか、サーバーとデータベース名を指定します。
