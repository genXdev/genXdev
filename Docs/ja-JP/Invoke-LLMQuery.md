...
> OpenAI互換の大規模言語チャット補完APIにクエリを送信し、応答を処理します。

## Description

この関数は、OpenAI互換の大規模言語チャット補完APIにクエリを送信し、応答を処理します。テキストおよび画像の入力に対応し、ツール関数の呼び出しを処理し、テキストや音声を含むさまざまなチャットモードで動作します。

この関数は、LLMインタラクションの包括的なサポートを提供します。
- テキストおよび画像の入力処理
- ツール関数の呼び出しとコマンド実行
- インタラクティブチャットモード（テキストおよび音声）
- モデルの初期化と設定
- 応答のフォーマットと処理
- セッション管理とキャッシュ
- ウィンドウの位置と表示制御
...
エイリアスを使用して、デフォルトパラメータでクエリを送信します。
