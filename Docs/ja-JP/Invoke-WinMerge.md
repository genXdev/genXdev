...
> WinMergeを起動して、2つのファイルを並べて比較します。

## Description

WinMergeアプリケーションを起動して、ソースファイルとターゲットファイルを横並びの差分ビューで比較します。この関数は、入力ファイルが両方存在することを検証し、WinMergeが適切にインストールされていることを確認してから起動します。WinMergeが閉じるまで実行を一時停止するオプションの待機機能を提供します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-SourcecodeFilePath` | String | ✅ | — | 0 | — | 比較元のソースファイルへのパス |
| `-TargetcodeFilePath` | String | ✅ | — | 1 | — | 比較対象となるターゲットファイルへのパス |
| `-Wait` | SwitchParameter | — | — | Named | — | 続行する前に WinMerge が閉じるのを待ってください |
