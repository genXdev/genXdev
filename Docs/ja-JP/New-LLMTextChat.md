...
AI機能を備えたインタラクティブなチャットセッションを開始します。ユーザーは会話中にPowerShell関数を追加または削除したり、PowerShellコマンドを実行したりできます。この関数は、豊富なツール統合とカスタマイズオプションを備えたAI搭載の会話のための包括的なインターフェースを提供します。
