...
> 指定された検索用語またはURLに対して、可能なすべてのクエリタイプを開きます。

## Description

URL と検索用語の両方を処理しながら、Webページクエリを処理するすべてのコマンドレットを実行します。URL の場合はサイト固有のクエリを実行し、検索用語の場合は一般的な Web クエリを実行します。この関数は、GenXdev.Queries モジュールファミリから利用可能なすべてのクエリコマンドレットを動的に検出して実行します。
...
| `-ShowWindow` | SwitchParameter | — | — | Named | — | 全画面モードで開く |
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | ブラウザプロファイルに対してセッションのみモードを使用します。 |
| `-ClearSession` | SwitchParameter | — | — | Named | — | ブラウザセッションを開く前にクリアします。 |
| `-SkipSession` | SwitchParameter | — | — | Named | — | ブラウザーのセッション復元をスキップします。 |
...
「powershell スクリプティング」に関するすべての可能なクエリタイプを既定のモニターで開きます。
...
エイリアスと短いパラメータを使用して、GitHub URLの可能なすべてのクエリタイプを開きます。
...
> ブラウザプロファイルに対してセッションのみモードを使用します。
...
- [ConvertTo-Uris](https://github.com/genXdev/genXdev/blob/main/Docs/ja-JP/ConvertTo-Uris.md)
