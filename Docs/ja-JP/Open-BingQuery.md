...
> Bing検索クエリをWebブラウザで開きます。

## Description

1つ以上のBing検索クエリを、設定可能なブラウザ設定とコマンドラインスイッチを使用してWebブラウザで開きます。この関数は、ウィンドウ位置、モニター選択、プライベートブラウジング、キーストローク自動化などの豊富なブラウザカスタマイズオプションを備えたBing検索の包括的なインターフェースを提供します。

この関数は自動的にクエリをURLエンコードし、Bingの検索API用にフォーマットします。Edge、Chrome、Firefoxなどの主要ブラウザをすべてサポートし、プライベートブラウジング、アプリケーションモード、精密なウィンドウ管理、キーストローク自動化などの機能を備えています。
...
既定のモニターで「PowerShell スクリプティング」のBing検索を開きます。
...
プライベートブラウジングセッションを開き、「機械学習」を英語の結果で検索します。
...
パイプラインを介して複数の用語を検索し、インストールされているすべてのブラウザで結果を開き、ウィンドウを画面の左側に配置します。
