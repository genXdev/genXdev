...
> BuiltWith のウェブサイトクエリをウェブブラウザで開きます。

## Description

構成可能なモニター設定とブラウザ動作オプションを備え、BuiltWithクエリをWebブラウザで開きます。この関数は、BuiltWith.comのテクノロジープロファイリングサービスにクエリを実行することで、ウェブサイトの技術スタックを分析します。このサービスは、ウェブサイトで使用されているウェブ技術、フレームワーク、およびサービスに関する詳細情報を提供します。

BuiltWith.comは、あらゆるウェブサイトが使用するウェブ技術、フレームワーク、コンテンツ管理システム、分析ツール、ホスティングプロバイダー、その他のソフトウェアコンポーネントを識別する包括的なテクノロジールックアップサービスです。この関数は、BuiltWithのウェブサイトに手動で移動することなく、技術スタックを迅速に分析するためのPowerShellインターフェースを提供します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | BuiltWith.comでクエリを実行するウェブサイトのURLまたはドメイン |
| `-Language` | String | — | — | 1 | — | 返された検索結果の言語 |
| `-Monitor` | Int32 | — | — | 2 | `-1` | 使用するモニター、0 = デフォルト、-1 = 破棄、-2 = 設定済みのセカンダリモニター、デフォルトは-1、位置指定なし |
...
既定のモニターで Microsoft.com の BuiltWith テクノロジー分析を開きます。
...
パイプライン入力を使用してMicrosoft.comを分析し、モニターの位置情報は破棄します。

## Parameter Details

### `-Queries <String[]>`

> BuiltWith.comでクエリを実行するウェブサイトのURLまたはドメイン
