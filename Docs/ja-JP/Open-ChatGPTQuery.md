...
> ChatGPT のクエリを Web ブラウザーで開きます。

## Description

ウェブブラウザでChatGPTを開き、指定されたクエリを自動的に入力して送信します。基盤となるOpen-WebsiteAndPerformQuery関数を使用して実行される複数のクエリをサポートします。この関数は、広範なブラウザ設定オプションを使用してPowerShellからOpenAIのChatGPTと対話する便利な方法を提供します。
...
| `-ShowWindow` | SwitchParameter | — | — | Named | — | ブラウザウィンドウを表示 |
