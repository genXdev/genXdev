...
この関数は、クラウドベースのさまざまなAIチャットサービスをWebブラウザで開くための統一インターフェースを提供します。ChatGPT、BingCopilot、Google Gemini、X Grok、DeepSearch、GitHub Copilotを含む複数のAIプラットフォームをサポートしています。関数は自動的に適切なエンドポイント固有の関数を選択し、ブラウザ設定とウィンドウ配置に関するすべての関連パラメータを透過的に渡します。
...
| `-EndPoint` | String | — | — | 1 | `'XGrok'` | クエリを実行するためのエンドポイント |
...
エイリアスを使用して、機械学習についてGoogle Geminiに質問します。
