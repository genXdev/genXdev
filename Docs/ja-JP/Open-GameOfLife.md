...
> ConwayのライフゲームのシミュレーションをWebブラウザで開きます。

## Description

豊富な配置オプションと設定オプションを備えたインタラクティブなConwayのライフゲームシミュレーションをWebブラウザウィンドウで開きます。Conwayのライフゲームは、1970年に数学者ジョン・コンウェイによって考案されたセル・オートマトンであり、生きているか死んでいるかの2つの状態のいずれかを持つセルのグリッドで構成されています。

この関数は、ウィンドウの配置、ブラウザの選択、プライベートブラウジング、自動操作機能など、包括的なブラウザ制御を提供します。シミュレーションは https://conway.genxdev.net/ で実行され、さまざまな操作モードをサポートしています。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Language` | String | — | — | 0 | — | ブラウザインターフェースと言語設定のローカライゼーション |
...
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | キーストロークをブラウザウィンドウに送信した後、PowerShellにキーボードフォーカスが戻るのを防ぎます。 |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | ブラウザにキーストロークを送信する際、通常のEnterキーの代わりにShift+Enterを使用します。 |
...
| `-RestoreFocus` | SwitchParameter | — | — | Named | — | ブラウザを開いた後、PowerShellのウィンドウのフォーカスを復元します。 |
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | ブラウザプロファイルにセッションのみモードを使用（閉じるとクッキーとデータを消去） |
| `-ClearSession` | SwitchParameter | — | — | Named | — | ブラウザのセッション/プロファイルデータを開く前にクリアしてください。 |
| `-SkipSession` | SwitchParameter | — | — | Named | — | 以前のブラウザーセッションの復元をスキップします。 |
...
モニター1でフルスクリーンモードでコンウェイのライフゲームを開きます。
...
フランス語のライフゲームをChromeのプライベートモードで開きます。
...
エイリアスを使用して、プライマリモニタでライフゲームをアプリケーションモードで開きます。
...
Microsoft Edge で Conway's Game of Life を開き、左側に配置して特定の寸法で表示します。

## Parameter Details

### `-Language <String>`

> ブラウザインターフェースと言語設定のローカライゼーション
...
> ブラウザウィンドウにキーストロークを送信する際は、制御文字をエスケープしてください。
...
> キーストロークをブラウザウィンドウに送信した後、PowerShellにキーボードフォーカスが戻るのを防ぎます。
...
> ブラウザにキーストロークを送信する際、通常のEnterキーの代わりにShift+Enterを使用します。
...
> ブラウザを開いた後、PowerShellのウィンドウのフォーカスを復元します。
...
> ブラウザプロファイルにセッションのみモードを使用（閉じるとクッキーとデータを消去）
...
> ブラウザのセッション/プロファイルデータを開く前にクリアしてください。
