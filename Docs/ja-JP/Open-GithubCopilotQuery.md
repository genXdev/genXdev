...
> Github CoPilotのクエリをウェブブラウザで開きます。

## Description

GitHub CoPilotのクエリをウェブブラウザで開き、指定されたクエリを自動入力します。この関数は、ページが読み込まれるのを待ってからクエリを入力します。この関数は、基礎となるOpen-WebsiteAndPerformQuery関数を使用してブラウザの自動化を処理し、PowerShellからGitHub Copilotとやり取りするための便利な方法を提供し、広範なブラウザ構成オプションを備えています。
