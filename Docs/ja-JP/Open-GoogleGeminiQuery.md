...
> Google Gemini のクエリをウェブブラウザで開く

## Description

WebブラウザでGoogle Geminiクエリを開き、自動的にクエリを入力して送信します。これにより、GoogleのGemini AIと迅速に対話できます。この関数は、モニターの選択、ウィンドウの配置、キーボード自動化機能を含む広範なブラウザ制御をサポートします。
...
Google Geminiを開き、「PowerShellとは何ですか？」というクエリを送信します。
...
エイリアスを使用して、パイプライン入力経由で配列に関するクエリを送信します。
