...
> 構成可能な設定と広範なカスタマイズオプションを使用して、GoogleクエリをWebブラウザで開きます。

## Description

1つ以上のGoogleクエリをWebブラウザで開きます。多言語対応、モニター選択、ウィンドウ配置、ブラウザ選択、キーボード自動化を包括的にサポートします。この関数は、ブラウザの動作と外観を正確に制御しながらGoogle検索を実行するための豊富な機能を提供します。

主な機能は以下の通りです。
- Googleの言語フィルタリングによる100以上の言語のサポート
...
- フォーカス管理とウィンドウ操作
- パイプラインサポートによるバッチクエリ処理
- ブラウザウィンドウへのキーストローク自動化
- 基盤となるブラウザ関数へのスマートなパラメータパススルー

この関数は、Googleの検索エンジン向けに検索クエリを自動的にフォーマットし、指定された場合に言語フィルタリングを適用します。すべてのブラウザ配置および管理機能は、基盤となるOpen-Webbrowser関数から継承されています。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Googleで検索するための検索語句 |
...
| `-Monitor` | Int32 | — | — | Named | `-1` | 使用するモニター、0 = デフォルト、-1 = 破棄、-2 = 設定済みのセカンダリモニター、デフォルトは-1 |
...
プライマリモニタで、英語の「PowerShell scripting」のGoogle検索を開きます。
...
エイリアスを使用して、モニター2で全画面モードの「機械学習アルゴリズム」のGoogle検索を開きます。
...
パイプライン入力を介して複数のGoogle検索をChromeで開きます（英語）。
...
アプリケーションモードで検索を開き（ブラウザコントロールなし）、画面中央に配置します。
...
> 使用するモニター、0 = デフォルト、-1 = 破棄、-2 = 設定済みのセカンダリモニター、デフォルトは-1
