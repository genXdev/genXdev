...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | ブラウザプロファイルにセッションのみモードを使用する（永続データなし） |
| `-ClearSession` | SwitchParameter | — | — | Named | — | 開く前にブラウザのセッション/プロファイルデータを消去する |
| `-SkipSession` | SwitchParameter | — | — | Named | — | ブラウザセッション/プロファイルを使用しない（デフォルトを使用） |
