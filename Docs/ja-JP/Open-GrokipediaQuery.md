...
> GrokipediaのクエリをWebブラウザで開きます。

## Description

1つ以上のGrokipedia検索クエリを、幅広い設定オプションを備えたWebブラウザで開きます。ブラウザの動作、モニターの選択、ウィンドウの配置を構成可能にサポートします。クエリはURLエンコードされ、指定された言語のGrokipediaドメインを使用して、完全なローカライゼーション対応で開かれます。

この関数は、プライベートブラウジングモード、ウィンドウの配置、キーボード自動化、マルチモニターサポートなど、包括的なブラウザ制御を提供します。国際的なGrokipediaドメインに対して、URLエンコードと言語コードマッピングを自動的に処理します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 実行するGrokipedia検索クエリ |
...
既定のモニターで「PowerShell」の英語のGrokipedia検索を開きます。
...
エイリアスと位置パラメーターを使用してGrokipedia検索を開きます。
...
プライベートブラウジングモードを使用して、Grokkipediaで複数の用語を検索します。
