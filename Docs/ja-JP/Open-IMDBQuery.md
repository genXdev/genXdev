...
> IMDBの検索クエリをウェブブラウザで開きます。

## Description

拡張設定オプションを使用して、Webブラウザで「Internet Movie Database」クエリを開きます。この関数はIMDBの検索機能に対する便利なラッパーを提供し、PowerShellから直接映画、テレビ番組、俳優、その他のエンターテイメントコンテンツを検索できます。

この関数は検索語を受け入れ、適切なIMDB検索URLを構築し、指定されたWebブラウザでカスタマイズ可能なウィンドウ配置、ブラウザ選択、表示オプションで開きます。Edge、Chrome、Firefoxを含むすべての主要ブラウザをサポートし、プライベートブラウジング、全画面モード、ウィンドウ管理のオプションを備えています。

主な機能にはマルチモニターサポート、言語ローカライゼーション、キーボード自動化、包括的なブラウザ設定オプションが含まれます。この関数は複数の検索クエリを同時に処理でき、対話型使用と自動ワークフローの両方にオプションを提供します。
...
| `-DisablePopupBlocker` | SwitchParameter | — | — | Named | — | ブラウザのポップアップブロック機能を無効にします |
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | キーストロークをブラウザに送信する際に制御文字をエスケープします。 |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | キーストローク送信後、PowerShellへのキーボードフォーカスの戻りを防ぎます |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | キー送信時に通常のEnterではなくShift+Enterを使用して改行します |
...
「マトリックス」のIMDB検索を、エイリアスと短縮パラメータを使用して開きます。
...
IMDBで複数の映画をフランス語で検索し、Chromeで結果を表示します。
...
IMDBでトム・ハンクスを全画面モードで検索し、その後フォーカスをPowerShellに戻します。
...
> キーストロークをブラウザに送信する際に制御文字をエスケープします。
...
> キーストローク送信後、PowerShellへのキーボードフォーカスの戻りを防ぎます
...
> キー送信時に通常のEnterではなくShift+Enterを使用して改行します
