...
> ウェブブラウザでInstantStreetViewクエリを開きます。

## Description

InstantStreetViewクエリを、Webブラウザのコマンドラインスイッチを使用して構成可能な方法で開きます。複数のクエリとモニターの選択をサポートします。ウィンドウの配置、言語設定、ブラウザ固有の機能など、包括的なブラウザ制御を提供します。

この関数は、位置情報クエリをURLエンコードして指定されたブラウザで開くことにより、InstantStreetViewのURLを自動的に構築します。Edge、Chrome、Firefoxなど、主要なブラウザをすべてサポートし、広範なカスタマイズオプションを提供します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | InstantStreetViewで表示する位置情報のクエリ |
...
プライマリモニターのデフォルトブラウザでタイムズスクエアのInstantStreetViewを開きます。
...
エイリアス 'isv' を使用してエッフェル塔のInstantStreetViewを開き、セカンダリモニタに表示します。
...
InstantStreetViewをプライベートブラウジングモードで英語でビッグベンに開きます。
...
パイプラインを介して複数のInstantStreetViewクエリを開き、Chromeブラウザで画面の左側に配置します。

## Parameter Details

### `-Queries <String[]>`

> InstantStreetViewで表示する位置情報のクエリ
