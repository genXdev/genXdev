...
> 指定された検索エンジンで検索クエリを開き、Webブラウザを使用します。

## Description

この関数は、Google、Bing、GitHub、Wikipedia、YouTubeなどを含む複数の検索エンジンを横断して検索するための統一インターフェースを提供します。ウィンドウの位置、サイズ、ブラウザの選択、言語設定など、広範なブラウザ設定オプションをサポートしています。この関数は、クエリを適切な検索エンジン固有の関数に動的にルーティングします。
...
| `-EndPoint` | String | — | — | 1 | `'Google'` | クエリを実行するためのエンドポイント |
| `-Language` | String | — | — | 2 | — | 返された検索結果の言語 |
| `-Monitor` | Int32 | — | — | 3 | `-1` | 使用するモニター、0 = デフォルト、-1 = 破棄、-2 = 設定済みのセカンダリモニター、デフォルトは-1、位置指定なし |
...
Googleで英語の結果で「PowerShell cmdlets」を検索します。
...
エイリアス 'q' を使用して、ChromeのシークレットモードでBing上で「GitHub PowerShell」を検索します。
