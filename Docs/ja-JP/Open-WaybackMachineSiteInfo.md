...
> WaybackMachineのサイト情報をウェブブラウザで開きます。

## Description

ウェブブラウザでWayback Machineクエリを開き、指定されたURLまたはウェブサイトの過去のバージョンを表示します。複数のクエリとモニター選択をサポートします。Internet ArchiveのWayback Machineサービスを通じてアーカイブされたウェブコンテンツへのアクセスを提供します。
...
| `-Language` | String | — | — | 1 | — | ブラウザーインターフェースとコンテンツの言語設定 |
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | Wayback Machine の設定に、セッションに保存された代替設定を使用する |
| `-ClearSession` | SwitchParameter | — | — | Named | — | Wayback Machineの設定に関する代替保存場所をセッションからクリアする |
...
既定のモニターで example.com のWayback Machineアーカイブを開きます。
...
エイリアスを使用して example.com の Wayback Machine アーカイブを開きます（モニター位置調整は破棄）。
...
Chromeのシークレットモードでmicrosoft.comのWayback Machineアーカイブを開きます。
...
> Wayback Machine の設定に、セッションに保存された代替設定を使用する
...
> Wayback Machineの設定に関する代替保存場所をセッションからクリアする
