...
> 1 つ以上のブラウザ ウィンドウで URL を開き、位置とスタイルをオプションで指定できます。

## Description

この関数は、ブラウザ起動の高度なラッパーを提供し、ウィンドウの配置、ブラウザの選択、動作のカスタマイズに関する豊富なオプションを備えています。Edge、Chrome、Firefoxなど複数のブラウザをサポートし、プライベートブラウジング、アプリケーションモード、正確なウィンドウ管理などの機能を提供します。
...
この関数は、システムの機能を自動的に検出し、それに応じて動作を調整できます。システムにインストールされていないブラウザについては、エラーなく処理をスキップします。
...
| `-Force` | SwitchParameter | — | — | Named | — | 新しいブラウザインスタンスを開く前に、既存のブラウザインスタンスを強制的に閉じます |
...
| `-PreferPlaywrightBrowser` | SwitchParameter | — | — | Named | — | チャンネル経由でOSにインストールされたブラウザではなく、バンドルされたPlaywrightブラウザバイナリを使用します。Chrome、Edge、Chromiumモードにのみ影響します。-PlayWrightを暗黙的に指定します。 |
...
Playwright 管理下の Chromium ブラウザで GitHub を開きます。
...
エイリアスを使用してプライマリモニターでGoogleを全画面モードで開きます。
...
インストールされているすべてのブラウザでパイプラインを介して複数のURLを開きます。
...
既に開いているブラウザウィンドウをプライマリモニタの右側に再配置します。
...
ブラウザコントロールなしでアプリモードでウェブアプリケーションを開きます。
...
> 新しいブラウザインスタンスを開く前に、既存のブラウザインスタンスを強制的に閉じます
...
> チャンネル経由でOSにインストールされたブラウザではなく、バンドルされたPlaywrightブラウザバイナリを使用します。Chrome、Edge、Chromiumモードにのみ影響します。-PlayWrightを暗黙的に指定します。
