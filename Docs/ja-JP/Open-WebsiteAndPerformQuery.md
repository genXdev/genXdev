...
この関数は、デフォルトのWebブラウザでURLを開き、1つ以上のクエリを入力して送信します。基盤となるOpen-Webbrowser関数を通じて、広範なモニター選択とウィンドウ配置オプションをサポートします。

この関数は、各クエリを自動的にクリップボードにコピーし、キーボード自動化を使用してブラウザウィンドウに送信します。元のクリップボードの内容を保存し、すべてのクエリが処理された後に復元します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Url` | String | ✅ | ✅ (ByPropertyName) | 0 | — | 開くWebサイトのURL |
...
デフォルトモニターでGoogleを開き、「PowerShell チュートリアル」を検索します。
