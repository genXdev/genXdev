...
> オランダ語版ウィキペディアの検索をウェブブラウザで開きます。

## Description

Webブラウザで「Wikipedia - オランダ」のクエリを開きます。表示オプションは構成可能です。複数の用語の検索と、結果を表示するモニターの指定をサポートします。
...
| `-SideBySide` | SwitchParameter | — | — | Named | — | 2つのブラウザウィンドウを並べて開きます |
| `-SessionOnly` | SwitchParameter | — | — | Named | — | セッションのみのCookieを使用する（永続的なCookieは使用しない） |
| `-ClearSession` | SwitchParameter | — | — | Named | — | ブラウザのセッションデータをクリアしてから開く |
| `-SkipSession` | SwitchParameter | — | — | Named | — | 前回のブラウザセッションの復元をスキップ |
...
> セッションのみのCookieを使用する（永続的なCookieは使用しない）
