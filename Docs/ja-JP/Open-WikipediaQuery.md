...
> Wikipediaのクエリをウェブブラウザで開きます。

## Description

設定可能なブラウザ動作、モニター選択、ウィンドウ配置をサポートし、1つ以上のWikipedia検索クエリをWebブラウザで開きます。クエリはURLエンコードされ、指定された言語のWikipediaドメインを使用して完全なローカライズ対応で開かれます。この関数は、プライベートブラウジングモード、ウィンドウ配置、キーボード自動化、マルチモニターサポートなど、包括的なブラウザ制御を提供します。国際的なWikipediaドメインに対して、URLエンコードと言語コードマッピングを自動的に処理します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 実行するWikipedia検索クエリ |
| `-Language` | String | — | — | 1 | — | Wikipediaの検索結果の言語 |
...
| `-PassThru` | SwitchParameter | — | — | Named | — | ブラウザプロセスを表すPowerShellプロセスオブジェクトを返します。 |
...
既定のモニターで「PowerShell」の英語版Wikipedia検索を開きます。
...
エイリアスと位置パラメータを使用してWikipedia検索を開きます。
...
プライベートブラウズモードを使用して、ドイツ語版ウィキペディアで複数の用語を検索します。
...
> ブラウザプロセスを表すPowerShellプロセスオブジェクトを返します。
