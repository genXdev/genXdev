...
> 設定可能なオプションを使用して、WebブラウザでYABゲームを開きます。

## Description

指定されたブラウザでYABゲームのWebサイトを開き、さまざまな表示オプションと動作オプションを提供します。観戦モードを含むさまざまなゲームモードをサポートしています。YABゲームはオンラインパズルゲームで、インタラクティブにプレイするか、AIが自動でプレイする観戦モードで視聴することができます。
...
| `-Monitor` | Int32 | — | — | Named | `-2` | 使用するモニター、0 = デフォルト、-1 = 破棄、-2 = 設定済みのセカンダリモニター、デフォルトは-1、位置指定なし |
| `-FullScreen` | SwitchParameter | — | — | Named | — | 全画面モードで開く |
| `-ShowWindow` | SwitchParameter | — | — | Named | — | ブラウザウィンドウを表示する（最小化または非表示にしない） |
...
Chromeブラウザを使用してYABゲームを観戦モードで開きます。フルスクリーンではなく、英語のインターフェースです。
...
エイリアスを使用してすばやくアクセスできるように、モニター2でプライベートブラウジングを使用してEdgeブラウザーでYABゲームを開きます。
...
- [Open-Timeline](https://github.com/genXdev/genXdev/blob/main/Docs/ja-JP/Open-Timeline.md)
