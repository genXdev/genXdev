...
> YouTubeの検索クエリをウェブブラウザで開きます。

## Description

1つ以上のYouTube検索クエリを、高度な位置指定と設定オプションを備えたWebブラウザで開きます。設定可能なモニター選択、ブラウザ設定、およびOpen-Webbrowserコマンドレットから継承された動的パラメーターによるキーストローク自動化をサポートします。各クエリは、ウィンドウの位置、ブラウザの種類、検索言語を完全に制御できる個別のブラウザインスタンスで開かれます。
...
既定のモニターで「PowerShell チュートリアル」のYouTube検索を開きます。
...
設定されたセカンダリモニターで「how to code」のYouTube検索を開きます（関数エイリアスを使用）。
