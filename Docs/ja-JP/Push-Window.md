...
> 現在のウィンドウをオプションの変更を加えてウィンドウスタックにプッシュします。

## Description

現在フォーカスされているウィンドウのウィンドウヘルパーをスタックにプッシュします。
プッシュの前後にさまざまなウィンドウ変換を適用できます。
この関数は現在のウィンドウをキャプチャし、後で Pop-Window で取得できるように保存します。
オプションで、位置指定、サイズ変更、視覚効果を適用できます。
