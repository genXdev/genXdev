...
> ウィンドウまたはプロセスにシミュレートされたキーストロークを送信します。

## Description

この関数は、Windows Script Host Shellオブジェクトを使用して、ターゲットウィンドウまたはプロセスにキーボード入力を送信します。プロセス名、プロセスID、またはウィンドウハンドルによってウィンドウをターゲットにできます。この関数は、特殊キーシーケンス、エスケープ文字、および信頼性の高いキー配信のためのさまざまなタイミングオプションをサポートしています。
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | 入力テキスト内の制御文字と修飾子をエスケープする |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | キー送信後にターゲットウィンドウにキーボードフォーカスを保持する |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | 改行にはEnterの代わりにShift+Enterを使用してください |
| `-SendKeyDelayMilliSeconds` | Int32 | — | — | Named | `10` | 異なる入力文字列間の遅延（ミリ秒） |
...
> キー送信後にターゲットウィンドウにキーボードフォーカスを保持する
...
> 改行にはEnterの代わりにShift+Enterを使用してください
