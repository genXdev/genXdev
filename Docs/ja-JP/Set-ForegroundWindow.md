...
* 最大の信頼性を得るために、複数のWin32 APIメソッドを使用してウィンドウを最前面ウィンドウにします。
* 最初にSwitchToThisWindow APIを使用して試行し、必要に応じてSetForegroundWindowにフォールバックします。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-WindowHandle` | IntPtr | ✅ | — | 0 | `0` | 前面ウィンドウとして設定するウィンドウハンドル |
...
ウィンドウハンドルを使用してメモ帳のウィンドウを前面に持ってきます。
...
簡潔な構文のために位置指定パラメーターを使用します。

## Parameter Details

### `-WindowHandle <IntPtr>`

> 前面ウィンドウとして設定するウィンドウハンドル
