...
> Windows 11+ のタスクバーの配置を中央と左の位置の間で構成します。

## Description

Windows 11以降のバージョンでタスクバーの配置を設定します。レジストリキー 'TaskbarAl' を HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced の下で変更します。配置は中央（値 1）または左（値 0）に設定できます。
