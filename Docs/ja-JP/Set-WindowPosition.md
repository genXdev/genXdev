...
> 明示的な配置パラメーターが指定された場合に、ウィンドウの位置とサイズを変更します。

## Description

位置指定パラメーターが指定された場合、ウィンドウの位置とサイズを正確に制御します。複数のモニター、境界線の削除、左右分割、上下分割、中央配置などの各種プリセット位置をサポートします。ウィンドウは座標または定義済みレイアウトで配置できます。位置指定パラメーターがない場合、関数はウィンドウに対して何も実行しません。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ProcessName` | String | — | ✅ (ByValue, ByPropertyName) | 0 | — | 配置するウィンドウのプロセス名 *(Parameter set: )* 🌐 *Supports wildcards* |
...
| `-Monitor` | Int32 | — | — | Named | `-1` | モニター選択: 0=プライマリ、1以上=特定のモニター、-1=現在のモニター、-2=セカンダリ |
| `-NoBorders` | SwitchParameter | — | — | Named | — | ウィンドウの境界線を削除します。 |
| `-Width` | Int32 | — | — | Named | `-1` | ウィンドウの幅（ピクセル単位） |
| `-Height` | Int32 | — | — | Named | `-999999` | ウィンドウの高さ（ピクセル単位） |
...
> モニター選択: 0=プライマリ、1以上=特定のモニター、-1=現在のモニター、-2=セカンダリ
