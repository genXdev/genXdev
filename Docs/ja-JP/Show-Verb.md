...
> すべてのPowerShell動詞の短いアルファベット順の一覧を表示します。

## Description

PowerShell の承認済み動詞をカンマ区切りリストで表示します。特定の動詞が入力として指定された場合、一致する動詞のみが表示されます。ワイルドカードをサポートします。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Verb` | String[] | — | ✅ (ByValue, ByPropertyName) | 0 | `@()` | フィルター処理する動詞パターンを1つ以上指定します（ワイルドカード対応） 🌐 *Supports wildcards* |
...
> フィルター処理する動詞パターンを1つ以上指定します（ワイルドカード対応）
