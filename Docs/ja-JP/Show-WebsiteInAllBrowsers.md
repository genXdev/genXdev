...
> 複数のブラウザーで同時にURLを開き、モザイク状に配置します。

## Description

この関数は、指定されたURLをChrome、Edge、Firefox、およびプライベートブラウジングウィンドウで開くことにより、ブラウザウィンドウのモザイクレイアウトを作成します。ブラウザは2x2のグリッドパターンで配置されます。
- Chrome: 左上のクアドラント
- Edge: 左下のクアドラント
- Firefox: 右上のクアドラント
- プライベートウィンドウ: 右下のクアドラント

Open-Webbrowserのすべてのパラメータがサポートされ、ブラウザの位置、動作、外観を制御するために渡されます。この関数は、一貫したクアドラントの位置を適用しながら、ブラウザ起動パラメータの完全なカスタマイズを可能にするラッパーとして機能します。
