...
> DeepStack顔認識システムから登録済みのすべての顔を削除します。

## Description

この関数は、データストアディレクトリからすべての顔ファイルを削除し、空の顔レジストリを再読み込みするためにサービスを再起動することで、DeepStack顔認識データベースに登録されたすべての顔をクリアします。これは元に戻せない破壊的な操作であり、登録されたすべての顔データを永久に削除します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Force` | SwitchParameter | — | — | Named | — | 登録されているすべての顔を削除する際の確認プロンプトをバイパスする |
...
エイリアスを使用して、すべての面を確認なしで削除します。

## Parameter Details

### `-Force`

> 登録されているすべての顔を削除する際の確認プロンプトをバイパスする
