...
> ハイパーリンク付きの表示名を使用してファイル情報を出力し、ユーザーエクスペリエンスを向上させます。

## Description

この関数はファイル入力オブジェクトを処理し、コンソールに表示する際にハイパーリンク付きのファイル名を持つユーザーフレンドリーな形式で出力します。文字列、FileInfoオブジェクト、その他のオブジェクトタイプなど、さまざまな入力タイプを処理します。この関数は、出力がリダイレクトされているかパイプラインでキャプチャされているかを自動的に検出し、それに応じて動作を調整します。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-CallerInvocation` | Object | ✅ | ✅ (ByValue) | 0 | — | 呼び出し元の関数からの呼び出し情報 |
| `-Input` | Object | ✅ | ✅ (ByValue) | 1 | — | 処理する入力オブジェクト。ファイルパスまたはオブジェクトを指定できます。 |
| `-Prefix` | String | — | — | Named | — | 出力表示に追加のコンテキストとして先頭に付けるオプションの文字列プレフィックス |
| `-RelativeBasePath` | String | — | — | Named | — | 出力内の相対ファイルパスを生成するためのベースパス |
| `-FullPaths` | SwitchParameter | — | — | Named | — | 出力に相対パスではなく完全な絶対パスを使用するように強制します。 |
...
> 処理する入力オブジェクト。ファイルパスまたはオブジェクトを指定できます。
...
> 出力表示に追加のコンテキストとして先頭に付けるオプションの文字列プレフィックス
...
> 出力に相対パスではなく完全な絶対パスを使用するように強制します。
