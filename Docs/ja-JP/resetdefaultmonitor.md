...
> デフォルトのセカンダリモニター構成を復元します。

## Description

このスクリプトは、システムのデフォルトのセカンダリモニター設定を復元し、セカンダリモニターを元のデフォルト値に設定します。これは、サイドバイサイド構成を使用した後、以前のマルチモニター設定に戻したいユーザーに便利です。関連項目: 新しいウィンドウをサイドバイサイドモードに切り替えるための 'sidebyside' 関数。
