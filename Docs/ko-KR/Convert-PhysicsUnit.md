...
> 동일한 범주 내에서 한 물리 단위의 값을 다른 단위로 변환합니다.

## Description

이 함수는 길이, 시간, 속도 등과 같은 범주에서 호환 가능한 단위 간의 물리량을 변환합니다. 단위로부터 범주를 추론하며, 호환되지 않는 변환의 경우 오류를 발생시킵니다.
