...
> 구성 가능한 기준에 따라 여러 디렉터리에서 중복 파일을 찾습니다.

## Description

지정된 디렉터리에서 중복 파일을 재귀적으로 검색합니다. 파일 내용이 정확히 동일하거나 (부분 비교를 위해 -ComparePartial 스위치를 사용하는 경우) 동일한 내용으로 시작하는 경우 중복으로 간주됩니다. 이 cmdlet은 전체 내용 비교를 수행하기 전에 효율적인 그룹화를 위해 파일의 초기 바이트에 대한 빠른 해시를 사용합니다. 이 빠른 해시의 크기는 -CompareByteLength 매개변수로 구성할 수 있어 중복 검색의 성능과 정확성 사이의 균형을 조정할 수 있습니다. CompareByteLength 값의 최소 길이를 가진 파일만 중복 검색 대상이 되며, 더 작은 파일은 파일 이름만 기준으로 그룹화됩니다.
...
| `-ComparePartial` | SwitchParameter | — | — | Named | — | 파일 이름과 내용이 동일한 한 크기가 다른 파일을 반환하여 다운로드/복사 작업의 손상을 감지합니다. |
| `-CompareByteLength` | Int32 | — | — | Named | `65536` | 파일의 바이트 길이를 빠르게 비교하여 중복 파일의 빠른 인덱싱 및 검색에 사용됩니다. 파일 내용은 비교하지 않습니다. |
...
> 파일 이름과 내용이 동일한 한 크기가 다른 파일을 반환하여 다운로드/복사 작업의 손상을 감지합니다.
...
> 파일의 바이트 길이를 빠르게 비교하여 중복 파일의 빠른 인덱싱 및 검색에 사용됩니다. 파일 내용은 비교하지 않습니다.
