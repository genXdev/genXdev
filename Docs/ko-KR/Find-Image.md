...
| `-OverallMood` | String[] | — | — | Named | `@()` | 전반적인 분위기 필터(예: '평온함', '기쁨', '슬픔' 등). 와일드카드 지원. |
...
| `-ShowInBrowser` | SwitchParameter | — | — | Named | — | {
  "type": "json_schema",
  "json_schema": {
...
| `-Interactive` | SwitchParameter | — | — | Named | — | 브라우저에 연결하여 편집 및 삭제와 같은 추가 버튼을 추가합니다. -ShowInBrowser와 함께 사용할 때만 효과적입니다. |
...
| `-MinConfidenceRatio` | Double | — | — | Named | — | 사람, 장면, 객체를 신뢰도로 필터링하기 위한 최소 신뢰도 비율(0.0-1.0)입니다. 이 값보다 크거나 같은 신뢰도를 가진 사람, 장면, 객체에 대한 데이터만 반환합니다. |
