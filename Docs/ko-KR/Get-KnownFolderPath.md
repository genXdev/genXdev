...
Shell32 API의 SHGetKnownFolderPath 함수를 사용하여 지정된 Windows 알려진 폴더의 경로를 검색합니다. 이 메서드는 환경 변수를 사용하는 것보다 다양한 Windows 버전 및 언어 설정에서 일관되게 작동하므로 더 안정적입니다. 문서, 다운로드, AppData 등 모든 표준 Windows 알려진 폴더를 지원합니다.
