...
> SQLite 데이터베이스 테이블의 특정 열에서 데이터를 검색합니다.

## Description

이 함수는 SQLite 데이터베이스 테이블의 단일 열에서 데이터를 추출하는 편리한 방법을 제공합니다. 직접 데이터베이스 파일 경로 또는 연결 문자열의 두 가지 연결 방법을 지원합니다. 반환되는 레코드 수를 제한하는 옵션이 포함되어 있으며, 최적의 성능을 위해 적절한 SQLite 쿼리 구성을 사용합니다.
