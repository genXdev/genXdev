...
> 지정된 디렉토리에 있는 이미지 파일의 얼굴 인식 메타데이터를 업데이트합니다.

## Description

이 함수는 지정된 디렉토리에서 이미지를 처리하여 AI 인식 기술을 사용해 얼굴을 식별하고 분석합니다. 각 이미지에 대한 얼굴 정보가 포함된 메타데이터 파일을 생성하거나 업데이트합니다. 메타데이터는 별도의 파일에 저장됩니다.
