...
> 지정된 디렉토리에 있는 이미지 파일에 대한 개체 검출 메타데이터를 업데이트합니다.

## Description

이 함수는 지정된 디렉터리의 이미지를 처리하여 인공 지능을 사용해 객체를 감지합니다. 감지된 객체, 해당 위치, 신뢰도 점수 및 레이블이 포함된 JSON 메타데이터 파일을 생성합니다. 이 함수는 구성 가능한 신뢰도 임계값으로 일괄 처리를 지원하며, 기존 메타데이터 파일을 건너뛰거나 이전에 실패한 감지를 다시 시도할 수 있습니다.
...
이 예제는 C:\Photos와 D:\Pictures의 모든 이미지와 모든 하위 디렉터리를 0.5 신뢰 임계값의 기본 설정을 사용하여 처리합니다.
...
이 예제는 위치 매개변수 구문을 사용하여 여러 디렉터리에서 새 이미지만 처리하고 이전에 실패한 이미지는 다시 시도합니다.
...
이 예제는 더 정확하지만 더 적은 개체 감지를 위해 0.7의 더 높은 신뢰도 임계값으로 GPU 가속을 사용합니다.
