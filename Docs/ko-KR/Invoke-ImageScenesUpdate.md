...
> 지정된 디렉터리에 있는 이미지 파일의 장면 분류 메타데이터를 업데이트합니다.

## Description

이 함수는 지정된 디렉토리의 이미지를 처리하여 인공지능을 사용해 장면을 분류합니다. 장면 분류, 신뢰도 점수 및 레이블이 포함된 JSON 메타데이터 파일을 생성합니다. 이 함수는 구성 가능한 신뢰도 임계값으로 일괄 처리를 지원하며, 기존 메타데이터 파일을 선택적으로 건너뛰거나 이전에 실패한 분류를 재시도할 수 있습니다.
...
| `-TimeoutSecond` | Int32 | — | — | Named | — | 요청의 시간 제한(초)이며, 기본값은 24시간입니다. |
...
| `-ShowWindow` | SwitchParameter | — | — | Named | — | 초기화 중 Docker 창 표시 |
...
C:\Photos 및 D:\Pictures와 하위 디렉터리의 모든 이미지를 장면 분류를 위해 처리합니다.
...
별칭을 사용하여 실패한 분류를 다시 시도하고 여러 디렉터리에서 새 이미지만 처리합니다.
...
컨테이너 재빌드를 강제하고 GPU 가속을 사용하여 처리 속도를 높입니다.
