...
> 작업 영역에 있는 각 GenXdev 모듈에서 스크립트 블록을 실행합니다.

## Description

이 함수는 작업 영역에서 GenXdev 모듈을 반복하면서 제공된 스크립트 블록을 각 모듈에 대해 실행합니다. 이름 패턴으로 모듈을 필터링하거나, 로컬 모듈을 제외하거나, 게시된 모듈만 포함하거나, 모듈 대신 스크립트를 처리할 수 있습니다. 함수는 스크립트 블록을 실행하기 전에 자동으로 올바른 모듈 디렉터리로 이동합니다.
...
| `-NoLocal` | SwitchParameter | — | — | Named | — | 로컬 개발 모듈을 처리에서 제외합니다 |
| `-OnlyPublished` | SwitchParameter | — | — | Named | — | LICENSE 파일과 README.md 파일이 있는 게시된 모듈만 포함합니다. |
| `-FromScripts` | SwitchParameter | — | — | Named | — | 모듈 디렉터리 대신 스크립트 디렉터리 처리 |
| `-IncludeScripts` | SwitchParameter | — | — | Named | — | 일반 모듈 외에도 스크립트 디렉터리를 포함합니다. |
...
별칭을 사용하여 GenXdev.AI 모듈 디렉터리의 내용을 나열합니다.

## Parameter Details

### `-Script <ScriptBlock>`

> 각 GenXdev 모듈에 대해 실행할 스크립트 블록
...
> LICENSE 파일과 README.md 파일이 있는 게시된 모듈만 포함합니다.
