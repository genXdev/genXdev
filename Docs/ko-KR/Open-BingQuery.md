...
하나 이상의 Bing 검색 쿼리를 웹 브라우저에서 열며, 구성 가능한 브라우저 설정 및 명령줄 스위치를 사용합니다. 이 함수는 창 위치 지정, 모니터 선택, 개인 브라우징, 키 입력 자동화 등 다양한 브라우저 사용자 지정 옵션을 포함한 포괄적인 인터페이스를 제공하여 Bing 검색을 수행합니다.

이 함수는 쿼리를 자동으로 URL 인코딩하고 Bing의 검색 API에 맞게 형식을 지정합니다. Edge, Chrome, Firefox 등 주요 브라우저를 모두 지원하며, 개인 브라우징, 애플리케이션 모드, 정확한 창 관리 및 키 입력 자동화 기능을 제공합니다.
...
별칭 및 짧은 매개 변수 이름을 사용하여 Bing 검색을 엽니다.
...
개인 브라우징 세션을 열어 영어 결과로 "머신 러닝"을 검색합니다.
...
파이프라인을 통해 여러 검색어를 검색하고 설치된 모든 브라우저에서 결과를 열며, 창을 화면 왼쪽에 배치합니다.
