...
> 웹 브라우저에서 GitHub Copilot 쿼리를 엽니다.

## Description

웹 브라우저에서 Github CoPilot 쿼리를 열고 지정된 쿼리를 자동으로 입력합니다. 함수는 페이지가 로드될 때까지 기다린 후 쿼리를 입력합니다. 이 함수는 기본 Open-WebsiteAndPerformQuery 함수를 사용하여 브라우저 자동화를 처리하며, PowerShell에서 GitHub Copilot과 상호 작용할 수 있는 편리한 방법을 제공하며 광범위한 브라우저 구성 옵션을 지원합니다.
