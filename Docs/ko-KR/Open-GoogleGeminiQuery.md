...
> 웹 브라우저에서 Google Gemini 쿼리를 엽니다.

## Description

웹 브라우저에서 Google Gemini 쿼리를 열고, 쿼리를 자동으로 입력한 후 제출합니다. 이는 Google의 Gemini AI와 빠르게 상호작용할 수 있는 방법을 제공합니다. 이 함수는 모니터 선택, 창 위치 지정, 키보드 자동화 기능을 포함한 광범위한 브라우저 제어를 지원합니다.
...
Google Gemini를 열고 "PowerShell이란?"라는 질문을 제출합니다.
...
파이프라인 입력을 통해 배열에 대한 쿼리를 제출하는 데 별칭을 사용합니다.
