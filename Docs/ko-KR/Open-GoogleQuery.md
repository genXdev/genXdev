...
> 구성 가능한 설정과 광범위한 사용자 지정 옵션을 통해 Google 쿼리를 웹 브라우저에서 엽니다.

## Description

웹 브라우저에서 구글 쿼리 하나 이상을 열며, 다양한 언어, 모니터 선택, 창 위치 지정, 브라우저 선택, 키보드 자동화를 포괄적으로 지원합니다. 이 함수는 브라우저 동작과 모양을 정밀하게 제어하여 구글 검색을 수행할 수 있는 풍부한 기능의 인터페이스를 제공합니다.

주요 기능은 다음과 같습니다:
- 구글의 언어 필터링을 통한 100개 이상의 언어 지원
- 자동 또는 수동 모니터 선택을 통한 다중 모니터 지원
- 창 위치 지정(왼쪽, 오른쪽, 위쪽, 아래쪽, 가운데, 전체 화면)
- 개인/시크릿 브라우징 모드 지원
- 방해 요소 없는 브라우징을 위한 응용 프로그램 모드
- 확장 프로그램 및 팝업 차단 옵션
- 포커스 관리 및 창 조작
- 파이프라인 지원을 통한 일괄 쿼리 처리
- 브라우저 창에 대한 키 입력 자동화
- 기본 브라우저 함수에 대한 스마트 매개변수 전달

이 함수는 검색 쿼리를 구글 검색 엔진용으로 자동 포맷하고, 지정된 경우 언어 필터링을 적용합니다. 모든 브라우저 위치 지정 및 관리 기능은 기본 Open-Webbrowser 함수에서 상속됩니다.
...
기본 모니터에서 영어로 "PowerShell 스크립팅"에 대한 Google 검색을 엽니다.
...
별칭을 사용하여 모니터 2에서 전체 화면 모드로 '머신 러닝 알고리즘'에 대한 Google 검색을 엽니다.
...
파이프라인 입력을 통해 Chrome에서 여러 개의 Google 검색을 영어로 엽니다.
...
새 창에서 '인공지능'에 대한 개인 정보 보호 검색을 엽니다.
...
애플리케이션 모드(브라우저 컨트롤 없음)로 검색을 열고 화면 중앙에 배치합니다.
