...
> 웹 브라우저에서 Grokipedia 쿼리를 엽니다.

## Description

확장된 구성 옵션을 사용하여 웹 브라우저에서 하나 이상의 그로키피디아 검색 쿼리를 엽니다. 구성 가능한 브라우저 동작, 모니터 선택 및 창 위치 지정을 지원합니다. 쿼리는 URL 인코딩되며 전체 지역화 지원과 함께 지정된 언어의 그로키피디아 도메인을 사용하여 열립니다.

이 함수는 프라이빗 브라우징 모드, 창 위치 지정, 키보드 자동화 및 다중 모니터 지원을 포함한 포괄적인 브라우저 제어를 제공합니다. 국제 그로키피디아 도메인에 대한 URL 인코딩 및 언어 코드 매핑을 자동으로 처리합니다.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 실행할 Grokipedia 검색 쿼리 |
...
기본 모니터에서 "PowerShell"에 대한 영어 Grokipedia 검색을 엽니다.
...
위치 매개변수와 함께 별칭을 사용하여 Grokipedia 검색을 엽니다.
...
개인 브라우징 모드를 사용하여 Grokipedia에서 여러 용어를 검색합니다.

## Parameter Details

### `-Queries <String[]>`

> 실행할 Grokipedia 검색 쿼리
