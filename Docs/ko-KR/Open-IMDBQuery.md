...
> 웹 브라우저에서 IMDB 검색 쿼리를 엽니다.

## Description

풍부한 구성 옵션과 함께 "인터넷 영화 데이터베이스" 쿼리를 웹 브라우저에서 엽니다. 이 함수는 IMDB 검색 기능에 대한 편리한 래퍼를 제공하여 사용자가 PowerShell에서 직접 영화, TV 프로그램, 배우 및 기타 엔터테인먼트 콘텐츠를 검색할 수 있도록 합니다.

이 함수는 검색어를 받아 적절한 IMDB 검색 URL을 구성한 후, 지정된 웹 브라우저에서 창 위치, 브라우저 선택 및 표시 옵션을 사용자 지정하여 엽니다. Edge, Chrome, Firefox를 포함한 모든 주요 브라우저를 지원하며, 개인 브라우징, 전체 화면 모드 및 창 관리 옵션을 제공합니다.

주요 기능으로는 다중 모니터 지원, 언어 지역화, 키보드 자동화 및 포괄적인 브라우저 구성 옵션이 포함됩니다. 이 함수는 여러 검색 쿼리를 동시에 처리할 수 있으며 대화형 사용과 자동화된 워크플로 모두에 대한 옵션을 제공합니다.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | IMDB에서 쿼리할 검색어 |
| `-Language` | String | — | — | 1 | — | 반환된 검색 결과의 언어 |
| `-Monitor` | Int32 | — | — | Named | `-1` | 사용할 모니터, 0 = 기본값, -1 = 폐기, -2 = 구성된 보조 모니터, 기본값은 -1 |
...
| `-AcceptLang` | String | — | — | Named | — | 브라우저 accept-lang HTTP 헤더 설정 |
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | 브라우저에 키 입력을 보낼 때 제어 문자를 이스케이프합니다. |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | 키 입력 전송 후 PowerShell로 키보드 포커스가 반환되지 않도록 방지합니다. |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | 키를 보낼 때 줄 바꿈에 일반 Enter 대신 Shift+Enter 사용 |
...
별칭 및 약어 매개변수를 사용하여 "The Matrix"에 대한 IMDB 검색을 엽니다.
...
IMDB에서 프랑스어 결과로 여러 영화를 검색하여 Chrome에 표시합니다.
...
전체 화면 모드로 IMDB에서 톰 행크스를 검색한 후 PowerShell로 포커스를 되돌립니다.
...
> 사용할 모니터, 0 = 기본값, -1 = 폐기, -2 = 구성된 보조 모니터, 기본값은 -1
...
> 브라우저에 키 입력을 보낼 때 제어 문자를 이스케이프합니다.
...
> 키 입력 전송 후 PowerShell로 키보드 포커스가 반환되지 않도록 방지합니다.
...
> 키를 보낼 때 줄 바꿈에 일반 Enter 대신 Shift+Enter 사용
