...
> 웹 브라우저에서 InstantStreetView 쿼리를 엽니다.

## Description

웹 브라우저 명령줄 스위치를 사용하여 구성 가능한 방식으로 InstantStreetView 쿼리를 엽니다. 여러 쿼리와 모니터 선택을 지원합니다. 창 위치, 언어 설정 및 브라우저별 기능을 포함한 포괄적인 브라우저 제어를 제공합니다.

이 함수는 위치 쿼리를 URL 인코딩하여 InstantStreetView URL을 자동으로 구성하고 지정된 브라우저에서 엽니다. Edge, Chrome, Firefox 등 모든 주요 브라우저를 광범위한 사용자 정의 옵션과 함께 지원합니다.
...
주 모니터의 기본 브라우저에서 타임스퀘어의 InstantStreetView를 엽니다.
...
별칭 'isv'를 사용하여 에펠탑에 대한 InstantStreetView를 열고 두 번째 모니터에 표시합니다.
...
비공개 브라우징 모드를 사용하여 영어로 빅 벤에 대한 InstantStreetView를 엽니다.
...
파이프라인을 통해 Chrome 브라우저에서 여러 InstantStreetView 쿼리를 열며, 화면 왼쪽에 배치됩니다.

## Parameter Details

### `-Queries <String[]>`

> InstantStreetView에서 볼 위치 쿼리
