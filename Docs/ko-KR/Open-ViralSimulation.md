...
> 웹 브라우저에서 광범위한 구성 옵션을 제공하는 바이러스 시뮬레이션 게임을 엽니다.

## Description

https://viral.genxdev.net/에서 바이럴 시뮬레이션 게임을 열고, 구성 가능한 창 설정, 위치 지정 및 동작 옵션을 통해 지정된 웹 브라우저에서 실행합니다. 여러 브라우저, 개인 브라우징, 애플리케이션 모드 및 포괄적인 창 관리 기능을 지원합니다.

이 함수는 다음과 같은 기능을 갖춘 대화형 바이럴 시뮬레이션 경험을 제공합니다:
...
비공개 브라우징 모드로 전체 화면 없이 Google Chrome에서 바이럴 시뮬레이션을 엽니다.
...
별칭을 사용하여 Chrome, 시크릿 모드 및 전체 화면 없이 바이럴 시뮬레이션을 엽니다.

## Parameter Details

### `-Language <String>`

> 웹 콘텐츠 표시 및 브라우저 언어 설정에 사용되는 언어
