...
> 하나 이상의 브라우저 창에서 선택적 위치 지정 및 스타일링과 함께 URL을 엽니다.

## Description

이 함수는 브라우저 실행에 대한 고급 래퍼를 제공하며 창 위치 지정, 브라우저 선택 및 동작 사용자 정의를 위한 광범위한 옵션을 포함합니다. Edge, Chrome, Firefox 등 여러 브라우저를 지원하며 비공개 브라우징, 응용 프로그램 모드 및 정밀한 창 관리 기능을 제공합니다.

주요 기능:
- 스마트 브라우저 감지 및 선택
- 창 위치 지정 (왼쪽, 오른쪽, 위쪽, 아래쪽, 가운데, 전체 화면)
...
이 함수는 시스템 기능을 자동으로 감지하여 그에 따라 동작을 조정할 수 있습니다. 시스템에 설치되지 않은 브라우저의 경우 오류 없이 자동으로 건너뜁니다.
...
| `-Force` | SwitchParameter | — | — | Named | — | 새 브라우저 인스턴스를 열기 전에 기존 브라우저 인스턴스를 강제로 닫습니다. |
...
| `-PreferPlaywrightBrowser` | SwitchParameter | — | — | Named | — | 채널을 통한 OS 설치 브라우저 대신 번들된 Playwright 브라우저 바이너리를 사용합니다. Chrome, Edge 및 Chromium 모드에만 영향을 미칩니다. -PlayWright를 암시합니다. |
...
Playwright로 관리되는 Chromium 브라우저에서 GitHub를 엽니다.
...
모니터 1의 왼쪽 절반에 Stack Overflow를 엽니다.
...
별칭을 사용하여 주 모니터에서 전체 화면 모드로 Google을 엽니다.
...
이미 열려 있는 브라우저 창을 주 모니터의 오른쪽으로 다시 배치합니다.
...
브라우저 컨트롤 없이 웹 애플리케이션을 앱 모드로 엽니다.
...
> 새 브라우저 인스턴스를 열기 전에 기존 브라우저 인스턴스를 강제로 닫습니다.
...
> 채널을 통한 OS 설치 브라우저 대신 번들된 Playwright 브라우저 바이너리를 사용합니다. Chrome, Edge 및 Chromium 모드에만 영향을 미칩니다. -PlayWright를 암시합니다.
