...
> 웹 브라우저에서 위키피디아 쿼리를 엽니다.

## Description

웹 브라우저에서 하나 이상의 Wikipedia 검색 쿼리를 열며 다양한 구성 옵션을 제공합니다. 구성 가능한 브라우저 동작, 모니터 선택 및 창 위치 지정을 지원합니다. 쿼리는 URL 인코딩되어 지정된 언어의 Wikipedia 도메인을 사용하여 열리며 완전한 지역화 지원이 포함됩니다.

이 함수는 개인 정보 보호 모드, 창 위치 지정, 키보드 자동화 및 다중 모니터 지원을 포함한 포괄적인 브라우저 제어 기능을 제공합니다. 국제 Wikipedia 도메인에 대한 URL 인코딩 및 언어 코드 매핑을 자동으로 처리합니다.
...
| `-Language` | String | — | — | 1 | — | 위키백과 검색 결과의 언어 |
...
별명을 사용하여 위치 매개변수로 위키백과 검색을 엽니다.
