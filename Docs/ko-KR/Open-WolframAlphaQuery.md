...
웹 브라우저에서 하나 이상의 Wolfram Alpha 쿼리를 엽니다. 동적 매개변수를 통해 구성 가능한 모니터 선택 및 브라우저 옵션을 지원합니다. 위치 지정, 크기 조정 및 자동화 기능을 포함한 포괄적인 브라우저 제어 기능을 제공하여 향상된 과학적 계산 워크플로를 지원합니다.
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | 키를 보낼 때 제어 문자 이스케이프 처리. |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | 키를 보낸 후 PowerShell로 키보드 포커스가 반환되지 않도록 방지합니다. |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | 키를 보낼 때 줄 바꿈을 위해 일반 Enter 대신 Shift+Enter를 보냅니다. |
...
| `-PassThru` | SwitchParameter | — | — | Named | — | 브라우저 프로세스의 [System.Diagnostics.Process] 개체를 반환합니다. |
...
파이프라인 입력을 통해 영어 언어 기본 설정으로 계산 쿼리를 전체 화면 모드로 엽니다.
...
> 키를 보낼 때 줄 바꿈을 위해 일반 Enter 대신 Shift+Enter를 보냅니다.
...
> 브라우저 프로세스의 [System.Diagnostics.Process] 개체를 반환합니다.
