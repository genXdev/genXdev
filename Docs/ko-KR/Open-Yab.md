...
> YAB 게임을 구성 가능한 설정으로 웹 브라우저에서 엽니다.

## Description

지정된 브라우저에서 YAB 게임 웹사이트를 다양한 표시 및 동작 옵션으로 엽니다. 관전자 모드를 포함한 여러 게임 모드를 지원합니다. YAB 게임은 온라인 퍼즐 게임으로, 대화형으로 플레이하거나 AI가 자동으로 플레이하는 관전자 모드에서 시청할 수 있습니다.
...
Chrome 브라우저를 사용하여 YAB 게임을 관전자 모드로 열고, 전체 화면 없이 영어 인터페이스로 실행합니다.
...
바로 가기를 사용하여 빠르게 액세스할 수 있도록 모니터 2의 비공개 브라우징에서 Edge 브라우저로 YAB 게임을 엽니다.
...
> 새 게임을 시작하지 말고, AI가 플레이하는 것만 지켜보세요
