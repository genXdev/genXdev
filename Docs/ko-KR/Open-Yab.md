...
> 웹 브라우저에서 설정 가능한 옵션으로 YAB 게임을 엽니다.

## Description

지정된 브라우저에서 다양한 표시 및 동작 옵션으로 YAB 게임 웹사이트를 엽니다. 관전자 모드를 포함한 여러 게임 모드를 지원합니다. YAB 게임은 대화형으로 플레이하거나 AI가 자동으로 플레이하는 관전자 모드로 시청할 수 있는 온라인 퍼즐 게임입니다.
...
Chrome 브라우저를 사용하여 전체 화면 없이 관전자 모드로 YAB 게임을 열며, 인터페이스 언어는 영어로 설정합니다.
...
Edge 브라우저에서 비공개 탐색 모드로 모니터 2에 YAB 게임을 열고, 빠른 접근을 위해 별칭을 사용합니다.
