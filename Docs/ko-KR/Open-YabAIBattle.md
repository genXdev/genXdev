...
> 웹 브라우저에서 YabAI 배틀 게임을 엽니다.

## Description

이 함수는 YabAI Battle 게임을 웹 브라우저에서 시작하며, 창 배치, 브라우저 선택 및 디스플레이 모드에 대한 광범위한 사용자 지정 옵션을 제공합니다. GenXdev\Open-Webbrowser 함수를 래핑하여 게임별 URL 구성 및 매개변수 처리를 수행합니다.

함수는 두 가지 게임 모드를 지원합니다:
- 배틀 모드: 새로운 AI 대 AI 배틀 시작
- 관전 모드: 기존 AI 배틀 관전

브라우저 위치 지정 옵션에는 왼쪽, 오른쪽, 위쪽, 아래쪽, 중앙 및 전체 화면 모드가 있으며 다중 모니터를 지원합니다. 이 함수는 최적의 게임 환경을 위해 애플리케이션 모드 설정 및 브라우저 확장 관리도 자동으로 처리합니다.
...
| `-Monitor` | Int32 | — | — | Named | `-2` | 사용할 모니터, 0 = 기본값, -1 = 폐기, -2 = 구성된 보조 모니터, 기본값은 $Global:DefaultSecondaryMonitor 이며 없으면 2 |
...
| `-SendKeyDelayMilliSeconds` | Int32 | — | — | Named | — | 각 키를 보내는 사이의 지연 시간(밀리초)입니다. |
...
| `-RestoreFocus` | SwitchParameter | — | — | Named | — | 브라우저를 연 후 이전 창으로 포커스를 되돌립니다. |
...
YabAI Battle 게임을 Microsoft Edge의 개인 정보 보호 브라우징 모드에서 전체 화면 없이 엽니다.
...
Edge의 별칭을 사용하여 게임을 열며, 시크릿 모드와 전체 화면 없이 실행합니다.
...
Google Chrome을 사용하여 관전 모드로 게임을 열고 AI 대결을 시청합니다.
...
스페인어 인터페이스로 모니터 1의 왼쪽에 위치한 게임을 엽니다.
