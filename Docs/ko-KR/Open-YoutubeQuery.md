...
> 웹 브라우저에서 YouTube 검색어를 엽니다.

## Description

웹 브라우저에서 하나 이상의 YouTube 검색 쿼리를 열며, 광범위한 위치 지정 및 구성 옵션을 제공합니다. 구성 가능한 모니터 선택, 브라우저 설정, Open-Webbrowser cmdlet에서 상속된 동적 매개변수를 통한 키 입력 자동화를 지원합니다. 각 쿼리는 별도의 브라우저 인스턴스에서 열리며, 창 위치, 브라우저 유형 및 검색 언어를 완전히 제어할 수 있습니다.
...
기본 모니터에서 'PowerShell 자습서'에 대한 YouTube 검색을 엽니다.
...
함수 별칭을 사용하여 구성된 보조 모니터에서 '코딩 방법'에 대한 YouTube 검색을 엽니다.
