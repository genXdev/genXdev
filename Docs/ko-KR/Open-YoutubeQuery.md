...
> 웹 브라우저에서 YouTube 검색 쿼리를 엽니다.

## Description

웹 브라우저에서 하나 이상의 YouTube 검색어를 열며, 다양한 위치 지정 및 구성 옵션을 제공합니다. 모니터 선택, 브라우저 설정, 키 입력 자동화를 지원하며, 이는 Open-Webbrowser cmdlet에서 상속된 동적 매개변수를 통해 이루어집니다. 각 검색어는 별도의 브라우저 인스턴스에서 열리며, 창 위치, 브라우저 종류, 검색 언어를 완전히 제어할 수 있습니다.
...
구성된 보조 모니터에서 함수 별칭을 사용하여 "how to code"에 대한 YouTube 검색을 엽니다.
