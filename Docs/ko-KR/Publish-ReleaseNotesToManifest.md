...
README.md 파일의 "## Release notes" 섹션을 읽고, 완료되지 않은 릴리스 노트(- ☐로 시작하는 줄)를 모두 수집하여 GenXdev.psd1의 ReleaseNotes 필드에 쓴 다음, README에서 완료됨으로 표시합니다(☐ → ☒). 두 파일이 한 번의 작업으로 원자적으로 업데이트됩니다.
