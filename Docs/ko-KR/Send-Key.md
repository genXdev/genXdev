...
> 창이나 프로세스에 시뮬레이션된 키 입력을 보냅니다.

## Description

이 함수는 Windows Script Host Shell 개체를 사용하여 키보드 입력을 대상 창 또는 프로세스로 전송합니다. 프로세스 이름, 프로세스 ID 또는 창 핸들로 창을 대상으로 지정할 수 있습니다. 이 함수는 안정적인 키 전달을 위해 특수 키 시퀀스, 이스케이프 문자 및 다양한 타이밍 옵션을 지원합니다.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-KeysToSend` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 보낼 키보드 입력(문자열 배열) |
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | 입력 텍스트에서 제어 문자와 수정자 이스케이프 처리 |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | 키를 전송한 후 대상 창에 키보드 포커스 유지 |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | 새 줄을 위해 Enter 대신 Shift+Enter를 사용하세요 |
...
> 새 줄을 위해 Enter 대신 Shift+Enter를 사용하세요
