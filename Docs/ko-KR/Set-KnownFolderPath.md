...
Windows Shell32 API를 사용하여 문서, 다운로드, 데스크톱 또는 기타 알려진 Windows 폴더와 같은 시스템 폴더를 새 위치로 이동합니다. 이 함수는 대상 경로가 존재하는지 확인하고, 포괄적인 알려진 폴더 레지스트리에서 폴더의 고유 GUID를 조회한 후 SHSetKnownFolderPath API를 사용하여 이동을 수행합니다. 일반적인 사용 사례로는 공간 관리나 구성을 위해 사용자 폴더를 다른 드라이브로 이동하는 경우가 있습니다. 시스템 안정성에 영향을 줄 수 있으므로 시스템에 중요한 폴더를 이동할 때는 주의하십시오.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-KnownFolder` | String | ✅ | — | 0 | — | 수정할 알려진 폴더 |
