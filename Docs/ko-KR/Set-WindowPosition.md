...
> 명시적 위치 매개 변수가 제공될 때 창을 배치하고 크기를 조정합니다.

## Description

위치 지정 매개 변수가 설정된 경우 창 위치 및 크기를 정밀하게 제어할 수 있습니다. 여러 모니터 지원, 테두리 제거, 왼쪽/오른쪽 분할, 위쪽/아래쪽 분할, 중앙 배치 등 다양한 사전 설정 위치를 지원합니다. 창은 좌표나 미리 정의된 레이아웃을 사용하여 배치할 수 있습니다. 위치 지정 매개 변수가 없으면 함수는 창에 대해 아무 작업도 수행하지 않습니다.
...
> 모니터 선택: 0=기본, 1+=특정 모니터, -1=현재, -2=보조
