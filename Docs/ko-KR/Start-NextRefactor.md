...
> 코드 리팩터링 세션을 계속하거나 다시 시작합니다.

## Description

우선 순위에 따라 리팩터 정의를 처리하여 코드 리팩터링 작업을 관리합니다. 파일 선택, 진행 상황 추적, 오류 처리 및 리팩터링 프로세스에 대한 대화형 사용자 제어를 제공합니다.
...
| `-FilesToAdd` | IO.FileInfo[] | — | — | 1 | `@()` | 추가할 파일 이름 |
| `-FilesToRemove` | IO.FileInfo[] | — | — | 2 | `@()` | 제거할 파일 이름 |
| `-CleanUpDeletedFiles` | SwitchParameter | — | — | Named | — | 삭제된 파일 정리 |
| `-Reset` | SwitchParameter | — | — | Named | — | 리팩터 세트의 처음부터 시작하세요 |
| `-ResetLMSelections` | SwitchParameter | — | — | Named | — | 모든 LLM 선택 항목을 다시 시작합니다 |
