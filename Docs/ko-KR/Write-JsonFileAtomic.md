...
* 재시도 로직: 최대 MaxRetries회까지 쓰기를 재시도하며, 각 시도 사이에 RetryDelayMs 밀리초의 지연이 있습니다.
* 객체 지원: Hashtable뿐만 아니라 모든 객체를 허용합니다. System.Text.Json을 통해 직렬화하며, 기본적으로 직렬화할 수 없는 복잡한 .NET 유형의 경우 ConvertTo-Json으로 폴백합니다.
* 디바운스 지원: DebounceMs > 0일 때, 동일한 파일에 대한 빠른 연속 쓰기가 병합됩니다—파일이 DebounceMs 밀리초 동안 건드려지지 않으면 마지막 페이로드만 기록됩니다.
* 디렉터리 생성: 상위 디렉터리가 존재하지 않는 경우 자동으로 생성합니다.
