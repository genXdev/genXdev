...
> 향상된 사용자 경험을 위해 하이퍼링크된 표시 이름으로 파일 정보를 출력합니다.

## Description

이 함수는 파일 입력 개체를 처리하여 콘솔에 표시할 때 하이퍼링크된 파일 이름과 함께 사용자 친화적인 형식으로 출력합니다. 문자열, FileInfo 개체 및 기타 개체 유형을 포함한 다양한 입력 유형을 처리합니다. 함수는 출력이 리디렉션되거나 파이프라인에서 캡처되는지 자동으로 감지하고 그에 따라 동작을 조정합니다.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-CallerInvocation` | Object | ✅ | ✅ (ByValue) | 0 | — | 호출 함수의 호출 정보 |
| `-Input` | Object | ✅ | ✅ (ByValue) | 1 | — | 처리할 입력 개체(파일 경로 또는 개체일 수 있음) |
| `-Prefix` | String | — | — | Named | — | 출력 표시에 추가 컨텍스트를 위해 앞에 붙일 선택적 문자열 접두사 |
| `-RelativeBasePath` | String | — | — | Named | — | 출력에서 상대 파일 경로를 생성하기 위한 기본 경로 |
| `-FullPaths` | SwitchParameter | — | — | Named | — | 출력에 상대 경로 대신 전체 절대 경로를 사용하도록 강제합니다. |
...
> 출력 표시에 추가 컨텍스트를 위해 앞에 붙일 선택적 문자열 접두사
...
> 출력에 상대 경로 대신 전체 절대 경로를 사용하도록 강제합니다.
