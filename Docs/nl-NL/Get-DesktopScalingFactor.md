...
> Haalt de Windows-display-schaalfactor (DPI-instelling) op voor een opgegeven monitor.

## Description

* Haalt de huidige Windows-weergaveschaalfactor op die is geconfigureerd voor een monitor
  in het systeem.
* De schaalfactor wordt uitgedrukt als een percentage waarbij 100 staat voor
  standaardschaling (96 DPI). Veelvoorkomende waarden zijn 100, 125, 150 en 200.
* Als er geen monitor is opgegeven, wordt de schaalfactor voor het primaire
  beeldscherm geretourneerd.
...
Retourneert het schaalpercentage voor de primaire monitor.
...
Geeft het schalingsfactorpercentage voor de tweede monitor terug met behulp van een positionele parameter.

## Parameter Details

### `-Monitor <Int32>`

> De monitorindex om te controleren (0 = primaire monitor)
