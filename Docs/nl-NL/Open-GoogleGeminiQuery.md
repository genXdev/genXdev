...
Opent een Google Gemini-query in de webbrowser, voert de query automatisch in en verzendt deze. Dit biedt een snelle manier om te communiceren met Google's Gemini AI.
De functie ondersteunt uitgebreide browserbesturing, inclusief monitorkeuze, vensterpositionering en toetsenbordautomatiseringsfuncties.
...
Opent Google Gemini en stuurt de zoekopdracht "Wat is PowerShell?" in.
...
Gebruikt het alias om via pijplijninvoer een query over matrices in te dienen.
