...
> Kopiuje pliki pasujące do kryteriów wyszukiwania do podfolderów opartych na dacie.

## Description

Wyszukuje pliki przy użyciu tego samego zestawu parametrów co `Find-Item` i kopiuje każdy dopasowany plik do podfolderu `TargetFolder` na podstawie daty utworzenia lub daty medialnej pliku. Polecenie cmdlet obsługuje dopasowywanie zawartości, wyszukiwanie w całym dysku i wiele filtrów.

Próbuje kilku strategii w celu ustalenia dokładnej daty utworzenia lub przechwycenia dla określonego pliku. Strategie obejmują odczytywanie metadanych EXIF obrazu, analizowanie informacji o dacie/godzinie z nazw plików oraz powrót do czasu ostatniego zapisu pliku, gdy nie są dostępne żadne inne wiarygodne informacje.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Główny folder docelowy, do którego dopasowane pliki zostaną skopiowane do podfolderów opartych na dacie |
...
> Główny folder docelowy, do którego dopasowane pliki zostaną skopiowane do podfolderów opartych na dacie
