...
> Przenosi pliki pasujące do kryteriów wyszukiwania do podfolderów opartych na dacie.

## Description

Wyszukuje pliki przy użyciu tego samego zestawu parametrów co `Find-Item` i przenosi każdy dopasowany plik do podfolderu `TargetFolder` na podstawie daty utworzenia lub daty medialnej pliku. Polecenie cmdlet obsługuje dopasowywanie zawartości, wyszukiwanie na całym dysku oraz wiele filtrów. Opcjonalnie może usuwać puste foldery źródłowe po przeniesieniu.

Próbuje kilku strategii w celu określenia dokładnej daty utworzenia lub przechwycenia dla określonego pliku. Strategie obejmują odczyt metadanych EXIF obrazu, analizowanie informacji o dacie/godzinie z nazw plików oraz powrót do czasu ostatniego zapisu pliku, gdy nie są dostępne żadne inne wiarygodne informacje.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Folder główny docelowy, do którego dopasowane pliki zostaną przeniesione do podfolderów na podstawie daty |
...
| `-DeleteEmptyDirs` | SwitchParameter | — | — | Named | — | Usuń puste katalogi źródłowe po przeniesieniu plików |
| `-TargetFolderNameDateSyntax` | String | — | — | Named | `'Year + Month'` | Składnia nazwy docelowego folderu daty |
...
Przenieś-PlikiDoFolderuDatowanego -FolderDocelowy 'E:\Media\Videos' -Nazwa '.\*.mp4' -UsuńPusteKatalogi

## Parameter Details

### `-TargetFolder <String>`

> Folder główny docelowy, do którego dopasowane pliki zostaną przeniesione do podfolderów na podstawie daty
...
> Usuń puste katalogi źródłowe po przeniesieniu plików
...
> Składnia nazwy docelowego folderu daty
