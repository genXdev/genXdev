...
> Zapisuje przycięte obrazy twarzy z wyników indeksowanego wyszukiwania obrazów.

## Description

Ta funkcja przyjmuje wyniki wyszukiwania obrazów i wyodrębnia/zapisuje poszczególne obszary twarzy jako osobne pliki obrazów. Może wyszukiwać twarze według różnych kryteriów i zapisywać je do określonego katalogu wyjściowego. Funkcja obsługuje wyszukiwanie według opisu, słów kluczowych, osób, obiektów, scen, typu obrazu, typu stylu oraz ogólnego nastroju. Może również filtrować według nagości i treści jawnie seksualnych.
...
| `-DescriptionSearch` | String[] | — | — | Named | `@()` | Tekst opisu do wyszukania, dopuszczalne symbole wieloznaczne. |
...
| `-Scenes` | String[] | — | — | Named | `@()` | Sceny do szukania, dozwolone symbole wieloznaczne. |
| `-PictureType` | String[] | — | — | Named | `@()` | Typy obrazów do filtrowania, dopuszczalne symbole wieloznaczne. |
| `-StyleType` | String[] | — | — | Named | `@()` | Typy stylów do filtrowania, symbole wieloznaczne dozwolone. |
| `-OverallMood` | String[] | — | — | Named | `@()` | Ogólne nastroje do filtrowania, dozwolone symbole wieloznaczne. |
| `-DatabaseFilePath` | String | — | — | Named | — | Ścieżka do pliku bazy danych SQLite. |
| `-Language` | String | — | — | Named | — | Język dla opisów i słów kluczowych. |
| `-PathLike` | String[] | — | — | Named | `@()` | Tablica ciągów wyszukiwania przypominających ścieżki katalogów do filtrowania obrazów według ścieżki (wzorce SQL LIKE, np. '%\\2024\\%') |
| `-InputObject` | Object[] | — | ✅ (ByValue) | Named | — | Akceptuje wyniki wyszukiwania z poprzedniego wywołania -PassThru, aby odtworzyć widok. |
| `-OutputDirectory` | String | — | — | Named | `'.\'` | Katalog do zapisywania przyciętych obrazów twarzy. |
...
| `-ForceIndexRebuild` | SwitchParameter | — | — | Named | — | Wymuś przebudowę bazy danych indeksu obrazów. |
...
| `-SaveUnknownPersons` | SwitchParameter | — | — | Named | — | Zapisuj również nieznane osoby wykryte jako obiekty. |
...
> Tekst opisu do wyszukania, dopuszczalne symbole wieloznaczne.
...
> Sceny do szukania, dozwolone symbole wieloznaczne.
...
> Typy obrazów do filtrowania, dopuszczalne symbole wieloznaczne.
...
> Typy stylów do filtrowania, symbole wieloznaczne dozwolone.
...
> Ogólne nastroje do filtrowania, dozwolone symbole wieloznaczne.
...
> Tablica ciągów wyszukiwania przypominających ścieżki katalogów do filtrowania obrazów według ścieżki (wzorce SQL LIKE, np. '%\\2024\\%')
...
> Akceptuje wyniki wyszukiwania z poprzedniego wywołania -PassThru, aby odtworzyć widok.
...
> Katalog do zapisywania przyciętych obrazów twarzy.
...
> Wymuś przebudowę bazy danych indeksu obrazów.
...
> Zapisuj również nieznane osoby wykryte jako obiekty.
