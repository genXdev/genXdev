...
Esta função atua como um mecanismo personalizado de ShouldProcess especificamente para instalações de software. Ela verifica uma preferência do usuário para determinar se a instalação automática é permitida para o aplicativo especificado. Se nenhuma preferência estiver definida, ela solicita ao usuário uma explicação clara do que será instalado, a origem, os riscos potenciais e opções para permitir ou negar a instalação (com opções para escolhas únicas ou persistentes).

Isso garante o consentimento explícito do usuário antes de prosseguir com qualquer instalação, ajudando a mitigar possíveis riscos legais ao exigir uma ação afirmativa do usuário. O prompt declara claramente que o autor do módulo (GenXdev) não é responsável por softwares de terceiros e que o usuário está consentindo com a instalação por sua própria conta e risco.

As preferências são armazenadas em um arquivo JSON, permitindo que os usuário definam "sempre permitir" para aplicativos específicos ou ativem o consentimento global para todas as instalações de terceiros da GenXdev, tornando-o conveniente enquanto permanece juridicamente sólido.

Se o consentimento for negado (ou a preferência estiver definida para negar), a função retorna $false e não prossegue com a instalação. Se permitido, retorna $true.
...
| `-Source` | String | ✅ | — | 1 | — | A fonte da instalação (por exemplo, Winget, PowerShell Gallery). |
| `-Description` | String | — | — | Named | `"This software is required for certain features in the GenXdev modules."` | Descrição opcional do software e sua finalidade. |
| `-Publisher` | String | — | — | Named | `"Third-party vendor"` | Editor ou fornecedor opcional do software. |
| `-ForceConsent` | SwitchParameter | — | — | Named | — | Force um prompt mesmo que a preferência esteja definida. |
...
Avisos com informações detalhadas antes de instalar o módulo Pester.

## Parameter Details

### `-ApplicationName <String>`

> O nome do aplicativo ou software que está sendo instalado.
...
> A fonte da instalação (por exemplo, Winget, PowerShell Gallery).
...
> Descrição opcional do software e sua finalidade.
...
> Editor ou fornecedor opcional do software.
...
> Force um prompt mesmo que a preferência esteja definida.
