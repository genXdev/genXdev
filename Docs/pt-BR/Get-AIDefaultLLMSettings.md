...
> Obtém todas as configurações padrão de LLM disponíveis para operações de IA no GenXdev.AI.

## Description

Recupera o conjunto completo de configurações padrão do Modelo de Linguagem Grande (LLM) configuradas para operações de IA. Os resultados podem ser filtrados por tipo de consulta, identificador do modelo, endpoint da API ou chave da API. Suporta recuperação baseada em sessão e preferências persistentes, com opções para limpar o estado da sessão ou ignorá-lo completamente para ler diretamente das preferências armazenadas.
...
| `-ApiEndpoint` | String | — | — | Named | — | Filtrar configurações por URL de endpoint da API |
...
Obtém todas as configurações padrão disponíveis para o tipo de consulta de Codificação.
...
> Filtrar configurações por identificador ou padrão de modelo
...
> Filtrar configurações por URL de endpoint da API
...
> Filtrar configurações por chave de API
