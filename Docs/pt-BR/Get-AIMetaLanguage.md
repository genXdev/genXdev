...
* Este cmdlet recupera o idioma padrão usado pelo módulo GenXdev.AI
  para operações de metadados de imagem.
* Ele verifica primeiro as variáveis Globais (a menos que SkipSession seja especificado) e, em seguida,
  recorre às preferências persistentes e, finalmente, usa os padrões do sistema.
...
Limpar a configuração da sessão e obter o idioma das preferências persistentes.

## Parameter Details

### `-Language <String>`

> O idioma padrão para operações de metadados de imagem
