...
> Recupera uma lista de visualizações de um banco de dados SQL Server.

## Description

Obtém todos os nomes de exibições do banco de dados SQL Server especificado. Retorna um array de nomes de exibições que podem ser usados para operações adicionais no banco de dados. A função suporta dois conjuntos de parâmetros para flexibilidade: fornecendo uma string de conexão ou um nome de banco de dados com servidor.
