...
> Cria e retorna um objeto de transação SQLite para operações em lote.

## Description

Cria um objeto de conexão e transação de banco de dados SQLite que pode ser usado para operações em lote. O chamador é responsável por confirmar ou reverter a transação. A conexão será criada automaticamente se o arquivo de banco de dados não existir.
...
| `-CreateDatabaseIfNotExists` | Boolean | — | — | Named | `$true` | Se deve criar o arquivo de banco de dados caso ele não exista. |
...
> Se deve criar o arquivo de banco de dados caso ele não exista.
