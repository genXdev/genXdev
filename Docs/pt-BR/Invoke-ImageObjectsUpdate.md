...
Esta função processa imagens em um diretório especificado para detectar objetos usando inteligência artificial. Ela cria arquivos de metadados JSON contendo objetos detectados, suas posições, pontuações de confiança e rótulos. A função suporta processamento em lote com limites de confiança configuráveis e pode opcionalmente ignorar arquivos de metadados existentes ou repetir detecções que falharam anteriormente.
...
Este exemplo processa todas as imagens em C:\Photos e D:\Pictures e todos os subdiretórios usando as configurações padrão com limite de confiança de 0,5.
...
Este exemplo processa apenas novas imagens e tenta novamente as que falharam anteriormente em múltiplos diretórios usando sintaxe de parâmetro posicional.
...
Este exemplo usa aceleração de GPU com um limite de confiança mais alto de 0,7 para detecções de objetos mais precisas, porém em menor quantidade.
