...
Cria e configura uma nova definição de refatoração com configurações especificadas para transformações de código baseadas em LLM. A função lida com:
- Configurar a definição de refatoração
- Configurar critérios de seleção e prompts
- Gerenciar configurações do modelo LLM
- Integrar com ambientes de desenvolvimento
- Persistir definições de refatoração
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Name` | String | ✅ | — | 0 | — | O nome desse novo conjunto de refatoração |
| `-PromptKey` | String | ✅ | — | 1 | — | A chave do prompt indica qual script de prompt usar |
| `-Prompt` | String | — | — | 2 | `''` | Texto de prompt personalizado para substituir o modelo |
| `-SelectionScript` | String | — | — | 3 | — | Script PowerShell para selecionar itens para refatoração |
| `-SelectionPrompt` | String | — | — | 4 | — | Instrução de seleção de LLM |
| `-LLMQueryType` | String | — | — | 5 | `'Coding'` | O tipo da consulta LLM |
...
| `-Priority` | Int32 | — | — | Named | `0` | Prioridade para esta refatoração |
| `-ExposedCmdLets` | GenXdev.Helpers.ExposedCmdletDefinition[] | — | — | Named | `@()` | Array de definições de comandos PowerShell para ferramentas LLM |
| `-FilesToAdd` | IO.FileInfo[] | — | — | Named | `@()` | Matriz de arquivos para processar |
...
| `-KeysToSend` | String[] | — | — | Named | `@()` | Teclas a serem enviadas após abrir arquivos |
...
> O nome desse novo conjunto de refatoração
...
> Texto de prompt personalizado para substituir o modelo
...
> Script PowerShell para selecionar itens para refatoração
...
> Array de definições de comandos PowerShell para ferramentas LLM
...
> Matriz de arquivos para processar
...
> Adicionará automaticamente arquivos modificados à fila
...
> Abrir arquivos no Visual Studio Code
...
> Teclas a serem enviadas após abrir arquivos
