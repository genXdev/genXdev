...
> Abre uma interface de chat LLM na nuvem para consultas de IA.

## Description

Esta função fornece uma interface unificada para abrir vários serviços de chat baseados em IA na nuvem em um navegador web. Ela suporta múltiplas plataformas de IA, incluindo ChatGPT, BingCopilot, Google Gemini, X Grok, DeepSearch e GitHub Copilot. A função seleciona automaticamente a função específica do endpoint apropriado e passa todos os parâmetros relevantes para configuração do navegador e posicionamento da janela.
...
> Minimizar a janela após o posicionamento
