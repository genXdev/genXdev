...
> Abre consultas do InstantStreetView em um navegador da web.

## Description

Abre consultas do InstantStreetView de forma configurável usando opções de linha de comando do navegador web. Suporta múltiplas consultas e seleção de monitor. Fornece controle abrangente do navegador, incluindo posicionamento de janela, configurações de idioma e recursos específicos do navegador.

A função constrói automaticamente URLs do InstantStreetView codificando as consultas de localização e abrindo-as no navegador especificado. Suporta todos os principais navegadores, incluindo Edge, Chrome e Firefox, com opções extensivas de personalização.
...
Abre o InstantStreetView para a Times Square no navegador padrão no monitor principal.
...
Abre várias consultas do InstantStreetView via pipeline no navegador Chrome posicionadas no lado esquerdo da tela.

## Parameter Details

### `-Queries <String[]>`

> As consultas de localização para visualizar no InstantStreetView
