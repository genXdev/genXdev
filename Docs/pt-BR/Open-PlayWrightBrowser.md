...
Inicia uma instância de navegador utilizando o Playwright. O parâmetro BrowserType determina qual mecanismo de navegador e modo de inicialização usar:

- ChromeNormal / EdgeNormal: Inicia o Chrome ou Edge instalado no seu sistema operacional através da API Channel do Playwright, evitando a barra de informações "controlado por automação" e problemas de detecção anti-bot.
- ChromiumNormal: Detecta automaticamente o Chrome ou Edge (o que estiver instalado e definido como padrão do sistema) e usa a API Channel. Volta para o Chromium empacotado do Playwright se nenhum estiver instalado.
- ChromiumPlaywright: Sempre usa o binário Chromium empacotado do Playwright (nunca o navegador instalado no sistema operacional).
- FirefoxPlaywright: Usa o binário Firefox empacotado do Playwright. A API Channel não está disponível para Firefox nas associações .NET do Playwright.
- WebKitPlaywright: Usa o binário WebKit empacotado do Playwright.

Medidas anti-detecção aplicadas para navegadores baseados em Chromium:
- Desabilita flags de automação do blink (remove a barra de informações)
- Define viewport como nulo e inicia maximizado para dimensionamento nativo da janela
- Usa esquema de cores 'no-preference' para evitar fundo branco
- Define argumentos padrão do navegador (no-first-run, no-default-browser-check)

Perfis persistentes por BrowserType são armazenados em GenXdev AppData. O navegador iniciado e seu contexto/página são armazenados em $Global:GenXdevPlaywright para uso por outros cmdlets.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-BrowserType` | String | — | — | 0 | `'ChromiumNormal'` | O navegador a ser iniciado: Chrome, Edge, Chromium (detecta automaticamente Chrome ou Edge), Firefox ou WebKit |
| `-Headless` | SwitchParameter | — | — | Named | — | Execute o navegador sem uma janela visível |
| `-Proxy` | String | — | — | Named | — | URL do servidor proxy (por exemplo, http://proxy:8080) |
...
| `-ForceConsent` | SwitchParameter | — | — | Named | — | Forçar a reexibição da caixa de diálogo de consentimento de instalação |
...
| `-Fullscreen` | SwitchParameter | — | — | Named | — | Maximiza a janela para preencher toda a tela |
...
| `-Minimize` | SwitchParameter | — | — | Named | — | Minimiza a janela após posicionar |
...
| `-PassThru` | SwitchParameter | — | — | Named | — | Retorna o objeto auxiliar da janela para manipulação adicional |
...
| `-OnlyOutputCoords` | SwitchParameter | — | — | Named | — | Apenas saída das coordenadas e tamanho calculados sem realmente posicionar a janela |
...
> O navegador a ser iniciado: Chrome, Edge, Chromium (detecta automaticamente Chrome ou Edge), Firefox ou WebKit
...
> URL do servidor proxy (por exemplo, http://proxy:8080)
...
> Definir o cabeçalho HTTP Accept-Language do navegador
...
> Largura inicial da janela de visualização em pixels
...
> Altura inicial da janela de visualização em pixels
...
> Feche o navegador existente e inicie um novo
...
> Forçar a reexibição da caixa de diálogo de consentimento de instalação
...
> Consentir automaticamente com a instalação de software de terceiros (navegadores Playwright)
...
> Maximiza a janela para preencher toda a tela
...
> Minimiza a janela após posicionar
...
> Retorna o objeto auxiliar da janela para manipulação adicional
...
> Apenas saída das coordenadas e tamanho calculados sem realmente posicionar a janela
