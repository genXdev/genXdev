...
> Abre consultas de pesquisa do Stack Overflow em um navegador da web.

## Description

Abre uma ou mais consultas de pesquisa do Stack Overflow em um navegador da web com opções abrangentes de exibição e configuração do navegador. Esta função fornece um wrapper avançado em torno da funcionalidade de pesquisa do Stack Overflow com amplas opções de posicionamento de janela, seleção de navegador e personalização de comportamento.

Principais recursos:
- Suporte a múltiplas consultas de pesquisa com entrada por pipeline
- Detecção e seleção inteligente de navegador (Edge, Chrome, Firefox, todos os navegadores)
- Posicionamento avançado de janela (esquerda, direita, superior, inferior, centralizado, tela cheia)
- Suporte a vários monitores com seleção automática ou manual
- Suporte a modo de navegação privada/anonimato
- Modo aplicativo para navegação sem distrações
- Suporte a localização de idioma para resultados de pesquisa internacionais
- Opções de bloqueio de extensões e pop-ups
- Gerenciamento de foco e manipulação de janela
- Automação de teclas para janelas do navegador
- Opções de retorno de URL para fluxos de trabalho de automação

A função detecta automaticamente as capacidades do sistema e ajusta o comportamento de acordo. Para navegadores não instalados no sistema, as operações são ignoradas silenciosamente sem erros.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | As consultas de pesquisa a serem realizadas no Stack Overflow |
| `-Language` | String | — | — | 1 | — | O idioma dos resultados de pesquisa retornados para conteúdo do Stack Overflow localizado |
| `-Monitor` | Int32 | — | — | Named | `-1` | O monitor a ser usado, 0 = padrão, -1 é ignorar, -2 = Monitor secundário configurado, o padrão é -1, sem posicionamento |
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | Use configurações alternativas armazenadas na sessão para preferências de pesquisa do Stack Overflow |
| `-ClearSession` | SwitchParameter | — | — | Named | — | Limpar configurações alternativas armazenadas na sessão para preferências de pesquisa do Stack Overflow |
| `-SkipSession` | SwitchParameter | — | — | Named | — | Armazenar configurações apenas em preferências persistentes sem afetar a sessão para pesquisa no Stack Overflow |
...
Abre uma pesquisa no Stack Overflow por "powershell array" no monitor principal.
...
Abre uma pesquisa do Stack Overflow usando o alias com posicionamento do monitor.
...
Abre várias pesquisas do Stack Overflow no Chrome com preferência de idioma inglês.
...
Retorna a URL de pesquisa do Stack Overflow sem abrir um navegador.

## Parameter Details

### `-Queries <String[]>`

> As consultas de pesquisa a serem realizadas no Stack Overflow
...
> O idioma dos resultados de pesquisa retornados para conteúdo do Stack Overflow localizado
...
> Use configurações alternativas armazenadas na sessão para preferências de pesquisa do Stack Overflow
...
> Limpar configurações alternativas armazenadas na sessão para preferências de pesquisa do Stack Overflow
...
> Armazenar configurações apenas em preferências persistentes sem afetar a sessão para pesquisa no Stack Overflow
