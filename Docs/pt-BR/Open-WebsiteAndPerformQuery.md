...
> Abre uma página da web em um navegador e realiza uma ou mais consultas.

## Description

Esta função abre um site por URL no navegador padrão, insere uma ou mais consultas e as envia. Oferece suporte a extensas opções de seleção de monitor e posicionamento de janela através da função Open-Webbrowser subjacente.

A função copia automaticamente cada consulta para a área de transferência e a envia para a janela do navegador usando automação de teclado. Preserva o conteúdo original da área de transferência e o restaura após todas as consultas serem processadas.
