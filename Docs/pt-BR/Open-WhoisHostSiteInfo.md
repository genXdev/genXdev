...
> Abre uma consulta de informações de host Whois em um navegador da web.

## Description

Abre consultas de informações de host Whois em um navegador da web para nomes de domínio ou endereços IP. Suporta seleção de monitor configurável e múltiplas consultas com opções abrangentes de configuração do navegador, incluindo posicionamento de janela, seleção de navegador e recursos de automação.
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | Utilize configurações alternativas armazenadas na sessão para preferências de IA, como Idioma, coleções de Imagens, etc. |
...
| `-SkipSession` | SwitchParameter | — | — | Named | — | Armazenar configurações apenas em preferências persistentes sem afetar a sessão para preferências de IA, como idioma, coleções de imagens, etc. |
...
Abre informações Whois para múltiplos domínios no monitor padrão.
...
Abre informações Whois para domínios do pipeline no modo privado do Chrome.
