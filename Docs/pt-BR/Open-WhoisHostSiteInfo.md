...
> Abre uma consulta de informações do host Whois em um navegador web.

## Description

Abre consultas de informações de host Whois em um navegador da web para nomes de domínio ou endereços IP. Suporta seleção de monitor configurável e várias consultas com opções extensas de configuração do navegador, incluindo posicionamento da janela, seleção do navegador e recursos de automação.
...
| `-SkipSession` | SwitchParameter | — | — | Named | — | Armazenar configurações apenas nas preferências persistentes sem afetar a sessão para preferências de IA, como Idioma, coleções de imagens, etc. |
...
Abre informações Whois para vários domínios no monitor padrão.
...
Abre as informações Whois para example.com usando aliases e parâmetros posicionais.
...
Abre informações de Whois para domínios do pipeline no modo privado do Chrome.
...
> Armazenar configurações apenas nas preferências persistentes sem afetar a sessão para preferências de IA, como Idioma, coleções de imagens, etc.
