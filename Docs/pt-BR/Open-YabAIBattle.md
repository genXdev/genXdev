...
> Abre o jogo YabAI Battle em um navegador da web.

## Description

Esta função inicia o jogo YabAI Battle em um navegador web com amplas opções de personalização para posicionamento da janela, seleção do navegador e modo de exibição. Ela fornece um invólucro em torno da função GenXdev\Open-Webbrowser com construção de URL específica do jogo e manipulação de parâmetros.

A função suporta dois modos de jogo:
- Modo Batalha: Iniciar uma nova batalha AI vs AI
- Modo Espectador: Assistir batalhas AI existentes

As opções de posicionamento do navegador incluem modos esquerdo, direito, superior, inferior, centralizado e tela cheia com suporte a múltiplos monitores. A função lida automaticamente com configurações de modo de aplicação e gerenciamento de extensões do navegador para uma experiência de jogo ideal.
...
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | Evitar que o foco do teclado retorne ao PowerShell após o envio das teclas. |
...
Abre o jogo YabAI Battle no Microsoft Edge usando o modo de navegação privada sem tela cheia.
...
Abre o jogo com interface em espanhol posicionada no lado esquerdo do monitor 1.
...
> Envie Shift+Enter em vez de Enter normal para quebras de linha ao enviar teclas.
...
> Atraso em milissegundos entre o envio de cada tecla.
...
> Restaurar o foco para a janela anterior após abrir o navegador.
