...
* Se a renomeação falhar, o parâmetro -MarkInPlace pode ser usado para marcar os arquivos em seu local original.
...
Marca um arquivo bloqueado para exclusão durante a próxima reinicialização do sistema.
...
Marca vários arquivos para exclusão, usando MarkInPlace para qualquer um que não possa ser renomeado.

## Parameter Details

### `-Path <String[]>`

> Caminho(s) para arquivos/diretórios a marcar para exclusão
...
> Marca arquivos para exclusão sem renomear
