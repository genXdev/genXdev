...
> Define as configurações de LLM para operações de IA no GenXdev.AI.

## Description

Este parâmetro [int] $TimeoutSeconds,
        ###############################################################################
        [Parameter(
            Mandatory = $false,
            HelpMessage = 'Indica se o endpoint não suporta o formato de resposta json_schema'
        )]
        [switch] $NoSupportForJsonSchema,
        ###############################################################################
        [Parameter(
            Mandatory = $false,
            HelpMessage = 'Indica se o endpoint não suporta a funcionalidade de upload de imagem'
        )]
        [switch] $NoSupportForImageUpload,
        ###############################################################################
        [Parameter(
            Mandatory = $false,
            HelpMessage = 'Indica se o endpoint não suporta a funcionalidade de chamada de ferramentas'
        )]
        [switch] $NoSupportForToolCalls,
        ###############################################################################
        [Parameter(
            Mandatory = $false,
            HelpMessage = ('Armazena as configurações apenas na sessão atual, sem ' +
                'persistência')
        )]
        [switch] $SessionOnly,
        ###############################################################################
        [Parameter(
            Mandatory = $false,
            HelpMessage = ('Limpa configurações alternativas armazenadas na sessão para ' +
                'preferências de IA')
        )]
        [switch] $ClearSession, ajusta as configurações do LLM (Large Language Model) usadas pelo
módulo GenXdev.AI para várias operações de IA. As configurações podem ser armazenadas persistentemente
em preferências (padrão), apenas na sessão atual (usando -SessionOnly), ou
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-LLMQueryType` | String | ✅ | — | 0 | — | O tipo da consulta LLM |
...
Define as configurações do LLM para o tipo de consulta de Codificação persistentemente nas preferências.
...
Define as configurações de LLM para SimpleIntelligence apenas para a sessão atual.
...
Limpa as configurações de LLM da sessão para o tipo de consulta Pictures sem afetar as preferências persistentes.
