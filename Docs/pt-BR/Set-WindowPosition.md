...
> Posiciona e redimensiona janelas quando parâmetros de posicionamento explícitos são fornecidos.

## Description

Fornece controle preciso sobre o posicionamento e dimensionamento da janela quando os parâmetros de posicionamento são especificados. Suporta múltiplos monitores, remoção de bordas e várias posições predefinidas, como divisão esquerda/direita, divisão superior/inferior e posicionamento central. As janelas podem ser posicionadas por coordenadas ou usando layouts predefinidos. Sem parâmetros de posicionamento, a função não executa nenhuma ação na janela.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ProcessName` | String | — | ✅ (ByValue, ByPropertyName) | 0 | — | O nome do processo da janela a ser posicionada *(Parameter set: )* 🌐 *Supports wildcards* |
| `-Process` | Diagnostics.Process | — | ✅ (ByValue, ByPropertyName) | Named | — | O processo da janela para posicionar *(Parameter set: )* |
...
> O nome do processo da janela a ser posicionada
...
> Objeto auxiliar Get-Window para manipulação direta de janelas
...
> Seleção de monitor: 0=primário, 1+=monitor específico, -1=atual, -2=secundário
...
> Posição horizontal da janela em pixels
...
> Posição vertical da janela em pixels
