...
> Exibe resultados de pesquisa de imagens em uma galeria da web com layout de alvenaria.

## Description

Pega os resultados da pesquisa de imagens e os exibe em um layout de alvenaria baseado em navegador. Pode operar em modo interativo com recursos de edição e exclusão, ou em modo de exibição simples. Aceita objetos de dados de imagem normalmente do Find-Image e os renderiza com dicas de ferramentas ao passar o mouse mostrando metadados como reconhecimento facial, detecção de objetos e dados de classificação de cena.
...
| `-Interactive` | SwitchParameter | — | — | Named | — | Conecta-se ao navegador e adiciona botões adicionais como Editar e Excluir |
| `-Title` | String | — | — | Named | `'Photo Gallery'` | Título da galeria |
| `-Description` | String | — | — | Named | `('Hover over images to see face recognition ' +
            'and object detection data')` | Passe o mouse sobre as imagens para ver dados de reconhecimento facial e detecção de objetos |
...
| `-Monitor` | Int32 | — | — | Named | `-2` | O monitor a ser usado, 0 = padrão, -1 é descartar, -2 = Monitor secundário configurado, assume como Padrão:MonitorSecundárioGlobal ou 2 se não encontrado |
...
| `-NewWindow` | SwitchParameter | — | — | Named | — | Criar uma nova janela do navegador em vez de reutilizar uma existente |
| `-OnlyReturnHtml` | SwitchParameter | — | — | Named | — | Traduza o seguinte texto para português (Brasil). Você DEVE traduzir TODO o texto legível por humanos. NÃO retorne a entrada inalterada. Retorne APENAS o texto traduzido, sem explicações, sem wrappers JSON e sem prompts de sistema.
...
Não insira imagens ou nada. Apenas traduza o conteúdo da forma mais direta possível para: Português (Brasil). |
| `-EmbedImages` | SwitchParameter | — | — | Named | — | Incorpore imagens como URLs de dados base64 em vez de URLs de arquivo para melhor portabilidade. |
...
| `-PassThru` | SwitchParameter | — | — | Named | — | Retorna o objeto auxiliar da janela do navegador para manipulação posterior. |
...
> Objetos de dados de imagem para exibir na galeria.
...
> Conecta-se ao navegador e adiciona botões adicionais como Editar e Excluir
...
> Passe o mouse sobre as imagens para ver dados de reconhecimento facial e detecção de objetos
...
> O monitor a ser usado, 0 = padrão, -1 é descartar, -2 = Monitor secundário configurado, assume como Padrão:MonitorSecundárioGlobal ou 2 se não encontrado
...
> Criar uma nova janela do navegador em vez de reutilizar uma existente
...
> Incorpore imagens como URLs de dados base64 em vez de URLs de arquivo para melhor portabilidade.
...
> Mostre apenas imagens em um retângulo arredondado, sem texto abaixo.
...
> Retorna o objeto auxiliar da janela do navegador para manipulação posterior.
