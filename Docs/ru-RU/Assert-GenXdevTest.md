...
> Запускает модульные тесты для модулей, подмодулей или командлетов GenXdev
с интеллектуальной отладкой и разрешением ошибок с помощью ИИ.

## Description

Эта функция предоставляет инфраструктуру тестирования для модулей GenXdev, обеспечивая многоуровневое выполнение тестов от целых модулей до отдельных командлетов. Она интегрирует PSScriptAnalyzer для статического анализа кода, Pester для модульного тестирования и PSScriptAnalyzer для статического анализа кода. Функция включает интеллектуальную обработку ошибок с возможностями разрешения на основе ИИ и подробное отслеживание прогресса для рабочих процессов разработки.
...
| `-ModuleName` | String[] | — | ✅ (ByValue, ByPropertyName) | 3 | — | Имена модулей GenXdev для поиска 🌐 *Supports wildcards* |
...
| `-ExactMatch` | SwitchParameter | — | — | Named | — | Требовать точного совпадения имен командлетов |
...
| `-AllowLongRunningTests` | SwitchParameter | — | — | Named | — | Также выбирает модульные тесты, которые имеют длительное время выполнения |
| `-SkipModuleImports` | SwitchParameter | — | — | Named | — | Пропускает импорт модулей GenXdev перед тестированием *(Parameter set: )* |
| `-SkipPSAnalyzerTests` | SwitchParameter | — | — | Named | — | Пропускает выполнение тестов PSAnalyzer |
| `-SkipPesterTests` | SwitchParameter | — | — | Named | — | Пропускает вызов тестов Pester |
...
> Действие при сбое теста. Варианты: Ask, Continue, Stop, SolveWithAI, Write-Error, Throw
...
> Требовать точного совпадения имен командлетов
...
> Уровень детализации трассировки стека
...
> Также выбирает модульные тесты, которые имеют длительное время выполнения
...
> Пропускает импорт модулей GenXdev перед тестированием
...
> Пропускает выполнение тестов PSAnalyzer
...
> Пропускает вызов тестов Pester
