...
Эта функция обеспечивает загрузку указанных сборок пакетов NuGet и их загрузку в текущий сеанс PowerShell. Она поддерживает автоматическое обнаружение и загрузку с запасным вариантом в виде JSON-манифеста для переопределения конфигурации, обрабатывая крайние случаи, такие как фиктивные зависимости (например, SQLite). Установка использует исключительно интерфейс командной строки dotnet для надежного управления пакетами и загружает сборки непосредственно из глобального кэша NuGet без копирования. Ленивая загрузка проверяет, доступен ли тип уже перед продолжением.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-PackageKey` | String | ✅ | — | 0 | — | Ключ пакета из packages.json или прямой NuGet PackageId. |
| `-ManifestPath` | String | — | — | Named | — | Путь к файлу packages.json; по умолчанию — корень модуля, если не указан. |
| `-Version` | String | — | — | Named | — | Конкретная версия; если опущено, использовать самую высокую из JSON или последнюю. |
| `-TypeName` | String | — | — | Named | — | TypeName для проверки загрузки. |
| `-ForceLatest` | SwitchParameter | — | — | Named | — | Использовать последнюю версию, если точная версия недоступна. |
| `-Destination` | String | — | — | Named | — | Пользовательский каталог установки; по умолчанию используется локальное постоянное или глобальное хранилище. |
| `-Description` | String | — | — | Named | — | Необязательное описание программного обеспечения и его назначения для согласия. |
| `-Publisher` | String | — | — | Named | — | Необязательный издатель или поставщик программного обеспечения для согласия. |
| `-ForceConsent` | SwitchParameter | — | — | Named | — | Принудительный запрос, даже если настроено предпочтение на согласие. |
...
> Ключ пакета из packages.json или прямой NuGet PackageId.
...
> Путь к файлу packages.json; по умолчанию — корень модуля, если не указан.
...
> Конкретная версия; если опущено, использовать самую высокую из JSON или последнюю.
...
> TypeName для проверки загрузки.
...
> Использовать последнюю версию, если точная версия недоступна.
...
> Пользовательский каталог установки; по умолчанию используется локальное постоянное или глобальное хранилище.
...
> Необязательное описание программного обеспечения и его назначения для согласия.
...
> Необязательный издатель или поставщик программного обеспечения для согласия.
...
> Принудительный запрос, даже если настроено предпочтение на согласие.
