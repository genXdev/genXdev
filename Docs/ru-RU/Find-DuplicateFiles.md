...
> Поиск дубликатов файлов в нескольких каталогах на основе настраиваемых критериев.

## Description

Рекурсивно ищет дубликаты файлов в указанных каталогах. Файлы считаются дубликатами, если их содержимое полностью совпадает или начинается одинаково (для частичного сравнения с использованием параметра -ComparePartial). Командлет использует быстрый хэш начальных байтов файла для эффективной группировки перед полным сравнением содержимого. Размер этого быстрого хэша можно настроить с помощью параметра -CompareByteLength, что позволяет найти баланс между производительностью и точностью обнаружения дубликатов. Только файлы с минимальной длиной, равной значению CompareByteLength, будут рассматриваться для обнаружения дубликатов; файлы меньшего размера будут группироваться только по имени.
...
| `-ComparePartial` | SwitchParameter | — | — | Named | — | Будет возвращать файлы с разными размерами, если они начинаются с одинакового имени и содержимого; это позволяет обнаруживать поврежденные загрузки/операции копирования. |
| `-CompareByteLength` | Int32 | — | — | Named | `65536` | Быстрое сравнение размера файлов в байтах, используется для быстрой индексации и поиска дубликатов; содержимое файлов не сравнивается |
...
> Будет возвращать файлы с разными размерами, если они начинаются с одинакового имени и содержимого; это позволяет обнаруживать поврежденные загрузки/операции копирования.
...
> Быстрое сравнение размера файлов в байтах, используется для быстрой индексации и поиска дубликатов; содержимое файлов не сравнивается
