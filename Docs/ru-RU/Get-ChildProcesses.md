...
> Извлекает все процессы, которые являются потомками текущего процесса PowerShell.

## Description

* Проверяет все запущенные процессы и определяет те, у которых текущий процесс PowerShell является предком в цепочке родительских процессов.
* Это включает как прямые дочерние процессы, так и их потомков (внучки и т.д.).
...
Получение дочерних процессов с подробным выводом информации о детальном обнаружении процесса.
