...
> Извлекает коэффициент масштабирования отображения Windows (настройка DPI) для указанного монитора.

## Description

* Получает текущий коэффициент масштабирования дисплея Windows, настроенный для монитора в системе.
* Коэффициент масштабирования выражается в процентах, где 100 соответствует стандартному масштабированию (96 DPI). Обычные значения: 100, 125, 150 и 200.
* Если монитор не указан, возвращается коэффициент масштабирования для основного дисплея.
...
Возвращает процент коэффициента масштабирования для основного монитора.
...
Возвращает процентный коэффициент масштабирования для второго монитора с использованием позиционного параметра.

## Parameter Details

### `-Monitor <Int32>`

> Индекс монитора для проверки (0 = основной монитор)
