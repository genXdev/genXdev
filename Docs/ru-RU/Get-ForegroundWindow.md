...
> Получает дескриптор текущего активного окна переднего плана.

## Description

* Этот командлет извлекает дескриптор окна (HWND) окна, которое
  в данный момент находится на переднем плане и имеет фокус ввода.
* Он использует функции Windows API через P/Invoke для взаимодействия с
  библиотекой User32.dll.
...
Сохраняет дескриптор окна переднего плана в переменную и отображает его.
