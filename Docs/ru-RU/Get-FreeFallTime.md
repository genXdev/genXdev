...
Вычисляет продолжительность времени, необходимого для падения объекта с заданной высоты, используя численный метод, учитывающий сопротивление воздуха и предельную скорость. Расчет использует малые временные шаги для точного моделирования физики падающих объектов с реалистичными ограничениями предельной скорости.
...
Вычисляет время, необходимое для падения на 50 метров, используя позиционный параметр и конечную скорость по умолчанию.
...
Вычисляет время, необходимое для падения на 100 метров, и возвращает результат в минутах.

## Parameter Details

### `-HeightInMeters <Double>`

> Высота падения в метрах
...
> Конечная скорость в метрах в секунду (по умолчанию: 53 м/с для человека)
...
> Единица измерения времени вывода
