...
> Извлекает значение предпочтения из хранилища предпочтений GenXdev.

## Description

* Реализует двухуровневую систему извлечения предпочтений.
* Сначала проверяет локальное хранилище на наличие значения предпочтения.
* Если не найдено, обращается к хранилищу по умолчанию.
* Если и там не найдено, возвращает указанное значение по умолчанию.
...
Извлекает предпочтение «Тема» с запасным вариантом «Тёмная» по умолчанию.
