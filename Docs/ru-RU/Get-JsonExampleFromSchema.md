...
> Генерирует удобочитаемую строку JSON из определения схемы JSON.

## Description

Преобразует любую JSON-схему (формат json_schema OpenAI или сырая схема) в компактную примерную JSON-строку, подходящую для вставки в системные инструкции LLM. Обрабатывает вложенные объекты, массивы, скалярные типы, перечисления, объединения, композиции oneOf/anyOf/allOf и подсказки формата, такие как date-time и email.

Функция строит дерево объектов PowerShell путем рекурсивного обхода схемы, затем сериализует его в компактную JSON-строку с помощью ConvertTo-Json.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Schema` | Object | — | — | 0 | — | Определение схемы JSON (строка JSON или PSCustomObject). Поддерживает формат обертки json_schema от OpenAI и сырые объекты схемы JSON. |
...
Генерирует: {"items":[{"id":0,"label":"..."}]}

## Parameter Details

### `-Schema <Object>`

> Определение схемы JSON (строка JSON или PSCustomObject). Поддерживает формат обертки json_schema от OpenAI и сырые объекты схемы JSON.
