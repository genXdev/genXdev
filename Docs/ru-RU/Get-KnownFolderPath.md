...
Извлекает путь указанной известной папки Windows с помощью функции SHGetKnownFolderPath из API Shell32. Этот метод более надежен, чем использование переменных среды, так как он последовательно работает в разных версиях Windows и языковых настройках. Поддерживает все стандартные известные папки Windows, такие как Documents, Downloads, AppData и т. д.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-KnownFolder` | String | ✅ | — | 0 | — | Имя известной папки Windows, для которой нужно получить путь |
...
> Имя известной папки Windows, для которой нужно получить путь
