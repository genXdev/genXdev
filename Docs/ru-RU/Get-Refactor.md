...
> Извлекает определения рефакторинга из предпочтений GenXdev на основе шаблонов имен.

## Description

Ищет и загружает наборы определений рефакторинга, хранящиеся в настройках GenXdev.
Каждый набор рефакторинга хранится в виде строки JSON в настройке, имя которой начинается
...
> Шаблон(ы) для поиска определений рефакторинга
