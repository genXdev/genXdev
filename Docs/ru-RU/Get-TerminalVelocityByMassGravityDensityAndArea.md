...
> Рассчитывает конечную скорость.

## Description

Использует v = sqrt(2 m g / (ρ A C)).
...
| `-GravityInMetersPerSecondSquared` | Double | — | — | 1 | `0` | Ускорение свободного падения в м/с² (по умолчанию: 9,81) |
| `-DensityInKilogramsPerCubicMeter` | Double | ✅ | — | 2 | `0` | Плотность жидкости в кг/м³ |
| `-AreaInSquareMeters` | Double | ✅ | — | 3 | `0` | Площадь поперечного сечения в м² |
| `-DragCoefficient` | Double | — | — | 4 | `0` | Коэффициент сопротивления (по умолчанию: 0,5) |
...
Рассчитывает конечную скорость для массы 80 кг в воздухе с коэффициентом лобового сопротивления 1,0, выводя результат в милях в час.
...
Использует позиционные параметры для расчета терминальной скорости.
...
> Коэффициент сопротивления (по умолчанию: 0,5)
