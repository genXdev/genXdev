...
> Импортирует все модули PowerShell GenXdev в глобальную область.

## Description

Сканирует родительский каталог на наличие модулей GenXdev и импортирует каждый из них в глобальную область. Использует управление стеком расположения для сохранения рабочего каталога и предоставляет визуальную обратную связь для успешного и неудачного импорта. Отслеживает изменения количества функций в процессе импорта.
...
> Включить отладочный вывод для неудачных определений модулей
