...
> Выполняет блок скрипта для каждого модуля GenXdev в рабочей области.

## Description

Эта функция перебирает модули GenXdev в рабочем пространстве и выполняет предоставленный блок скрипта для каждого модуля. Она может фильтровать модули по шаблону имени, исключать локальные модули, включать только опубликованные модули или обрабатывать скрипты вместо модулей. Функция автоматически переходит в правильный каталог модуля перед выполнением блока скрипта.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Script` | ScriptBlock | ✅ | — | 0 | — | Блок сценария для выполнения для каждого модуля GenXdev |
...
| `-FromScripts` | SwitchParameter | — | — | Named | — | Обрабатывать каталог сценариев вместо каталогов модулей |
| `-IncludeScripts` | SwitchParameter | — | — | Named | — | Включает каталог скриптов в дополнение к обычным модулям |
...
Выводит список всех имен модулей GenXdev.
...
> Блок сценария для выполнения для каждого модуля GenXdev
...
> Фильтр для применения к именам модулей
...
> Исключает локальные модули разработки из обработки
...
> Включает только опубликованные модули, имеющие файлы LICENSE и README.md
...
> Обрабатывать каталог сценариев вместо каталогов модулей
