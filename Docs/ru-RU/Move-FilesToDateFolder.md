...
> Перемещает файлы, соответствующие критериям поиска, в подпапки на основе даты.

## Description

Выполняет поиск файлов с использованием того же набора параметров, что и Find-Item, и перемещает каждый найденный файл в подпапку TargetFolder на основе даты создания или медиа-даты файла. Командлет поддерживает поиск по содержимому, поиск по всему диску и множество фильтров. При необходимости он может удалять пустые исходные папки после перемещения.

Пытается использовать несколько стратегий для определения точной даты создания или съемки для указанного файла. Стратегии включают чтение метаданных EXIF изображения, анализ информации о дате/времени из имен файлов и возврат к времени последней записи файла, когда другая надежная информация недоступна.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Целевая корневая папка, в которую перемещаются соответствующие файлы в подпапки на основе дат |
...
> Целевая корневая папка, в которую перемещаются соответствующие файлы в подпапки на основе дат
