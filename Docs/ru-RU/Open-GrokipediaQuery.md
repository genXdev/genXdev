...
> Открывает запрос Grokipedia в веб-браузере.

## Description

Открывает один или несколько поисковых запросов Grokipedia в веб-браузере с расширенными параметрами конфигурации. Поддерживает настраиваемое поведение браузера, выбор монитора и позиционирование окна. Запросы кодируются в URL и открываются с использованием домена Grokipedia на указанном языке с полной поддержкой локализации.

Функция обеспечивает полный контроль над браузером, включая режимы приватного просмотра, позиционирование окна, автоматизацию клавиатуры и поддержку нескольких мониторов. Она автоматически обрабатывает кодировку URL и сопоставление языковых кодов для международных доменов Grokipedia.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Поисковый запрос Grokipedia для выполнения |
...
Открывает поиск в Grokipedia по запросу "PowerShell" на английском языке на мониторе по умолчанию.
...
Открывает поиск Grokipedia с использованием псевдонима с позиционными параметрами.
...
Выполняет поиск нескольких терминов в Grokipedia с использованием режима приватного просмотра.

## Parameter Details

### `-Queries <String[]>`

> Поисковый запрос Grokipedia для выполнения
