...
> Открывает запросы InstantStreetView в веб-браузере.

## Description

Открывает запросы InstantStreetView в настраиваемом режиме с использованием параметров командной строки веб-браузера. Поддерживает несколько запросов и выбор монитора. Обеспечивает всестороннее управление браузером, включая позиционирование окна, языковые настройки и функции, специфичные для браузера.

Функция автоматически создает URL-адреса InstantStreetView, кодируя URL-запросы местоположения и открывая их в указанном браузере. Поддерживает все основные браузеры, включая Edge, Chrome и Firefox, с широкими возможностями настройки.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Запросы местоположения для просмотра в InstantStreetView |
...
Открывает InstantStreetView для Таймс-сквер в браузере по умолчанию на основном мониторе.
...
Открывает InstantStreetView для Эйфелевой башни с помощью псевдонима 'isv' и отображает на дополнительном мониторе.
...
Открывает InstantStreetView для Биг-Бена на английском языке в режиме приватного просмотра.
...
Открывает несколько запросов InstantStreetView через конвейер в браузере Chrome, расположенном в левой части экрана.

## Parameter Details

### `-Queries <String[]>`

> Запросы местоположения для просмотра в InstantStreetView
