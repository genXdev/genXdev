...
> Запускает и управляет VLC Media Player с обширными возможностями настройки.

## Description

Эта функция предоставляет комплексный интерфейс для запуска и управления VLC Media Player с поддержкой позиционирования окна, выбора монитора, настроек воспроизведения, аудио и видео фильтров, управления субтитрами, поддержки нескольких языков, настроек сетевого прокси и расширенных параметров производительности. Функция может автоматически установить VLC, если он отсутствует, и предоставляет широкие возможности настройки для сценариев медиа-плейбуков. Она бесшовно интегрируется с системой управления окнами GenXdev и поддерживает автоматизацию ввода с клавиатуры через функциональность Send-Key.
...
| `-SendKeyDelayMilliSeconds` | Int32 | — | — | Named | `50` | Задержка между различными входными строками в миллисекундах при отправке клавиш в VLC |
...
> Медиафайл(ы) или URL-адрес(а) для открытия в VLC
...
> Восстановление фокуса окна PowerShell после открытия VLC
