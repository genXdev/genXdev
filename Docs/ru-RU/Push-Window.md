...
> Помещает текущее окно в стек окон с возможными изменениями.

## Description

Помещает вспомогательное окно для текущего активного окна в стек.
Позволяет применять различные преобразования окна до или после помещения.
Эта функция захватывает текущее окно и сохраняет его для последующего извлечения
с помощью Pop-Window, при этом опционально применяя позиционирование, изменение размера и визуальные
эффекты.
