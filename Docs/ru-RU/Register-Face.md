...
Данная функция регистрирует изображение лица в API распознавания лиц DeepStack, загружая изображение на локальную конечную точку API. Она проверяет, что сервис DeepStack работает, и проверяет файл изображения перед загрузкой. Функция включает логику повторных попыток, обработку ошибок и очистку при сбое.
...
> Уникальный идентификатор лица (например, имя человека)
...
> Массив локальных путей к файлам изображений (png, jpg, jpeg или gif)
