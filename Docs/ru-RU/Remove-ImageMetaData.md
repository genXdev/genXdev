...
> Удаляет файлы метаданных из каталогов изображений.

## Description

Функция Remove-ImageMetaData удаляет сопутствующие JSON-файлы метаданных, связанные с изображениями. Она может выборочно удалять только ключевые слова (description.json), данные о людях (people.json), данные об объектах (objects.json) или данные о сценах (scenes.json), либо удалять все файлы метаданных, если не указан конкретный параметр. Языкозависимые файлы метаданных можно удалить, указав параметр Language, а все языковые варианты можно удалить с помощью параметра AllLanguages.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ImageDirectories` | String[] | — | — | 0 | `@(".\")` | Массив путей к каталогам для обработки с целью удаления метаданных изображений. Если не указан, используются системные каталоги по умолчанию. |
| `-Language` | String | — | — | 1 | — | Язык для удаления файлов метаданных, зависящих от языка. |
| `-PreferencesDatabasePath` | String | — | — | 2 | — | Путь к базе данных для файлов данных предпочтений |
| `-Recurse` | SwitchParameter | — | — | Named | — | Рекурсивный обход каталогов. |
| `-OnlyKeywords` | SwitchParameter | — | — | Named | — | Удалить только файлы description.json (ключевые слова/описания). |
| `-OnlyPeople` | SwitchParameter | — | — | Named | — | Удалять только файлы people.json (данные распознавания лиц). |
| `-OnlyObjects` | SwitchParameter | — | — | Named | — | Удаляйте только файлы objects.json (данные обнаружения объектов). |
| `-OnlyScenes` | SwitchParameter | — | — | Named | — | Удалите только файлы scenes.json (данные классификации сцен). |
| `-AllLanguages` | SwitchParameter | — | — | Named | — | Удалить файлы метаданных для всех поддерживаемых языков. |
...
Рекурсивно удаляет файлы описаний на английском и испанском языках из каталогов по умолчанию.
...
> Массив путей к каталогам для обработки с целью удаления метаданных изображений. Если не указан, используются системные каталоги по умолчанию.
...
> Язык для удаления файлов метаданных, зависящих от языка.
...
> Удалить только файлы description.json (ключевые слова/описания).
...
> Удалять только файлы people.json (данные распознавания лиц).
...
> Удаляйте только файлы objects.json (данные обнаружения объектов).
...
> Удалите только файлы scenes.json (данные классификации сцен).
...
> Удалить файлы метаданных для всех поддерживаемых языков.
