...
> Задает каталог для файлов изображений лиц, используемых в операциях GenXdev.AI.

## Description

Эта функция настраивает глобальный каталог лиц, используемый модулем GenXdev.AI для различных операций распознавания лиц и ИИ. Настройки могут сохраняться постоянно в предпочтениях (по умолчанию), только в текущем сеансе (с помощью -SessionOnly) или очищаться из сеанса (с помощью -ClearSession).
...
Очищает настройку каталога сессионных лиц (глобальную переменную), не затрагивая постоянные настройки.
