...
Использует Windows Shell32 API для перемещения системных папок, таких как «Документы», «Загрузки», «Рабочий стол» и других известных папок Windows, в новое расположение. Функция проверяет существование целевого пути, находит уникальный GUID папки в полном реестре известных папок и использует API SHSetKnownFolderPath для выполнения перемещения. Типичные сценарии использования включают перемещение пользовательских папок на другой диск для управления пространством или организации. Соблюдайте осторожность при перемещении критически важных для системы папок, так как это может повлиять на стабильность системы.
...
> Новый путь для известной папки
