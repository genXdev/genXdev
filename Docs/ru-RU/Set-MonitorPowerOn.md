...
Использует Windows API через GenXdev.Helpers.WindowObj для пробуждения монитора из режима сна/выключения. Это полезно для скриптов автоматизации, которым необходимо убедиться, что монитор включен.
