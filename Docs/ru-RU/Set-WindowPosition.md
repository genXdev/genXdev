...
> Перемещает и изменяет размер окон, когда указаны явные параметры позиционирования.

## Description

Обеспечивает точное управление положением и размером окна при указании параметров позиционирования. Поддерживает несколько мониторов, удаление рамок и различные предустановленные позиции, такие как разделение слева/справа, сверху/снизу и размещение по центру. Окна могут быть расположены по координатам или с помощью предопределенных макетов. Без параметров позиционирования функция не выполняет никаких действий над окном.
...
| `-Process` | Diagnostics.Process | — | ✅ (ByValue, ByPropertyName) | Named | — | Процесс расположения окна *(Parameter set: )* |
...
| `-Monitor` | Int32 | — | — | Named | `-1` | Выбор монитора: 0 = основной, 1+ = конкретный монитор, -1 = текущий, -2 = дополнительный |
...
| `-Y` | Int32 | — | — | Named | `-999999` | Вертикальное положение окна в пикселях |
...
> Имя процесса окна для позиционирования
...
> Выбор монитора: 0 = основной, 1+ = конкретный монитор, -1 = текущий, -2 = дополнительный
...
> Горизонтальное положение окна в пикселях
...
> Вертикальное положение окна в пикселях
