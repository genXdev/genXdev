...
> Отображает краткий алфавитный список всех глаголов PowerShell.

## Description

Отображает утвержденные глаголы PowerShell в виде списка, разделенного запятыми. Если в качестве входных данных указаны определенные глаголы, будут показаны только соответствующие глаголы. Поддерживает подстановочные знаки.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Verb` | String[] | — | ✅ (ByValue, ByPropertyName) | 0 | `@()` | Одна или несколько масок глаголов для фильтрации (поддерживаются подстановочные знаки) 🌐 *Supports wildcards* |
...
> Одна или несколько масок глаголов для фильтрации (поддерживаются подстановочные знаки)
