...
* Запускает исполняемый файл с настраиваемым уровнем приоритета и предоставляет
  возможности для ожидания и управления процессом.
* Обертка для Start-Process с дополнительной функциональностью для управления
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-FilePath` | String | ✅ | — | 0 | — | Путь к исполняемому файлу для запуска |
| `-ArgumentList` | String[] | — | — | 1 | — | Аргументы для передачи исполняемому файлу |
...
Запускает Блокнот с низким приоритетом и сразу возвращает управление.
...
> Аргументы для передачи исполняемому файлу
