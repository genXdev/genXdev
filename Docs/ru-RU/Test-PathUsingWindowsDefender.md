...
* Выполняет целенаправленное сканирование указанных файлов или каталогов с помощью интерфейса командной строки Защитника Windows (MpCmdRun.exe).
* Может сканировать в режиме только обнаружения или с включенным автоматическим исправлением угроз.
* Возвращает true, если угрозы не обнаружены, false — если угрозы найдены или сканирование не удалось.
...
| `-EnableRemediation` | SwitchParameter | — | — | Named | `False` | Дает указание Защитнику Windows выполнить действия с угрозами |
...
Передает путь к файлу в командлет с помощью псевдонима.

## Parameter Details

### `-FilePath <String>`

> Путь к файлу или каталогу для сканирования
...
> Дает указание Защитнику Windows выполнить действия с угрозами
