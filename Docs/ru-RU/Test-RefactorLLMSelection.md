...
> Оценивает файлы исходного кода на пригодность для рефакторинга с помощью анализа LLM.

## Description

Использует анализ языковой модели (LLM) для определения, следует ли выбрать файл исходного кода для рефакторинга на основе заданных критериев. Функция обрабатывает содержимое файла через запрос к LLM и возвращает логический ответ.
...
> Определение рефакторинга, содержащее настройки LLM
...
> Путь к исходному файлу для оценки
