...
Эта функция удаляет лицо из системы распознавания лиц DeepStack, используя его уникальный идентификатор. Она взаимодействует с конечной точкой API, чтобы навсегда удалить зарегистрированные данные лица из системы.
...
| `-ContainerName` | String | — | — | 1 | `'deepstack_face_recognition'` | Имя для контейнера Docker |
| `-VolumeName` | String | — | — | 2 | `'deepstack_face_data'` | Имя тома Docker для постоянного хранения |
| `-ServicePort` | Int32 | — | — | 3 | `5000` | Номер порта для службы DeepStack |
| `-HealthCheckTimeout` | Int32 | — | — | 4 | `60` | Максимальное время в секундах ожидания проверки работоспособности службы |
| `-HealthCheckInterval` | Int32 | — | — | 5 | `3` | Интервал в секундах между попытками проверки работоспособности |
...
> Уникальный идентификатор лица для удаления из системы DeepStack
