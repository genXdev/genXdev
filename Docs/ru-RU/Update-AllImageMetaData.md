...
Эта функция систематически обрабатывает изображения в различных системных каталогах для обновления их ключевых слов, данных распознавания лиц, данных обнаружения объектов и данных классификации сцен с помощью ИИ-сервисов. Она охватывает папки медиа-хранилища, системных файлов, загрузок, OneDrive и личных фотографий.

Функция обрабатывает изображения, проходя по каждому каталогу и обрабатывая файлы по отдельности. Сначала выполняются функции DeepStack (лица, объекты, сцены), затем генерируются ключевые слова и описания.

Это обеспечивает структурированный вывод данных для операций конвейера, таких как:
...
| `-NoRecurse` | SwitchParameter | — | — | Named | — | Не рекурсировать в подкаталоги при обработке изображений |
| `-RedoAll` | SwitchParameter | — | — | Named | — | Повторно обработать все изображения, независимо от предыдущей обработки |
...
> Массив путей к каталогам для обработки обновлений изображений
...
> URL конечной точки API, по умолчанию http://localhost:1234/v1/chat/completions
...
> Не рекурсировать в подкаталоги при обработке изображений
...
> Повторно обработать все изображения, независимо от предыдущей обработки
