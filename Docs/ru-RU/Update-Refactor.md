...
> Обновляет наборы рефакторинга и управляет ими, включая выбор файлов и обработку.
...
- Управления информацией о состоянии и отслеживанием прогресса
- Обработки выбора и обработки файлов на основе LLM
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Name` | String[] | — | ✅ (ByValue, ByPropertyName) | 0 | `@('*')` | Имя рефакторинга, принимает символы подстановки *(Parameter set: )* 🌐 *Supports wildcards* |
| `-Refactor` | GenXdev.Helpers.RefactorDefinition[] | — | — | 1 | — | Набор для рефакторинга для обновления *(Parameter set: )* |
| `-FilesToAdd` | IO.FileInfo[] | — | — | 2 | `@()` | Добавляемые имена файлов |
| `-FilesToRemove` | IO.FileInfo[] | — | — | 3 | `@()` | Удаляемые имена файлов |
| `-SelectByModifiedDateFrom` | DateTime | — | — | Named | — | Выберите файлы по дате изменения |
| `-SelectByModifiedDateTo` | DateTime | — | — | Named | — | Выборка файлов по дате изменения |
| `-SelectByCreationDateFrom` | DateTime | — | — | Named | — | Выберите файлы по дате создания из |
| `-SelectByCreationDateTo` | DateTime | — | — | Named | — | Выбрать файлы по дате создания для |
| `-PromptKey` | String | — | — | Named | — | Ключ prompt указывает, какой сценарий подсказки использовать |
| `-Prompt` | String | — | — | Named | `''` | Ключ prompt указывает, какой сценарий подсказки использовать |
| `-SelectionScript` | String | — | — | Named | — | Сценарий PowerShell для функции выбора элементов для рефакторинга |
...
| `-Priority` | Int32 | — | — | Named | — | Приоритет этого рефакторинга установлен |
| `-ExposedCmdLets` | GenXdev.Helpers.ExposedCmdletDefinition[] | — | — | Named | `@()` | Массив определений команд PowerShell, используемых в качестве инструментов во время выбора LLM |
| `-KeysToSend` | String[] | — | — | Named | `@()` | Клавиши для вызова в виде нажатий клавиш после открытия файла |
| `-PreferencesDatabasePath` | String | — | — | Named | — | Путь к базе данных для файлов данных предпочтений |
| `-CleanUpDeletedFiles` | SwitchParameter | — | — | Named | — | Очистить удаленные файлы |
| `-AskBeforeLLMSelection` | SwitchParameter | — | — | Named | — | Переключиться для подавления взаимодействия с пользователем |
| `-PerformAutoSelections` | SwitchParameter | — | — | Named | — | Переключатель для включения обработки выбора файлов на основе LLM |
| `-PerformAISelections` | SwitchParameter | — | — | Named | — | Переключиться на обработку всех файлов в наборе рефакторинга |
| `-RetryFailedLLMSelections` | SwitchParameter | — | — | Named | — | Переключиться на повторные попытки неудачных выборов LLM |
...
| `-ResetLMSelections` | SwitchParameter | — | — | Named | — | Перезапустить все LLMSelections |
...
| `-ReprocessModifiedFiles` | SwitchParameter | — | — | Named | — | Автоматически повторно обрабатывать файлы, измененные с момента последнего обновления |
...
| `-SelectByFreeRam` | SwitchParameter | — | — | Named | — | Выбор конфигурации на основе доступной системной памяти |
| `-SelectByFreeGpuRam` | SwitchParameter | — | — | Named | — | Выбор конфигурации по доступной памяти GPU |
...
> Набор для рефакторинга для обновления
...
> Выберите файлы по дате изменения
...
> Выборка файлов по дате изменения
...
> Выберите файлы по дате создания из
...
> Выбрать файлы по дате создания для
...
> Ключ prompt указывает, какой сценарий подсказки использовать
...
> Сценарий PowerShell для функции выбора элементов для рефакторинга
...
> Если указано, будет вызвана LLM для выполнения выбора на основе содержимого скрипта
...
> Приоритет этого рефакторинга установлен
...
> Массив определений команд PowerShell, используемых в качестве инструментов во время выбора LLM
...
> Клавиши для вызова в виде нажатий клавиш после открытия файла
...
> Переключиться для подавления взаимодействия с пользователем
...
> Переключатель для включения обработки выбора файлов на основе LLM
...
> Переключиться на обработку всех файлов в наборе рефакторинга
...
> Переключиться на повторные попытки неудачных выборов LLM
...
> Очистить все файлы из набора рефакторинга
...
> Очистить журнал набора рефакторинга
...
> Перезапустить все LLMSelections
...
> Автоматически повторно обрабатывать файлы, измененные с момента последнего обновления
...
> Выбор конфигурации на основе доступной системной памяти
...
> Выбор конфигурации по доступной памяти GPU
