...
* Атомарная запись: использует стратегию temp-файл + переименование, чтобы гарантировать, что целевой файл никогда не останется в поврежденном состоянии при прерывании процесса.
* Логика повторных попыток: повторяет запись до MaxRetries раз с задержкой RetryDelayMs миллисекунд между попытками.
* Поддержка подавления (debounce): когда DebounceMs > 0, быстрые последовательные записи в один и тот же файл объединяются — записывается только последнее содержимое, когда файл не изменялся в течение DebounceMs мс.
...
Быстро записывает 100 раз, но только последний полезный нагрузка (100) сохраняется на диск после 5-секундного периода затишья.
