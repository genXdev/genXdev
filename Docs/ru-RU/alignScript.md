...
> Возвращает строку (с измененным отступом) предоставленной строки скриптблока

## Description

Изменяет отступы строки скриптблока, сохраняя исходные отступы кодового блока
