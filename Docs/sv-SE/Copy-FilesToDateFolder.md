...
> Kopierar filer som matchar sökvillkor till datumbaserade undermappar.

## Description

Söker efter filer med samma parameteruppsättning som `Find-Item` och kopierar varje matchad fil till en undermapp till `TargetFolder` baserat på filens skapelse- eller mediadatum. Cmdleten stöder innehållsmatchning, enhetssökningar och många filter.

Försöker flera strategier för att bestämma ett korrekt skapelse- eller infångningsdatum för den angivna filen. Strategier inkluderar att läsa bildens EXIF-metadata, tolka datum/tid-information från filnamn och falla tillbaka till filens senaste skrivtid när ingen annan tillförlitlig information finns tillgänglig.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Målrotmapp dit matchade filer kommer att kopieras till datumbaserade undermappar |
...
| `-DeleteEmptyDirs` | SwitchParameter | — | — | Named | — | Ta bort tomma källkataloger efter att ha flyttat filer |
| `-TargetFolderNameDateSyntax` | String | — | — | Named | `'Year + Month'` | Målmappens namn med datum |
...
Copy-FilesToDateFolder -TargetFolder 'D:\Arkiv' -Namn '*.jpg' -WhatIf
...
Kopierar filer till en mapp för datum.
Denna cmdlet kopierar filer som matchar angivet mönster från källmappen till målmappen. Filer ordnas i undermappar baserat på datum då de skapades.

Parametrar:
-TargetFolder
Anger målmappen där kopierade filer ska lagras.

-Name
Anger filnamnsmönster för de filer som ska kopieras.

Exempel:
Copy-FilesToDateFolder -TargetFolder 'E:\Media\Videos' -Name '*.mp4'
Kopierar alla MP4-filer från aktuell mapp och underkataloger till 'E:\Media\Videos'.

## Parameter Details

### `-TargetFolder <String>`

> Målrotmapp dit matchade filer kommer att kopieras till datumbaserade undermappar
...
> Ta bort tomma källkataloger efter att ha flyttat filer
