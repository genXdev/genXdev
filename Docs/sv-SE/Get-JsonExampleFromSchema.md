...
> Genererar en läsbar exempel-JSON-sträng från en JSON-Schemadefinition.

## Description

Konverterar ett JSON-schema (OpenAI json_schema-format eller rått schema) till en kompakt exempel-JSON-sträng lämplig för att injicera i LLM-systeminstruktioner. Hanterar kapslade objekt, matriser, skalära typer, enum, unioner, oneOf/anyOf/allOf-sammansättningar och formathintar som datum-tid och e-post.

Funktionen bygger ett PowerShell-objektträd genom att rekursivt gå igenom schemat och serialiserar det sedan till en kompakt JSON-sträng via ConvertTo-Json.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Schema` | Object | — | — | 0 | — | En JSON Schema-definition (JSON-sträng eller PSCustomObject). Stöder OpenAI json_schema-omslagsformat och JSON Schema-objekt i raw-format. |
...
Genererar: {"items":[{"id":0,"label":"..."}]}

## Parameter Details

### `-Schema <Object>`

> En JSON Schema-definition (JSON-sträng eller PSCustomObject). Stöder OpenAI json_schema-omslagsformat och JSON Schema-objekt i raw-format.
