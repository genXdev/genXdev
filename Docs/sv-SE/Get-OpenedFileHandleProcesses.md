...
> Hämtar processer som har öppna filhandtag för specificerade filer.

## Description

* Använder Sysinternals-verktyget handle.exe för att identifiera processer som
  för närvarande har öppna handtag till en eller flera angivna filer.
* Användbart för att avgöra vilka processer som hindrar filoperationer
  som borttagning eller ändring.
* Kräver att handle.exe från Sysinternals-paketet är installerat och
  tillgängligt i systemets sökväg.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-FilePath` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Sökvägen till filen/filerna som ska kontrolleras för öppna handtag |
...
Identifierar alla processer som har öppna referenser till den angivna filen.
...
Kontrollerar flera filer via pipeline-indata efter processer med öppna hanterare.

## Parameter Details

### `-FilePath <String[]>`

> Sökvägen till filen/filerna som ska kontrolleras för öppna handtag
