...
> Flyttar filer som matchar sökvillkor till datumbaserade undermappar.

## Description

Söker efter filer med samma parameteruppsättning som `Find-Item` och flyttar varje matchad fil till en undermapp i `TargetFolder` baserat på filens skapelse- eller mediadatum. Cmdleten stöder innehållsmatchning, enhetsomfattande sökningar och många filter. Den kan eventuellt ta bort tomma källmappar efter flyttning.

Försöker flera strategier för att fastställa ett korrekt skapelse- eller inspelningsdatum för den angivna filen. Strategier inkluderar att läsa bildens EXIF-metadata, analysera datum/tidsinformation från filnamn och falla tillbaka till filens senaste skrivtid när ingen annan tillförlitlig information finns tillgänglig.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | @{response=Målrotmapp där matchade filer flyttas till datum-baserade undermappar} |
...
Flytta-filerTillDatumsmapp -Målmapp 'E:\Media\Videos' -Namn '.\*.mp4' -TaBortTommaMappar

## Parameter Details

### `-TargetFolder <String>`

> @{response=Målrotmapp där matchade filer flyttas till datum-baserade undermappar}
