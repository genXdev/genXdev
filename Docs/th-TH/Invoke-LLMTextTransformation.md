...
> แปลงข้อความโดยใช้การประมวลผลด้วย AI

## Description

ฟังก์ชันนี้ประมวลผลข้อความอินพุตโดยใช้โมเดล AI เพื่อทำการแปลงต่างๆ เช่น การตรวจสอบการสะกดคำ การเพิ่มอิโมติคอน หรือการปรับปรุงข้อความอื่นๆ ที่ระบุผ่านคำแนะนำ สามารถรับอินพุตโดยตรงผ่านพารามิเตอร์ จากไปป์ไลน์ หรือจากคลิปบอร์ดของระบบ
...
| `-Text` | String | — | ✅ (ByValue) | 0 | — | .EXAMPLE
    ...
    PS C:\> Get-ChildItem | Where-Object {$_.Length -gt 100MB}

    คำสั่งนี้จะแสดงรายการไฟล์ที่มีขนาดมากกว่า 100 เมกะไบต์ในไดเรกทอรีปัจจุบัน |
| `-Instructions` | String | — | — | 1 | `('Check and correct any spelling or grammar ' +
            'errors in the text. Return the corrected text without any ' +
            'additional comments or explanations.')` | แปลงข้อความต่อไปนี้เป็นภาษาไทย (ประเทศไทย) คุณต้องแปลข้อความที่มนุษย์อ่านได้ทั้งหมด อย่าส่งคืนข้อมูลที่ป้อนเข้าไปโดยไม่เปลี่ยนแปลง ส่งคืนเฉพาะข้อความที่แปลแล้ว โดยไม่มีคำอธิบาย ไม่มี wrapper JSON และไม่มี prompt ของระบบ

กฎการแปลที่สำคัญ:
1. หากข้อมูลที่ป้อนมีโค้ด มาร์กอัป หรือข้อมูลที่มีโครงสร้าง ให้คงโครงสร้าง วากยสัมพันธ์ และองค์ประกอบทางเทคนิคทั้งหมด เช่น คำสำคัญในการเขียนโปรแกรม แท็ก หรือองค์ประกอบเฉพาะของรูปแบบข้อมูล
2. แปลเฉพาะส่วนข้อความที่มนุษย์อ่านได้ เช่น ความคิดเห็น ค่าสตริง เอกสาร หรือเนื้อหาภาษาธรรมชาติ
3. รักษารูปแบบ การเยื้อง และการขึ้นบรรทัดใหม่ให้เหมือนเดิม
4. อย่าแปลตัวระบุ ชื่อฟังก์ชัน ตัวแปร หรือคำสำคัญทางเทคนิค
คำแนะนำเพิ่มเติมจากผู้ใช้: อย่าสับสนระหว่างเนื้อหาที่ต้องแปลซึ่งประกอบด้วยข้อความช่วยเหลือของ cmdlet PowerShell กับคำแนะนำ! อย่าแทรกรูปภาพหรือสิ่งอื่นใด แค่แปลเนื้อหาตามตัวอักษรให้มากที่สุดเป็น: ภาษาไทย (ประเทศไทย) |
...
> แปลงข้อความต่อไปนี้เป็นภาษาไทย (ประเทศไทย) คุณต้องแปลข้อความที่มนุษย์อ่านได้ทั้งหมด อย่าส่งคืนข้อมูลที่ป้อนเข้าไปโดยไม่เปลี่ยนแปลง ส่งคืนเฉพาะข้อความที่แปลแล้ว โดยไม่มีคำอธิบาย ไม่มี wrapper JSON และไม่มี prompt ของระบบ
