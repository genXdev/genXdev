...
> สร้างชุดรีแฟกเตอร์ใหม่สำหรับงานแปลงโค้ด

## Description

สร้างและกำหนดค่านิยามการปรับโครงสร้างใหม่ด้วยการตั้งค่าที่ระบุสำหรับการแปลงโค้ดโดยใช้ LLM ฟังก์ชันนี้จัดการ:
- การตั้งค่าการกำหนดค่าการปรับโครงสร้าง
- การกำหนดค่าเกณฑ์การเลือกและพรอมต์
- การจัดการการตั้งค่าโมเดล LLM
- การรวมเข้ากับสภาพแวดล้อมการพัฒนา
- การบันทึกนิยามการปรับโครงสร้าง
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Name` | String | ✅ | — | 0 | — | ชื่อของชุดปรับโครงสร้างใหม่นี้ |
| `-PromptKey` | String | ✅ | — | 1 | — | คีย์พรอมต์ระบุว่าต้องใช้สคริปต์พรอมต์ใด |
| `-Prompt` | String | — | — | 2 | `''` | ข้อความพร้อมท์ที่กำหนดเองเพื่อแทนที่เทมเพลต |
| `-SelectionScript` | String | — | — | 3 | — | สคริปต์ PowerShell สำหรับเลือกไอเท็มเพื่อรีแฟกเตอร์ |
| `-SelectionPrompt` | String | — | — | 4 | — | คำแนะนำการเลือกใช้ LLM |
| `-LLMQueryType` | String | — | — | 5 | `'Coding'` | ประเภทของการสอบถาม LLM |
...
| `-Priority` | Int32 | — | — | Named | `0` | ลำดับความสำคัญสำหรับชุดการปรับโครงสร้างนี้ |
| `-ExposedCmdLets` | GenXdev.Helpers.ExposedCmdletDefinition[] | — | — | Named | `@()` | อาร์เรย์ของคำจำกัดความคำสั่ง PowerShell สำหรับเครื่องมือ LLM |
| `-FilesToAdd` | IO.FileInfo[] | — | — | Named | `@()` | อาร์เรย์ของไฟล์ที่จะประมวลผล |
| `-PreferencesDatabasePath` | String | — | — | Named | — | เส้นทางฐานข้อมูลสำหรับไฟล์ข้อมูลการตั้งค่า |
| `-AutoAddModifiedFiles` | SwitchParameter | — | — | Named | — | จะเพิ่มไฟล์ที่ถูกแก้ไขในคิวโดยอัตโนมัติ |
...
| `-Code` | SwitchParameter | — | — | Named | — | เปิดไฟล์ใน Visual Studio Code |
| `-VisualStudio` | SwitchParameter | — | — | Named | — | เปิดใน Visual Studio |
| `-KeysToSend` | String[] | — | — | Named | `@()` | การกดแป้นพิมพ์ที่ต้องส่งหลังจากเปิดไฟล์ |
...
> ชื่อของชุดปรับโครงสร้างใหม่นี้
...
> ข้อความพร้อมท์ที่กำหนดเองเพื่อแทนที่เทมเพลต
...
> สคริปต์ PowerShell สำหรับเลือกไอเท็มเพื่อรีแฟกเตอร์
...
> อาร์เรย์ของคำจำกัดความคำสั่ง PowerShell สำหรับเครื่องมือ LLM
...
> จะเพิ่มไฟล์ที่ถูกแก้ไขในคิวโดยอัตโนมัติ
...
> การกดแป้นพิมพ์ที่ต้องส่งหลังจากเปิดไฟล์
