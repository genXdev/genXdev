...
> Belirtilen dizinlerde filtreleme özellikleri ve isteğe bağlı tarayıcı tabanlı galeri görüntüleme ile resim dosyalarını ve meta verilerini arar.

## Description

Belirtilen dizin ve alt dizinlerindeki resim dosyalarını (jpg, jpeg, png, gif, bmp, webp, tiff, tif) arar. Her resim için, ilişkili description.json, keywords.json, people.json ve objects.json dosyalarını metadata için kontrol eder. Anahtar kelime eşleşmeleri, kişi tanıma ve nesne algılamaya dayalı olarak resimleri filtreleyebilir ve sonuçları nesneler olarak döndürebilir. Sonuçları tarayıcı tabanlı bir masonry düzeninde görüntülemek için -ShowInBrowser kullanın.

Parametre Mantığı:
- Her parametre türü içinde (Anahtar Kelimeler, Kişiler, Nesneler, vb.): VEYA mantığı kullanılır.
  Örnek: -Keywords "kedi","köpek" YA kedi YA da köpek içeren resimleri bulur.
- Farklı parametre türleri arasında: VE mantığı kullanılır.
  Örnek: -Keywords "kedi" -People "Ahmet" hem kedi hem de Ahmet içeren resimleri bulur.
- EXIF aralık parametreleri: Aralıkları filtrelemek için [min, max] değerleri sağlayın.
- Dize parametreleri: * ve ? ile joker karakter eşleştirmeyi destekler.

İşlev, resim dizinlerini arar ve JSON formatında metadata içeren alternatif veri akışlarını inceler. Joker karakter kalıplarıyla anahtar kelimeleri eşleştirebilir, belirli kişiler için filtreleme yapabilir ve algılanan nesneleri arayabilir. Varsayılan olarak resim veri nesnelerini döndürür. Bir web tarayıcısında görüntülemek için -ShowInBrowser kullanın.
...
| `-Name` | String[] | — | — | 1 | `@('.\')` | Görsellerin aranacağı dizin yolları dizisi |
| `-Language` | String | — | — | Named | — | Açıklamalar ve anahtar kelimeler için dil. |
| `-DescriptionSearch` | String[] | — | — | Named | `@()` | Aranacak açıklama metni, joker karakterlere izin verilir. |
...
| `-Scenes` | String[] | — | — | Named | `@()` | Aranacak sahne kategorileri, joker karakterlere izin verilir. |
| `-PictureType` | String[] | — | — | Named | `@()` | Filtrelenecek resim türü (örneğin, 'gün ışığı', 'akşam', 'iç mekan' vb.). Joker karakterleri destekler. |
| `-StyleType` | String[] | — | — | Named | `@()` | Filtrelenecek stil türü (ör. 'gündelik', 'resmi' vb.). Joker karakterleri destekler. |
| `-OverallMood` | String[] | — | — | Named | `@()` | Filtrelenecek genel ruh hali (örneğin, 'sakin', 'neşeli', 'üzgün' vb). Joker karakterleri destekler. |
| `-MetaCameraMake` | String[] | — | — | Named | `@()` | Resim EXIF meta verilerinde kamera markasına göre filtrele. Joker karakterleri destekler. |
| `-MetaCameraModel` | String[] | — | — | Named | `@()` | Görüntü EXIF meta verilerinde kamera modeline göre filtrele. Joker karakterleri destekler. |
| `-MetaGPSLatitude` | Double[] | — | — | Named | — | Görüntü EXIF meta verilerinde GPS enlem aralığına göre filtrele. |
| `-MetaGPSLongitude` | Double[] | — | — | Named | — | Görüntü EXIF meta verilerinde GPS boylam aralığına göre filtrele. |
| `-MetaGPSAltitude` | Double[] | — | — | Named | — | Görüntü EXIF meta verilerinde GPS yükseklik aralığına göre filtrele (metre cinsinden). |
...
| `-MetaExposureTime` | Double[] | — | — | Named | — | Görüntü EXIF meta verilerinde pozlama süresi aralığına göre filtrele (saniye cinsinden). |
| `-MetaFNumber` | Double[] | — | — | Named | — | Görüntü EXIF meta verilerinde F-sayısı (diyafram) aralığına göre filtrele. |
| `-MetaISO` | Int32[] | — | — | Named | — | Görüntü EXIF meta verilerinde ISO duyarlılık aralığına göre filtrele. |
| `-MetaFocalLength` | Double[] | — | — | Named | — | Görüntü EXIF meta verilerinde odak uzaklığı aralığına (mm cinsinden) göre filtrele. |
| `-MetaWidth` | Int32[] | — | — | Named | — | EXIF meta verisinden piksel cinsinden görüntü genişlik aralığına göre filtrele. |
| `-MetaHeight` | Int32[] | — | — | Named | — | EXIF meta verilerinden piksel cinsinden görüntü yüksekliği aralığına göre filtrele. |
| `-MetaDateTaken` | DateTime[] | — | — | Named | — | EXIF meta verilerinden alınan çekim tarihine göre filtrele. Bir tarih aralığı olabilir. |
| `-Title` | String | — | — | Named | `'Photo Gallery'` | Galeri için başlık |
| `-Description` | String | — | — | Named | `('Hover over images to see face recognition ' +
            'and object detection data')` | Galeri için açıklama |
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | Tuş gönderirken, kontrol karakterlerini ve değiştiricileri kaçış karakteri ile belirtin. |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | Tuşları gönderdikten sonra klavye odağının PowerShell'e geri dönmesini engelle |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | Tuş gönderirken Enter yerine Shift+Enter kullanın |
| `-SendKeyDelayMilliSeconds` | Int32 | — | — | Named | — | Tuş gönderirken farklı giriş dizeleri arasındaki milisaniye cinsinden gecikme |
...
| `-Monitor` | Int32 | — | — | Named | `-2` | Kullanılacak monitör, 0 = varsayılan, -1 = yok say, -2 = yapılandırılmış ikincil monitör, varsayılan olarak `Global:DefaultSecondaryMonitor` veya bulunamazsa 2 |
...
| `-EmbedImages` | SwitchParameter | — | — | Named | — | Görselleri base64 olarak gömün. |
| `-NoFallback` | SwitchParameter | — | — | Named | — | Geri dönüş davranışını devre dışı bırakmak için anahtar. |
| `-NeverRebuild` | SwitchParameter | — | — | Named | — | Veritabanı başlatma ve yeniden oluşturmayı atlamak için geçiş yapın. |
...
| `-ShowInBrowser` | SwitchParameter | — | — | Named | — | Arama sonuçlarını tarayıcı tabanlı bir resim galerisinde görüntüleyin. |
| `-PassThru` | SwitchParameter | — | — | Named | — | Görüntü verilerini nesneler olarak döndürür. -ShowInBrowser ile kullanıldığında, hem galeriyi görüntüler hem de nesneleri döndürür. |
| `-NoBorders` | SwitchParameter | — | — | Named | — | Daha temiz bir görünüm için pencere kenarlıklarını ve başlık çubuğunu kaldırın |
| `-SideBySide` | SwitchParameter | — | — | Named | — | Tarayıcı penceresini, aynı monitörde PowerShell'in yanına yerleştirin |
| `-Interactive` | SwitchParameter | — | — | Named | — | Tarayıcıya bağlanacak ve Düzenle ve Sil gibi ek düğmeler ekleyecektir. Yalnızca -ShowInBrowser ile kullanıldığında etkilidir. |
| `-Private` | SwitchParameter | — | — | Named | — | Gizli/özel tarama modunda açar |
| `-Force` | SwitchParameter | — | — | Named | — | Gerekirse mevcut tarayıcıları durdurarak hata ayıklama bağlantı noktasını zorla etkinleştir |
| `-Edge` | SwitchParameter | — | — | Named | — | Microsoft Edge'de açar |
| `-Chrome` | SwitchParameter | — | — | Named | — | Google Chrome'da açar |
| `-Chromium` | SwitchParameter | — | — | Named | — | Varsayılan tarayıcının ne olduğuna bağlı olarak Microsoft Edge veya Google Chrome'da açılır |
| `-Firefox` | SwitchParameter | — | — | Named | — | Firefox'ta açar |
| `-PlayWright` | SwitchParameter | — | — | Named | — | Playwright tarafından yönetilen tarayıcıyı kullan, işletim sistemine kurulu tarayıcıyı değil |
...
| `-OnlyReturnHtml` | SwitchParameter | — | — | Named | — | ('Oluşturulan HTML'i tarayıcıda görüntülemek yerine yalnızca döndür.') |
| `-ShowOnlyPictures` | SwitchParameter | — | — | Named | — | Yalnızca resimleri yuvarlak dikdörtgen içinde göster, altında metin yok. |
| `-SessionOnly` | SwitchParameter | — | — | Named | — | AI tercihleri (Dil, Görsel koleksiyonları vb.) için oturumda depolanan alternatif ayarları kullan |
| `-ClearSession` | SwitchParameter | — | — | Named | — | Oturumda depolanan AI tercihleri için (Dil, Görsel koleksiyonları vb.) alternatif ayarları temizle |
| `-SkipSession` | SwitchParameter | — | — | Named | — | AI tercihleri için oturumda saklanan alternatif ayarları kullanma (Dil, Görsel koleksiyonları vb.) |
...
| `-MinConfidenceRatio` | Double | — | — | Named | — | Minimum güven oranı (0.0-1.0) kişileri, sahneleri ve nesneleri güvene göre filtrelemek için. Yalnızca bu değerden büyük veya eşit güvene sahip kişiler, sahneler ve nesneler için veri döndürür. |
...
> Görsellerin aranacağı dizin yolları dizisi
...
> Aranacak açıklama metni, joker karakterlere izin verilir.
...
> Aranacak sahne kategorileri, joker karakterlere izin verilir.
...
> Filtrelenecek resim türü (örneğin, 'gün ışığı', 'akşam', 'iç mekan' vb.). Joker karakterleri destekler.
...
> Filtrelenecek stil türü (ör. 'gündelik', 'resmi' vb.). Joker karakterleri destekler.
...
> Filtrelenecek genel ruh hali (örneğin, 'sakin', 'neşeli', 'üzgün' vb). Joker karakterleri destekler.
...
> Resim EXIF meta verilerinde kamera markasına göre filtrele. Joker karakterleri destekler.
...
> Görüntü EXIF meta verilerinde kamera modeline göre filtrele. Joker karakterleri destekler.
...
> Görüntü EXIF meta verilerinde GPS enlem aralığına göre filtrele.
...
> Görüntü EXIF meta verilerinde GPS boylam aralığına göre filtrele.
...
> Görüntü EXIF meta verilerinde GPS yükseklik aralığına göre filtrele (metre cinsinden).
...
> Görüntü EXIF meta verilerinde pozlama süresi aralığına göre filtrele (saniye cinsinden).
...
> Görüntü EXIF meta verilerinde F-sayısı (diyafram) aralığına göre filtrele.
...
> Görüntü EXIF meta verilerinde ISO duyarlılık aralığına göre filtrele.
...
> Görüntü EXIF meta verilerinde odak uzaklığı aralığına (mm cinsinden) göre filtrele.
...
> EXIF meta verisinden piksel cinsinden görüntü genişlik aralığına göre filtrele.
...
> EXIF meta verilerinden piksel cinsinden görüntü yüksekliği aralığına göre filtrele.
...
> EXIF meta verilerinden alınan çekim tarihine göre filtrele. Bir tarih aralığı olabilir.
...
> Tuş gönderirken, kontrol karakterlerini ve değiştiricileri kaçış karakteri ile belirtin.
...
> Tuşları gönderdikten sonra klavye odağının PowerShell'e geri dönmesini engelle
...
> Tuş gönderirken farklı giriş dizeleri arasındaki milisaniye cinsinden gecikme
...
> Kullanılacak monitör, 0 = varsayılan, -1 = yok say, -2 = yapılandırılmış ikincil monitör, varsayılan olarak `Global:DefaultSecondaryMonitor` veya bulunamazsa 2
...
> Görselleri base64 olarak gömün.
...
> Geri dönüş davranışını devre dışı bırakmak için anahtar.
...
> Veritabanı başlatma ve yeniden oluşturmayı atlamak için geçiş yapın.
...
> Arama sonuçlarını tarayıcı tabanlı bir resim galerisinde görüntüleyin.
...
> Görüntü verilerini nesneler olarak döndürür. -ShowInBrowser ile kullanıldığında, hem galeriyi görüntüler hem de nesneleri döndürür.
...
> Tarayıcı penceresini, aynı monitörde PowerShell'in yanına yerleştirin
...
> Tarayıcıya bağlanacak ve Düzenle ve Sil gibi ek düğmeler ekleyecektir. Yalnızca -ShowInBrowser ile kullanıldığında etkilidir.
...
> Gerekirse mevcut tarayıcıları durdurarak hata ayıklama bağlantı noktasını zorla etkinleştir
...
> Varsayılan tarayıcının ne olduğuna bağlı olarak Microsoft Edge veya Google Chrome'da açılır
...
> Playwright tarafından yönetilen tarayıcıyı kullan, işletim sistemine kurulu tarayıcıyı değil
...
> ('Oluşturulan HTML'i tarayıcıda görüntülemek yerine yalnızca döndür.')
...
> Yalnızca resimleri yuvarlak dikdörtgen içinde göster, altında metin yok.
...
> AI tercihleri (Dil, Görsel koleksiyonları vb.) için oturumda depolanan alternatif ayarları kullan
...
> Oturumda depolanan AI tercihleri için (Dil, Görsel koleksiyonları vb.) alternatif ayarları temizle
...
> AI tercihleri için oturumda saklanan alternatif ayarları kullanma (Dil, Görsel koleksiyonları vb.)
...
> Minimum güven oranı (0.0-1.0) kişileri, sahneleri ve nesneleri güvene göre filtrelemek için. Yalnızca bu değerden büyük veya eşit güvene sahip kişiler, sahneler ve nesneler için veri döndürür.
