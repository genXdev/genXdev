...
> Doppler kaydırılmış frekansı hesaplar.

## Description

f' = f * (v + vo) / (v - vs) kullanır, hızlar birbirine doğru pozitif olarak alınır.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-OriginalFrequencyInHertz` | Double | ✅ | — | 0 | `0` | Hz cinsinden orijinal frekans |
| `-SourceSpeedInMetersPerSecond` | Double | ✅ | — | 1 | `0` | m/s cinsinden kaynak hızı (gözlemciye doğru pozitif) |
| `-ObserverSpeedInMetersPerSecond` | Double | ✅ | — | 2 | `0` | Gözlemci hızı m/s (kaynağa doğru pozitif) |
| `-SpeedOfSoundInMetersPerSecond` | Double | — | — | 3 | `0` | Ses hızı (varsayılan: 343) m/s cinsinden *(Parameter set: )* |
| `-Medium` | String | ✅ | — | 3 | — | Ortam *(Parameter set: )* |
| `-As` | String | — | — | 4 | — | Frekans için çıkış birimi |
...
Su ortamında, kaynak 10 m/s hızla gözlemciye doğru hareket ederken, gözlemci 5 m/s hızla kaynağa doğru hareket ederken, 440 Hz'lik bir ton için Doppler kaymalı frekansı hesaplar, çıktıyı kilohertz cinsinden verir.
...
Kaynağı 10 m/s hızla hareket eden, gözlemcinin sabit olduğu, ses hızının 1480 m/s olduğu bir ortamda 440 Hz'lik bir ton için Doppler kaymalı frekansı hesaplar.

## Parameter Details

### `-OriginalFrequencyInHertz <Double>`

> Hz cinsinden orijinal frekans
...
> m/s cinsinden kaynak hızı (gözlemciye doğru pozitif)
...
> Gözlemci hızı m/s (kaynağa doğru pozitif)
