...
> Şu anda etkin olan ön plan penceresinin tanıtıcısını alır.

## Description

* Bu cmdlet, şu anda ön planda olan ve klavye odağına sahip pencerenin pencere tutamacını (HWND) alır.
* P/Invoke aracılığıyla Windows API işlevlerini kullanarak User32.dll kütüphanesiyle etkileşime girer.
...
Şu anda aktif olan pencerenin IntPtr tanıtıcısını alır ve döndürür.
...
Ön plan penceresi tanıtıcısını bir değişkende saklar ve görüntüler.
