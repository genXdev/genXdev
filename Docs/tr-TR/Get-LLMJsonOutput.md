...
> PowerShell cmdlet yardım metinlerini çeviriyorum. Lütfen çevrilecek metni sağlayın.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Text` | String | ✅ | — | 0 | — | @{response=Hata ayıklamayı etkinleştirmek için -Debug parametresini kullanın veya $DebugPreference değişkenini 'Continue' veya 'Inquire' olarak ayarlayın.} |
| `-ResponseFormat` | String | — | — | 1 | — | @{response=İsteğe bağlı JSON şeması dizesi, çıkarmayı yönlendirmek için (OpenAI json_schema formatı)} |

## Examples

### Get-LLMJsonOutput -Metin '
...
> @{response=Hata ayıklamayı etkinleştirmek için -Debug parametresini kullanın veya $DebugPreference değişkenini 'Continue' veya 'Inquire' olarak ayarlayın.}
...
> @{response=İsteğe bağlı JSON şeması dizesi, çıkarmayı yönlendirmek için (OpenAI json_schema formatı)}
