...
> Yapay zeka destekli işleme kullanarak metni dönüştürür.

## Description

Bu işlev, yazım denetimi, ifade ekleme veya talimatlarla belirtilen diğer metin iyileştirmeleri gibi çeşitli dönüşümler gerçekleştirmek için AI modellerini kullanarak girdi metnini işler. Girdiyi doğrudan parametreler aracılığıyla, işlem hattından veya sistem panosundan kabul edebilir.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Text` | String | — | ✅ (ByValue) | 0 | — | Konu Başlığı: Birden çok bilgisayarda bir PowerShell oturumu (PSSession) çalıştırma

Kısa Açıklama
    Bir veya daha fazla uzak bilgisayarda komutları çalıştırmak için bir veya daha fazla kalıcı bağlantı oluşturur.

Söz Dizimi
    New-PSSession [[-ComputerName] <String[]>] [-Credential <PSCredential>] [-Name <String[]>] [-Port <Int32>] [-SessionOption <PSSessionOption>] [-ThrottleLimit <Int32>] [-UseSSL] [-CommonParameters>]

Açıklama
    New-PSSession cmdlet'i, yerel veya uzak bilgisayarlara kalıcı bir PowerShell oturumu (PSSession) oluşturur. Bir PSSession oluşturduğunuzda, PowerShell uzak bilgisayarla kalıcı bir bağlantı kurar. Oluşturduğunuz PSSession'ları, birden çok komut ve işlev çalıştıran görevler için kullanın. Bir PSSession'da bir komut dizisi çalıştırmak için, her komutun ayrı ayrı gönderilmesi gerekmez. Bir PSSession kullanıyorsanız, verileri paylaşabilir ve işlevlerin ad alanını, değişkenleri ve diğer kaynakları kullanabilirsiniz.

    New-PSSession cmdlet'i, bir veya daha fazla bilgisayarda bir PSSession oluşturur. Uzak bilgisayarlarda arka plan işleri oluşturmak için Start-Job cmdlet'ini içeren bir PSSession kullanabilirsiniz. Bir uzak bilgisayarda etkileşimli bir oturum çalıştırmak için Enter-PSSession cmdlet'ini kullanın. Sistem yönetimi meta verileri ve yapılandırma bilgileri için WS-Management oturumları oluşturmak üzere WSMan sağlayıcısının New-WSManSession cmdlet'ini de kullanabilirsiniz. Daha fazla bilgi için about_WSMan_Provider konusuna bakın.

    Ana bilgisayar 64 bit ve 32 bit işletim sistemleri çalıştırdığında, PowerShell 64-bit sürümünde çalıştırıldığında New-PSSession hedef bilgisayarda 64-bit bir oturum alır. PowerShell 32-bit sürümünde çalıştırıldığında, hedef bilgisayarda 32-bit bir oturum alır.

Parametreler
    -ComputerName <String[]>
        Uzak bilgisayarları belirtir. Bir bilgisayar adı veya birden çok bilgisayar adı ve IP adresi belirtebilirsiniz. Varsayılan yerel bilgisayardır.

        Bilgisayar adı NetBIOS adı, IP adresi veya tam etki alanı adı (FQDN) biçiminde yazın. Tek bir bilgisayarı belirtmek için ComputerName parametresini kullanın. Birden çok bilgisayar belirtmek için, bilgisayarları virgülle ayırın. Uzak bilgisayara bağlanmak için, bilgisayarın Seans Açma'ya izin vermek üzere yapılandırılmış olması gerekir. Daha fazla bilgi için about_Remote_Troubleshooting konusuna bakın.

        Bu parametreyi atlarsanız, New-PSSession yerel bilgisayarda bir PSSession oluşturur.

    -Credential <PSCredential>
        Bu eylemi yapma yetkisine sahip bir kullanıcı hesabı belirtir. Varsayılan geçerli kullanıcıdır. User01 veya Domain01\User01 gibi bir kullanıcı adı yazın veya Get-Credential cmdlet'i tarafından oluşturulan bir PSCredential nesnesi girin. Bir kullanıcı adı yazdığınızda, bir parola girmeniz istenir. |
| `-Instructions` | String | — | — | 1 | `('Check and correct any spelling or grammar ' +
            'errors in the text. Return the corrected text without any ' +
            'additional comments or explanations.')` | Cmdlet'i etkinleştirme etkisini belirtir; yani bu işlevin bir cmdlet olduğunu belirten öznitelik uygulanmıştır. Cmdlet'i etkinleştirme, bunu bir cmdlet olarak etiketler. Bu parametre $true olarak ayarlandığında, Get-Help çağrıldığında cmdlet için Yardım alınır. |
...
> Konu Başlığı: Birden çok bilgisayarda bir PowerShell oturumu (PSSession) çalıştırma
...
> Cmdlet'i etkinleştirme etkisini belirtir; yani bu işlevin bir cmdlet olduğunu belirten öznitelik uygulanmıştır. Cmdlet'i etkinleştirme, bunu bir cmdlet olarak etiketler. Bu parametre $true olarak ayarlandığında, Get-Help çağrıldığında cmdlet için Yardım alınır.
