...
> Bir Bing arama sorgusunu bir web tarayıcısında açar.

## Description

Bir veya daha fazla Bing arama sorgusunu, yapılandırılabilir tarayıcı ayarları ve komut satırı anahtarları kullanarak bir web tarayıcısında açar. Bu işlev, kapsamlı tarayıcı özelleştirme seçenekleri (pencere konumlandırma, monitör seçimi, gizli tarama ve tuş vuruşu otomasyonu dahil) ile Bing aramaları gerçekleştirmek için kapsamlı bir arayüz sağlar.

İşlev, sorguları otomatik olarak URL kodlar ve Bing'in arama API'si için biçimlendirir. Edge, Chrome ve Firefox dahil tüm büyük tarayıcıları, gizli tarama, uygulama modu, hassas pencere yönetimi ve tuş vuruşu otomasyonu gibi özelliklerle destekler.
...
Varsayılan monitörde "PowerShell betiği" için bir Bing araması açar.
...
Takma ad ve kısa parametre adlarını kullanarak bir Bing araması açar.
...
"makine öğrenimi" için İngilizce dil sonuçlarıyla özel bir tarama oturumu açar.
...
İşlem hattı aracılığıyla birden fazla terim arar ve sonuçları tüm yüklü tarayıcılarda açarak pencereleri ekranın sol tarafına konumlandırır.
