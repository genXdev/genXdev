...
> Google sorgularını, yapılandırılabilir ayarlar ve kapsamlı özelleştirme seçenekleriyle bir web tarayıcısında açar.

## Description

Bir veya daha fazla Google sorgusunu bir web tarayıcısında açarak, birden çok dil, monitör seçimi, pencere konumlandırma, tarayıcı seçimi ve klavye otomasyonu için kapsamlı destek sağlar. İşlev, tarayıcı davranışı ve görünümü üzerinde hassas kontrol ile Google aramaları gerçekleştirmek için özellik açısından zengin bir arayüz sunar.

Temel özellikler şunları içerir:
- Google'ın dil filtrelemesi yoluyla 100'den fazla dil desteği
- Otomatik veya manuel monitör seçimi ile çoklu monitör desteği
- Pencere konumlandırma (sol, sağ, üst, alt, ortalanmış, tam ekran)
- Gizli/özel tarama modu desteği
- Dikkat dağıtmayan tarama için uygulama modu
- Uzantı ve açılır pencere engelleme seçenekleri
- Odak yönetimi ve pencere manipülasyonu
- Boru hattı desteği ile toplu sorgu işleme
- Tarayıcı pencerelerine tuş vuruşu otomasyonu
- Altta yatan tarayıcı işlevlerine akıllı parametre aktarımı

İşlev, arama sorgularını Google'ın arama motoru için otomatik olarak biçimlendirir ve belirtildiğinde dil filtrelemesi uygular. Tüm tarayıcı konumlandırma ve yönetim özellikleri, alttaki Open-Webbrowser işlevinden miras alınır.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Google'da arama yapmak için kullanılacak anahtar kelimeler |
| `-Language` | String | — | — | 1 | — | Google arama sonuçları için dil |
...
| `-Monitor` | Int32 | — | — | Named | `-1` | Kullanılacak monitör, 0 = varsayılan, -1 = at, -2 = Yapılandırılmış ikincil monitör, varsayılan -1 |
...
Birincil monitörde İngilizce olarak "PowerShell scripting" için Google araması açar.
...
"Makine öğrenmesi algoritmaları" için Google'da tam ekran modunda, monitör 2'de takma adlar kullanarak bir arama açar.
...
Pipeline girişi üzerinden, her biri İngilizce olarak Chrome'da birden çok Google araması açar.
...
"Yapay zeka" için yeni bir pencerede özel gezinti araması açar.
...
Ekranın ortasında, tarayıcı kontrolleri olmadan uygulama modunda bir arama açar.

## Parameter Details

### `-Queries <String[]>`

> Google'da arama yapmak için kullanılacak anahtar kelimeler
...
> Google arama sonuçları için dil
...
> Kullanılacak monitör, 0 = varsayılan, -1 = at, -2 = Yapılandırılmış ikincil monitör, varsayılan -1
