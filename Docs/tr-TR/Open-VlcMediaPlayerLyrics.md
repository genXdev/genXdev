...
> Şu anda VLC'de oynatılan medyanın şarkı sözlerini aramak için bir web tarayıcısı açar.

## Description

Bu işlev, VLC Media Player'ın çalışıp çalışmadığını algılar ve o anda oynatılan medya başlığına göre şarkı sözlerini arar. Geçerli parçanın şarkı sözlerini bulmak için bir web tarayıcısında otomatik olarak bir arama sorgusu açar. İşlev, birden çok tarayıcıyı destekler ve geniş pencere konumlandırma ve görüntüleme seçenekleri sunar.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | — | ✅ (ByValue, ByPropertyName) 🌐 Remaining | 0 | — | Bir parçayı bulmak için arama terimleri |
| `-EndPoint` | String | — | — | 1 | `'Google'` | Sorguyu çağıracak uç nokta |
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | Tarayıcı kapatıldıktan sonra kalıcı olmayan bir oturum kullanın |
| `-ClearSession` | SwitchParameter | — | — | Named | — | Tarayıcıyı açmadan önce tarayıcı oturum verilerini temizleyin |
| `-SkipSession` | SwitchParameter | — | — | Named | — | Mevcut herhangi bir tarayıcı oturumunu kullanmayı atla |
...
Şu anda oynatılan VLC medyasının şarkı sözleri için bir Google araması açar.
...
Microsoft Edge'de gizli/özel modda şarkı söz aramasını açar.

## Parameter Details

### `-Queries <String[]>`

> Bir parçayı bulmak için arama terimleri
...
> Tarayıcı kapatıldıktan sonra kalıcı olmayan bir oturum kullanın
...
> Tarayıcıyı açmadan önce tarayıcı oturum verilerini temizleyin
...
> Mevcut herhangi bir tarayıcı oturumunu kullanmayı atla
