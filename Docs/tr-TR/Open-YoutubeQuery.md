...
> YouTube arama sorgularını bir web tarayıcısında açar.

## Description

Bir veya daha fazla YouTube arama sorgusunu, kapsamlı konumlandırma ve yapılandırma seçenekleriyle bir web tarayıcısında açar. Yapılandırılabilir monitör seçimi, tarayıcı ayarları ve Open-Webbrowser cmdlet'inden devralınan dinamik parametreler aracılığıyla tuş vuruşu otomasyonunu destekler. Her sorgu, pencere konumlandırma, tarayıcı türü ve arama dili üzerinde tam kontrol sağlayan ayrı bir tarayıcı örneğinde açılır.
...
Varsayılan monitörde "PowerShell tutorial" için bir YouTube araması açar.
...
Fonksiyon takma adını kullanarak yapılandırılmış ikinci monitörde 'nasıl kod yazılır' için bir YouTube araması açar.
