...
> Виконує модульні тести для модулів, підмодулів або командлетів GenXdev
з інтелектуальним налагодженням та вирішенням помилок на основі ШІ.

## Description

Ця функція надає тестову інфраструктуру для модулів GenXdev,
пропонуючи багаторівневе виконання тестів від цілих модулів до окремих
командлетів. Вона інтегрує PSScriptAnalyzer для статичного аналізу коду, Pester для
модульного тестування та PSScriptAnalyzer для статичного аналізу коду. Функція
включає інтелектуальну обробку помилок з використанням можливостей вирішення на основі штучного інтелекту
та детальне звітування про прогрес для робочих процесів розробки.
...
| `-TestFailedAction` | String | — | — | 1 | `'Continue'` | Дія, яку потрібно виконати при збої тесту. Параметри: Ask, Continue, Stop, SolveWithAI, Write-Error, Throw |
| `-DefinitionMatches` | String | — | ✅ (ByValue, ByPropertyName) | 2 | — | Регулярний вираз для відповідності визначенням командлетів |
| `-ModuleName` | String[] | — | ✅ (ByValue, ByPropertyName) | 3 | — | Імена модулів GenXdev для пошуку 🌐 *Supports wildcards* |
...
| `-ExactMatch` | SwitchParameter | — | — | Named | — | Вимагати точних збігів для назв командлетів |
| `-Verbosity` | String | — | — | Named | `'Normal'` | Рівень деталізації виведення |
| `-StackTraceVerbosity` | String | — | — | Named | `'FirstLine'` | Рівень деталізації трасування стеку |
| `-AllowLongRunningTests` | SwitchParameter | — | — | Named | — | Також вибирає модульні тести, які мають тривалий час виконання |
| `-SkipModuleImports` | SwitchParameter | — | — | Named | — | Пропускає імпорт модулів GenXdev перед тестуванням *(Parameter set: )* |
| `-SkipPSAnalyzerTests` | SwitchParameter | — | — | Named | — | Пропускає виконання тестів PSAnalyzer |
| `-SkipPesterTests` | SwitchParameter | — | — | Named | — | Пропускає виклик тестів Pester |
...
> Дія, яку потрібно виконати при збої тесту. Параметри: Ask, Continue, Stop, SolveWithAI, Write-Error, Throw
...
> Вимагати точних збігів для назв командлетів
...
> Рівень деталізації трасування стеку
...
> Також вибирає модульні тести, які мають тривалий час виконання
...
> Пропускає імпорт модулів GenXdev перед тестуванням
...
> Пропускає виконання тестів PSAnalyzer
...
> Пропускає виклик тестів Pester
