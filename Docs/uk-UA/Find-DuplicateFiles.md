...
> Знаходить дублікати файлів у кількох каталогах на основі настроюваних критеріїв.

## Description

Рекурсивно шукає дублікати файлів у вказаних каталогах. Файли вважаються дублікатами, якщо їхній вміст або повністю однаковий, або починається з однакового вмісту (для часткового порівняння за допомогою параметра -ComparePartial). Командлет використовує швидке хешування початкових байтів файлу для ефективного групування перед виконанням повного порівняння вмісту. Розмір цього швидкого хешу можна налаштувати за допомогою параметра -CompareByteLength, що дозволяє знайти баланс між продуктивністю та точністю виявлення дублікатів. Лише файли, що мають мінімальну довжину, яка дорівнює значенню CompareByteLength, будуть розглядатися для виявлення дублікатів; менші файли будуть згруповані лише за назвою.
...
| `-ComparePartial` | SwitchParameter | — | — | Named | — | Поверне файли різного розміру, доки вони починаються з однакової назви та вмісту, це виявляє пошкоджені завантаження/операції копіювання |
| `-CompareByteLength` | Int32 | — | — | Named | `65536` | Швидке порівняння довжини файлів у байтах, використовується для швидкого індексування та пошуку дублікатів, цей метод не порівнює вміст файлів |
...
> Поверне файли різного розміру, доки вони починаються з однакової назви та вмісту, це виявляє пошкоджені завантаження/операції копіювання
...
> Швидке порівняння довжини файлів у байтах, використовується для швидкого індексування та пошуку дублікатів, цей метод не порівнює вміст файлів
