...
> Отримує модулі GenXDev у порядку залежностей.

## Description

Ця функція повертає список модулів GenXDev, упорядкованих у правильному порядку залежностей, щоб забезпечити належне завантаження модулів. Спочатку вона отримує всю інформацію про модулі, а потім впорядковує їх на основі залежностей, починаючи з основних модулів і закінчуючи залежними модулями. Це гарантує, що модулі завантажуються у правильній послідовності.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ModuleName` | String[] | — | ✅ (ByValue, ByPropertyName) | 0 | `@('GenXdev*')` | Одна або більше назв модулів для фільтрування |
...
> Одна або більше назв модулів для фільтрування
