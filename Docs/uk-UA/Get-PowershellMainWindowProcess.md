...
> Повертає об'єкт процесу для вікна, в якому розміщено термінал PowerShell.

## Description

* Піднімається по дереву процесів, починаючи з поточного процесу PowerShell, щоб знайти батьківське вікно, яке відповідає за хостинг термінала.
* Якщо безпосередній батьківський процес не має дескриптора головного вікна, він шукає подібні процеси, які мають головні вікна.
* Корисно для ідентифікації фактичного процесу вікна термінала (наприклад, Windows Terminal, ConHost тощо), який містить сеанс PowerShell.
...
Повертає процес, який розміщує поточний сеанс PowerShell, і відображає його
назву.
