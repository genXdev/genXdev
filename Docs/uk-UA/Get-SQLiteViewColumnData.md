...
> Отримує дані стовпців з подання SQLite з можливим обмеженням записів.

## Description

Виконує запит SELECT до вказаного подання SQLite для отримання даних з одного стовпця. Підтримує підключення через рядок з'єднання або шлях до файлу бази даних. Дозволяє обмежити кількість записів, що повертаються, або отримати всі записи.
...
| `-ColumnName` | String | ✅ | — | 2 | — | Назва стовпця. |
| `-Count` | Int32 | — | — | 3 | `100` | Кількість записів для повернення. Тихо за замовчуванням — 100. -1 для всіх. |
