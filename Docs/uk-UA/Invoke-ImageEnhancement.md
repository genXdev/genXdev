...
> Покращує зображення, збільшуючи його в 4 рази з підвищенням якості за допомогою DeepStack.

## Description

Ця функція покращує зображення, збільшуючи його в 4 рази по ширині та висоті, одночасно підвищуючи якість зображення. Вона використовує локальний DeepStack API покращення зображень, який працює на порту, що налаштовується, та повертає покращене зображення у вигляді даних base64 або зберігає його у файл. Функція підтримує прискорення GPU та керування Docker-контейнерами.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ImagePath` | String | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Локальний шлях до файлу зображення для покращення |
| `-OutputPath` | String | — | — | 1 | — | Необов'язковий шлях, де слід зберегти покращене зображення |
...
> Локальний шлях до файлу зображення для покращення
...
> Необов'язковий шлях, де слід зберегти покращене зображення
