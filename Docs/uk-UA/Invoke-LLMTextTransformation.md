...
> Перетворює текст за допомогою обробки на основі штучного інтелекту.

## Description

Ця функція обробляє вхідний текст за допомогою моделей ШІ для виконання різноманітних
перетворень, таких як перевірка орфографії, додавання емодзі або будь-які інші
покращення тексту, визначені через інструкції. Вона може приймати вхідні дані безпосередньо
через параметри, з конвеєра або з системного буфера обміну.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Text` | String | — | ✅ (ByValue) | 0 | — | Розпізнайте справжній DHCP-сервер. Надійне надсилання DHCP-повідомлень лише до авторизованого DHCP-сервера. |
| `-Instructions` | String | — | — | 1 | `('Check and correct any spelling or grammar ' +
            'errors in the text. Return the corrected text without any ' +
            'additional comments or explanations.')` | Інструкції для моделі ШІ щодо трансформації тексту |
...
> Розпізнайте справжній DHCP-сервер. Надійне надсилання DHCP-повідомлень лише до авторизованого DHCP-сервера.
...
> Інструкції для моделі ШІ щодо трансформації тексту
