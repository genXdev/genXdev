...
> Переміщує файли, що відповідають критеріям пошуку, до підпапок на основі дат.

## Description

Шукає файли, використовуючи той самий набір параметрів, що й `Find-Item`, і переміщує кожен знайдений файл у підпапку `TargetFolder` на основі дати створення або медіа-дати файлу. Командлет підтримує зіставлення вмісту, пошук по всьому диску та багато фільтрів. За бажанням може видаляти порожні вихідні папки після переміщення.

Використовує кілька стратегій для визначення точної дати створення або зйомки зазначеного файлу. Стратегії включають зчитування метаданих EXIF зображення, аналіз інформації про дату/час з імен файлів та повернення до часу останнього запису файлу, якщо немає іншої надійної інформації.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Коренева цільова папка, до якої відповідні файли будуть переміщені в підпапки на основі дати |
...
Move-FilesToDateFolder -TargetFolder 'D:\Архів' -Name '*.jpg' -WhatIf
...
> Коренева цільова папка, до якої відповідні файли будуть переміщені в підпапки на основі дати
