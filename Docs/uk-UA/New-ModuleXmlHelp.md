...
> Генерує XML-файли довідки MAML для будь-якого модуля PowerShell.

## Description

Створює файли довідки MAML XML для будь-якого модуля PowerShell шляхом вилучення метаданих з усіх командлетів у модулі за допомогою Get-CmdletMetaData та створює як ModuleName.dll-Help.xml (для командлетів C#), так і ModuleName-help.xml (для
...
| `-Force` | SwitchParameter | — | — | Named | — | Перезаписувати наявні файли довідки XML без підтвердження |
...
| `-LinkPrefix` | String | — | — | Named | — | URL-префікс для посилання на онлайнову версію в кожному пов'язаному посиланні cmdlet (наприклад, https://github.com/genXdev/genXdev/blob/main/Docs/en-US/) |
| `-TranslationInstructions` | String | — | — | Named | — | Користувацькі інструкції для перекладу LLM (передані до Get-TextTranslation) |
...
Генерує XML-довідку англійською мовою для модуля Pester у en-US.
...
Створює XML-довідку нідерландською мовою для GenXdev, перезаписуючи наявні файли.
...
Показує, які файли були б створені без фактичного їх запису.
...
Створює англомовний довідковий XML з URL-адресами онлайн-версій для кожного командлета, які вказують на документи GitHub Markdown. Команда Get-Help <cmdlet> -Online відкриває браузер.
...
> Перезаписувати наявні файли довідки XML без підтвердження
...
> URL-префікс для посилання на онлайнову версію в кожному пов'язаному посиланні cmdlet (наприклад, https://github.com/genXdev/genXdev/blob/main/Docs/en-US/)
...
> Користувацькі інструкції для перекладу LLM (передані до Get-TextTranslation)
