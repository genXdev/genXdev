...
> Встановлює файли до буфера обміну Windows для операцій з файлами, таких як копіювання/вставлення.

## Description

Ця функція копіює один або декілька шляхів до файлів у буфер обміну Windows,
забезпечуючи операції з файлами, такі як вставка у Провіднику Windows. Вона автоматично
обробляє режими потоків STA та MTA, забезпечуючи сумісність у різних контекстах
виконання PowerShell. Функція перевіряє існування файлів перед додаванням шляхів до буфера обміну.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-InputObject` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Масив шляхів до файлів, які потрібно додати до буфера обміну |
...
> Масив шляхів до файлів, які потрібно додати до буфера обміну
