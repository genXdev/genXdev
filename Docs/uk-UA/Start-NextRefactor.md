...
> Продовжує або перезапускає сеанс рефакторингу коду.

## Description

Керує операціями рефакторингу коду, обробляючи визначення рефакторингу в порядку пріоритету. Виконує вибір файлів, відстеження прогресу, обробку помилок та забезпечує інтерактивний контроль користувача над процесом рефакторингу.
...
| `-FilesToAdd` | IO.FileInfo[] | — | — | 1 | `@()` | Імена файлів для додавання |
| `-FilesToRemove` | IO.FileInfo[] | — | — | 2 | `@()` | Імена файлів для видалення |
...
| `-ResetLMSelections` | SwitchParameter | — | — | Named | — | Перезапустити всі вибране LLM |
