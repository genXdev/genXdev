...
> Пакетно оновлює ключові слова зображень, обличчя, об'єкти та сцени в кількох системних каталогах.

## Description

Ця функція систематично обробляє зображення в різних системних каталогах для оновлення їхніх ключових слів, даних розпізнавання облич, даних виявлення об'єктів і даних класифікації сцен за допомогою служб AI. Вона охоплює медіасховище, системні файли, завантаження, OneDrive та особисті папки з фотографіями.

Функція обробляє зображення, переглядаючи кожен каталог та обробляючи файли окремо. Спочатку виконуються функції DeepStack (обличчя, об'єкти, сцени), а потім генерування ключових слів і описів.

Це забезпечує структурований вивід даних для операцій конвеєра, таких як:
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ImageDirectories` | String[] | ✅ | — | 0 | — | Масив шляхів до каталогів для обробки оновлень зображень |
...
| `-ConfidenceThreshold` | Double | — | — | Named | `0.7` | Мінімальний поріг впевненості (0.0-1.0) для виявлення об'єктів |
| `-Language` | String | — | — | Named | — | Мова для згенерованих описів та ключових слів |
| `-Model` | String | — | ✅ (ByValue) | Named | — | Ім'я або частковий шлях моделі для ініціалізації 🌐 *Supports wildcards* |
| `-ApiEndpoint` | String | — | — | Named | `$null` | URL кінцевої точки API, за замовчуванням http://localhost:1234/v1/chat/completions |
| `-ApiKey` | String | — | — | Named | `$null` | Ключ API для використання в запиті |
| `-NoSupportForJsonSchema` | String | — | — | Named | — | Вказує, що LLM не підтримує JSON-схеми |
| `-TimeoutSeconds` | Int32 | — | — | Named | — | Час очікування запиту в секундах, за замовчуванням 24 години |
...
| `-NoRecurse` | SwitchParameter | — | — | Named | — | Не рекурсувати в підкаталоги під час обробки зображень |
| `-RedoAll` | SwitchParameter | — | — | Named | — | Переробити всі зображення, незалежно від попередньої обробки |
...
> Масив шляхів до каталогів для обробки оновлень зображень
...
> Мінімальний поріг впевненості (0.0-1.0) для виявлення об'єктів
...
> Ім'я або частковий шлях моделі для ініціалізації
...
> URL кінцевої точки API, за замовчуванням http://localhost:1234/v1/chat/completions
...
> Ключ API для використання в запиті
...
> Час очікування запиту в секундах, за замовчуванням 24 години
...
> Не рекурсувати в підкаталоги під час обробки зображень
...
> Переробити всі зображення, незалежно від попередньої обробки
