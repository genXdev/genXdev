...
> Відновлює стандартну конфігурацію додаткового монітора.

## Description

Цей сценарій відновлює стандартну конфігурацію додаткового монітора для системи, встановлюючи додатковому монітору початкове значення за замовчуванням. Це корисно для користувачів, які хочуть повернутися до попередньої багатоекранної конфігурації після використання режиму "поруч". Див. також: функція 'sidebyside' для перемикання в режим "поруч" для нових вікон.
...
Відновлює конфігурацію додаткового монітора за замовчуванням для системи.
