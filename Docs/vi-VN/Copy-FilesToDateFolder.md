...
> Sao chép các tệp khớp với tiêu chí tìm kiếm vào các thư mục con dựa trên ngày.

## Description

Tìm kiếm tệp bằng cùng bộ tham số như `Find-Item` và sao chép từng
tệp khớp vào một thư mục con của `TargetFolder` dựa trên ngày tạo
hoặc ngày phương tiện của tệp. Lệnh ghép hỗ trợ đối sánh nội dung, tìm kiếm trên toàn bộ ổ đĩa và
nhiều bộ lọc.

Thử một số chiến lược để xác định ngày tạo hoặc chụp chính xác
cho tệp đã chỉ định. Các chiến lược bao gồm đọc siêu dữ liệu EXIF của ảnh,
phân tích thông tin ngày/giờ từ tên tệp và dự phòng sang thời gian ghi cuối cùng của tệp
khi không có thông tin đáng tin cậy nào khả dụng.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Thư mục gốc đích nơi các tệp tin khớp sẽ được sao chép vào các thư mục con dựa trên ngày |
...
> Thư mục gốc đích nơi các tệp tin khớp sẽ được sao chép vào các thư mục con dựa trên ngày
