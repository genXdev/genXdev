...
> Truy xuất các tiến trình có tay cầm tệp mở đến các tệp được chỉ định.

## Description

* Sử dụng công cụ handle.exe của Sysinternals để xác định các tiến trình
  hiện đang có handle mở tới một hoặc nhiều tệp tin cụ thể.
* Hữu ích để xác định tiến trình nào đang ngăn chặn các thao tác với tệp
  như xóa hoặc sửa đổi.
* Yêu cầu handle.exe từ bộ Sysinternals phải được cài đặt và có sẵn trong
  đường dẫn hệ thống.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-FilePath` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Đường dẫn đến (các) tệp để kiểm tra các tay cầm đang mở |
...
Xác định tất cả các tiến trình có tay cầm mở tới tệp được chỉ định.
...
Kiểm tra nhiều tệp qua đầu vào đường ống để tìm các tiến trình có tay cầm đang mở.

## Parameter Details

### `-FilePath <String[]>`

> Đường dẫn đến (các) tệp để kiểm tra các tay cầm đang mở
