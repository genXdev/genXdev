...
> Truy xuất danh sách các view từ cơ sở dữ liệu SQL Server.

## Description

Lấy tất cả tên view từ cơ sở dữ liệu SQL Server đã chỉ định. Trả về một mảng các tên view có thể được sử dụng cho các thao tác cơ sở dữ liệu tiếp theo. Hàm hỗ trợ hai bộ tham số để linh hoạt: cung cấp chuỗi kết nối hoặc tên cơ sở dữ liệu với máy chủ.
