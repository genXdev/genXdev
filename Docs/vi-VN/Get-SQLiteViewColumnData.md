...
> Truy xuất dữ liệu cột từ một view SQLite với tùy chọn giới hạn bản ghi.

## Description

Thực thi một truy vấn SELECT trên một view SQLite được chỉ định để lấy dữ liệu từ một cột duy nhất. Hỗ trợ kết nối thông qua chuỗi kết nối hoặc đường dẫn tệp cơ sở dữ liệu. Cho phép giới hạn số lượng bản ghi trả về hoặc lấy tất cả bản ghi.
...
| `-ColumnName` | String | ✅ | — | 2 | — | Tên của cột. |
| `-Count` | Int32 | — | — | 3 | `100` | Số lượng bản ghi cần trả về. Mặc định là 100. -1 cho tất cả. |
