...
> Cập nhật siêu dữ liệu phân loại cảnh cho các tệp hình ảnh trong một thư mục được chỉ định.

## Description

Hàm này xử lý hình ảnh trong một thư mục được chỉ định để phân loại cảnh bằng trí tuệ nhân tạo. Nó tạo ra các tệp tin JSON chứa metadata với các phân loại cảnh, điểm số tin cậy và nhãn. Hàm hỗ trợ xử lý hàng loạt với ngưỡng tin cậy có thể cấu hình và có thể tùy chọn bỏ qua các tệp tin metadata đã tồn tại hoặc thử lại các phân loại thất bại trước đó.
...
Sử dụng bí danh để thử lại các phân loại thất bại và chỉ xử lý hình ảnh mới trong nhiều thư mục.
...
Buộc tái tạo container và sử dụng tăng tốc GPU để xử lý nhanh hơn.
...
Xử lý đệ quy tất cả hình ảnh và chỉ lưu trữ các phân loại cảnh có độ tin cậy >= 60%.
