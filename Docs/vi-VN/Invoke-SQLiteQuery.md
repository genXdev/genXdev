...
> Thực thi một hoặc nhiều truy vấn SQL đối với cơ sở dữ liệu SQLite với hỗ trợ giao dịch.

## Description

Thực thi các truy vấn SQL trên cơ sở dữ liệu SQLite với hỗ trợ tham số và cô lập giao dịch có thể cấu hình. Có thể sử dụng giao dịch bên ngoài cho các thao tác hàng loạt hoặc tạo giao dịch nội bộ riêng. Khi sử dụng giao dịch bên ngoài, người gọi có trách nhiệm cam kết/hủy bỏ. Ưu tiên kết nối: Giao dịch > ConnectionString > DatabaseFilePath.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ConnectionString` | String | — | — | 0 | — | Chuỗi kết nối đến cơ sở dữ liệu SQLite. |
| `-DatabaseFilePath` | String | — | — | 1 | — | Đường dẫn đến tệp cơ sở dữ liệu SQLite. |
| `-Transaction` | Object | — | — | 2 | — | Một giao dịch SQLite hiện có để sử dụng cho các truy vấn. |
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) 🌐 Remaining | 3 | — | Truy vấn hoặc các truy vấn cần thực thi. |
| `-SqlParameters` | Collections.Hashtable[] | — | ✅ (ByValue, ByPropertyName) 🌐 Remaining | 4 | `@()` | Các tham số truy vấn dưới dạng bảng băm. |
...
> Một giao dịch SQLite hiện có để sử dụng cho các truy vấn.
