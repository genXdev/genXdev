...
> Di chuyển các tệp tin phù hợp với tiêu chí tìm kiếm vào các thư mục con dựa trên ngày tháng.

## Description

Tìm kiếm tệp bằng cách sử dụng cùng bộ tham số như `Find-Item` và di chuyển từng tệp khớp vào một thư mục con của `TargetFolder` dựa trên ngày tạo hoặc ngày phương tiện của tệp. Cmdlet hỗ trợ khớp nội dung, tìm kiếm trên toàn bộ ổ đĩa và nhiều bộ lọc. Nó có thể tùy chọn xóa các thư mục nguồn rỗng sau khi di chuyển.

Thử một số chiến lược để xác định ngày tạo hoặc ngày chụp chính xác cho tệp được chỉ định. Các chiến lược bao gồm đọc siêu dữ liệu EXIF của hình ảnh, phân tích thông tin ngày/giờ từ tên tệp và dự phòng đến thời gian ghi cuối cùng của tệp khi không có thông tin tin cậy nào khả dụng.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-TargetFolder` | String | ✅ | — | 0 | — | Thư mục gốc đích nơi các tệp tin khớp sẽ được di chuyển vào các thư mục con dựa trên ngày |
...
| `-DeleteEmptyDirs` | SwitchParameter | — | — | Named | — | Xóa các thư mục nguồn trống sau khi di chuyển tệp |
| `-TargetFolderNameDateSyntax` | String | — | — | Named | `'Year + Month'` | Cú pháp ngày của tên thư mục đích |
...
DiChuyen-TepVaoThuMucNgay -ThuMucDich 'D:\Archive' -Ten '*.jpg' -WhatIf
...
> Thư mục gốc đích nơi các tệp tin khớp sẽ được di chuyển vào các thư mục con dựa trên ngày
...
> Xóa các thư mục nguồn trống sau khi di chuyển tệp
...
> Cú pháp ngày của tên thư mục đích
