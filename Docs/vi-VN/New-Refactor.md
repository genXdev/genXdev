...
> Tạo một bộ tái cấu trúc mới cho các tác vụ chuyển đổi mã nguồn.

## Description

Tạo và cấu hình một định nghĩa tái cấu trúc mới với các thiết lập cụ thể cho các biến đổi mã nguồn dựa trên LLM. Hàm xử lý:
- Thiết lập cấu hình tái cấu trúc
- Cấu hình tiêu chí lựa chọn và lời nhắc
- Quản lý thiết lập mô hình LLM
- Tích hợp với môi trường phát triển
- Lưu trữ các định nghĩa tái cấu trúc
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Name` | String | ✅ | — | 0 | — | Tên của bộ tái cấu trúc mới này |
| `-PromptKey` | String | ✅ | — | 1 | — | Khóa nhắc (prompt key) chỉ định tập lệnh nhắc nào sẽ sử dụng |
| `-Prompt` | String | — | — | 2 | `''` | Văn bản tùy chỉnh để ghi đè lên mẫu |
| `-SelectionScript` | String | — | — | 3 | — | Tập lệnh PowerShell để chọn các mục cần tái cấu trúc |
| `-SelectionPrompt` | String | — | — | 4 | — | Hướng dẫn lựa chọn LLM |
| `-LLMQueryType` | String | — | — | 5 | `'Coding'` | Loại truy vấn LLM |
...
| `-Priority` | Int32 | — | — | Named | `0` | Mức độ ưu tiên cho đợt tái cấu trúc này |
| `-ExposedCmdLets` | GenXdev.Helpers.ExposedCmdletDefinition[] | — | — | Named | `@()` | Mảng các định nghĩa lệnh PowerShell cho công cụ LLM |
| `-FilesToAdd` | IO.FileInfo[] | — | — | Named | `@()` | Mảng các tệp tin cần xử lý |
| `-PreferencesDatabasePath` | String | — | — | Named | — | Đường dẫn cơ sở dữ liệu cho các tệp dữ liệu ưu tiên |
| `-AutoAddModifiedFiles` | SwitchParameter | — | — | Named | — | Sẽ tự động thêm các tệp đã sửa đổi vào hàng đợi |
...
| `-Code` | SwitchParameter | — | — | Named | — | Mở tệp trong Visual Studio Code |
| `-VisualStudio` | SwitchParameter | — | — | Named | — | Mở trong Visual Studio |
| `-KeysToSend` | String[] | — | — | Named | `@()` | Các tổ hợp phím để gửi sau khi mở tệp tin |
...
> Tên của bộ tái cấu trúc mới này
...
> Văn bản tùy chỉnh để ghi đè lên mẫu
...
> Tập lệnh PowerShell để chọn các mục cần tái cấu trúc
...
> Mảng các định nghĩa lệnh PowerShell cho công cụ LLM
...
> Sẽ tự động thêm các tệp đã sửa đổi vào hàng đợi
...
> Mở tệp trong Visual Studio Code
...
> Các tổ hợp phím để gửi sau khi mở tệp tin
