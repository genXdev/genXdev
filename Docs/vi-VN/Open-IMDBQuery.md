...
> Mở một truy vấn tìm kiếm IMDB trong trình duyệt web.

## Description

Mở truy vấn "Internet Movie Database" trong trình duyệt web với các tùy chọn cấu hình phong phú. Hàm này cung cấp một trình bao bọc thuận tiện cho chức năng tìm kiếm IMDB, cho phép người dùng tìm kiếm phim, chương trình truyền hình, diễn viên và nội dung giải trí khác trực tiếp từ PowerShell.

Hàm chấp nhận các cụm từ tìm kiếm và xây dựng các URL tìm kiếm IMDB thích hợp, sau đó mở chúng trong trình duyệt web được chỉ định với các tùy chọn vị trí cửa sổ, lựa chọn trình duyệt và tùy chọn hiển thị có thể tùy chỉnh. Nó hỗ trợ tất cả các trình duyệt chính bao gồm Edge, Chrome và Firefox, với các tùy chọn cho duyệt web riêng tư, chế độ toàn màn hình và quản lý cửa sổ.

Các tính năng chính bao gồm hỗ trợ nhiều màn hình, bản địa hóa ngôn ngữ, tự động hóa bàn phím và các tùy chọn cấu hình trình duyệt toàn diện. Hàm có thể xử lý nhiều truy vấn tìm kiếm cùng lúc và cung cấp các tùy chọn cho cả sử dụng tương tác và quy trình làm việc tự động.
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | Các từ khóa tìm kiếm trên IMDB |
...
| `-DisablePopupBlocker` | SwitchParameter | — | — | Named | — | Tắt chức năng chặn cửa sổ bật lên của trình duyệt |
...
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | Thoát khỏi các ký tự điều khiển khi gửi tổ hợp phím đến trình duyệt |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | Ngăn trả lại tiêu điểm bàn phím cho PowerShell sau khi gửi tổ hợp phím |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | Sử dụng Shift+Enter thay vì Enter thông thường để xuống dòng khi gửi phím |
...
Mở tìm kiếm IMDB cho "The Matrix" trên màn hình mặc định.
...
Mở tìm kiếm IMDB cho "The Matrix" sử dụng các tham số bí danh và viết tắt.
...
Tìm kiếm nhiều phim trên IMDB với kết quả bằng tiếng Pháp trong Chrome.
...
Tìm kiếm Tom Hanks trên IMDB ở chế độ toàn màn hình, sau đó trả lại tiêu điểm cho PowerShell.

## Parameter Details

### `-Queries <String[]>`

> Các từ khóa tìm kiếm trên IMDB
...
> Tắt chức năng chặn cửa sổ bật lên của trình duyệt
...
> Thoát khỏi các ký tự điều khiển khi gửi tổ hợp phím đến trình duyệt
...
> Ngăn trả lại tiêu điểm bàn phím cho PowerShell sau khi gửi tổ hợp phím
...
> Sử dụng Shift+Enter thay vì Enter thông thường để xuống dòng khi gửi phím
