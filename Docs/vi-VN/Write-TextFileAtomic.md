...
> Ghi văn bản vào một tệp một cách nguyên tử để ngăn hỏng tệp.

## Description

* Ghi nguyên tử: sử dụng chiến lược tệp tạm thời + đổi tên để đảm bảo tệp đích không bao giờ ở trạng thái hỏng nếu tiến trình bị gián đoạn.
* Logic thử lại: thử ghi tối đa MaxRetries lần với độ trễ RetryDelayMs mili giây giữa các lần thử.
* Hỗ trợ mã hóa: chấp nhận tham số System.Text.Encoding để kiểm soát mã hóa đầu ra. Mặc định là UTF-8.
* Hỗ trợ Debounce: khi DebounceMs > 0, các lần ghi liên tiếp nhanh vào cùng một tệp sẽ được gộp lại — chỉ tải trọng cuối cùng được ghi khi tệp không bị chạm trong DebounceMs ms.
...
| `-Data` | String | — | — | 1 | — | Nội dung văn bản cần ghi vào tệp. |
...
| `-Encoding` | Text.Encoding | — | — | Named | — | Mã hóa văn bản được sử dụng khi ghi tệp. Mặc định là UTF-8. |
...
Viết một chuỗi vào config.txt bằng cách sử dụng mã hóa UTF-8 một cách nguyên tử.
...
Truyền nhanh 50 dòng; chỉ dòng cuối cùng được duy trì sau khoảng thời gian yên lặng 2 giây.
...
Ghi một chuỗi vào data.csv bằng cách sử dụng mã hóa ASCII một cách nguyên tử.
...
Một công việc nền ghi dấu thời gian mỗi 100ms nhưng debounce gộp chúng — chỉ một lần ghi mỗi giây thực sự xảy ra trên đĩa.
...
> Nội dung văn bản cần ghi vào tệp.
...
> Mã hóa văn bản được sử dụng khi ghi tệp. Mặc định là UTF-8.
