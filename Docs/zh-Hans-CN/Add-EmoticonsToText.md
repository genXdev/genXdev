...
> 使用AI添加符合语境的表情符号来增强文本。

## Description

此函数处理输入文本，添加与情感语境匹配的表情符号。它可以直接通过参数、管道或系统剪贴板接受输入。该函数利用AI模型分析文本并选择合适的情感符号，使消息更具表现力和吸引力。
...
Get-AppLockerPolicy -Local | Test-AppLockerPolicy -Path C:\Windows\System32\calc.exe -User Everyone

Description
-----------
The Test-AppLockerPolicy cmdlet checks whether a specified list of files and users matches any AppLocker rules in a given policy. This cmdlet outputs a list of files that are identified as matching policy rules. |
