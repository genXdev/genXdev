...
> 向 README 文件添加带时间戳的 ReleaseNote 行。

## Description

向指定的README文件添加一条发布说明行，前缀为当前日期（yyyyMMdd格式）。该行可格式化为代码并可选择显示。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Line` | String | — | — | 0 | `''` | 要添加的发行说明描述文本 |
| `-Code` | SwitchParameter | — | — | Named | — | 将这一行格式化为代码 |
| `-Show` | SwitchParameter | — | — | Named | — | 在添加该行之后显示README |
