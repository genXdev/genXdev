...
> 使用 DeepStack 比较两张不同图像中的人脸并返回其相似度。

## Description

此函数比较两张图像中的人脸以确定相似度。它使用在可配置端口上运行的本地 DeepStack 人脸匹配 API，并返回介于 0.0 和 1.0 之间的相似度分数。这通常用于将身份证件与人物照片进行匹配，或验证两张照片是否显示同一个人。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ImagePath1` | String | ✅ | — | 0 | — | 要比较的第一个图像文件的本地路径 |
| `-ImagePath2` | String | ✅ | — | 1 | — | 要比较的第二张图片文件的本地路径 |
