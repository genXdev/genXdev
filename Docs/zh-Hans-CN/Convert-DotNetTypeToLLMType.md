...
> 将 .NET 类型名称转换为 LLM（语言模型）类型名称。

## Description

以 .NET 类型名称作为输入，并返回语言模型中使用的相应简化类型名称。它处理常见的 .NET 类型并提供适当的类型映射。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-DotNetType` | String | ✅ | — | 0 | — | 要转换为 LLM 类型名称的 .NET 类型名称 |
...
将 System.String 类型转换为其 LLM 等价形式。

## Parameter Details

### `-DotNetType <String>`

> 要转换为 LLM 类型名称的 .NET 类型名称
