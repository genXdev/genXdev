...
> Converts polite, professional corporate language into direct, plain speech using AI.

## Description

该函数处理输入文本，将外交和企业沟通转化为更直接、清晰的语言。它可以通过参数直接接受输入、从管道接收输入或从系统剪贴板获取输入。该函数利用AI模型分析和改写文本，同时保留原始意图。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Text` | String | — | ✅ (ByValue) | 0 | — | 将企业用语转换的文本 |
