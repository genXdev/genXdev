...
> Converts polite, professional corporate language into direct, clear speech using AI.

## Description

该函数处理输入文本，将外交、企业沟通转化为更直接、清晰的语言。它可以通过参数直接输入、通过管道输入或从系统剪贴板输入。该函数利用AI模型分析和改写文本，同时保留原始意图。
