...
> 根据另一个函数的可能参数，将绑定参数中的参数值复制到新的哈希表中。

## Description

* 此 cmdlet 创建一个新的哈希表，仅包含与指定目标函数中定义的参数相匹配的参数值。
* 然后可以使用 PowerShell 的展开语法，利用生成的哈希表来调用该函数。
* 开关参数仅当在绑定参数中显式提供且设置为 $true 时，才会包含在结果中。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-BoundParameters` | Object[] | ✅ | — | 0 | — | 要复制的源绑定参数 |
| `-FunctionName` | String | ✅ | — | 1 | — | 要筛选参数的目标函数名称 |
