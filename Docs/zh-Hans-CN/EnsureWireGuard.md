...
> 确保通过 Docker 容器安装并运行 WireGuard VPN 服务。

## Description

此函数使用 Docker Desktop 设置并管理 WireGuard VPN 服务。它自动确保 Docker Desktop 正在运行，拉取最新的 WireGuard Docker 镜像，创建持久化存储卷，并管理容器生命周期，包括健康监控和重启功能。

WireGuard 是一种简单、快速且现代的 VPN，采用最先进的密码学技术。与 OpenVPN 等传统 VPN 解决方案相比，它具有卓越的性能和简单性，配置开销极小，并具有出色的跨平台支持。
