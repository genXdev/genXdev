...
* 它会首先检查全局变量（除非指定了 SkipSession），然后回退到持久化偏好设置，最后使用系统默认值。
...
| `-SessionOnly` | SwitchParameter | — | — | Named | `False` | 使用会话中存储的替代设置来配置AI偏好，如语言、图像集合等 |
| `-ClearSession` | SwitchParameter | — | — | Named | `False` | 在检索之前清除会话设置（全局变量） |
| `-SkipSession` | SwitchParameter | — | — | Named | `False` | 不要使用会话中存储的替代设置来获取AI偏好设置，如语言、图像集合等 |
