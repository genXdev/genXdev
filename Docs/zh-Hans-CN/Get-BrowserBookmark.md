...
从系统上安装的 Microsoft Edge、Google Chrome 或 Mozilla Firefox 浏览器中检索书签。该函数可以按浏览器类型进行筛选，并返回详细的书签信息，包括名称、URL、文件夹位置和时间戳。当读取 Firefox 书签时，会自动处理 System.Data.SQLite NuGet 软件包安装的同意请求。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Chrome` | SwitchParameter | — | — | Named | — | 从Google Chrome返回书签 |
| `-Edge` | SwitchParameter | — | — | Named | — | 从 Microsoft Edge 返回书签 |
| `-Firefox` | SwitchParameter | — | — | Named | — | 从Mozilla Firefox返回书签 *(Parameter set: )* |
