...
返回一个包含当前浏览器页面的 Playwright Page、BrowserContext 和会话数据的哈希表。此引用可与 Invoke-WebbrowserEvaluation -ByReference 一起使用，以定位特定页面。

可以使用 -Edge、-Chrome、-Chromium、-Firefox 或 -Webkit 开关指定浏览器类型。如果未指定开关，则默认使用 Chromium。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Edge` | SwitchParameter | — | — | Named | — | 首选 Microsoft Edge 浏览器会话 |
| `-Chrome` | SwitchParameter | — | — | Named | — | 首选 Google Chrome 浏览器会话 |
| `-Chromium` | SwitchParameter | — | — | Named | — | 首选 Microsoft Edge 或 Google Chrome，具体取决于默认浏览器是什么 |
| `-Firefox` | SwitchParameter | — | — | Named | — | 首选 Firefox 浏览器会话 |
| `-Webkit` | SwitchParameter | — | — | Named | — | 优先使用 WebKit 浏览器会话 |
...
> 首选 Microsoft Edge 或 Google Chrome，具体取决于默认浏览器是什么
