...
> 根据名称模式从 GenXdev 首选项中检索重构定义。

## Description

搜索并加载存储在 GenXdev 偏好设置中的重构定义集。每个重构集都存储为一个 JSON 字符串，其偏好名称以 'refactor_set_' 开头。此函数将匹配到的集合反序列化为对象。
...
| `-DefaultValue` | String | — | — | Named | — | 未找到首选项时的默认值 |
