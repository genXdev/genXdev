...
* 通过查询 Windows 注册表检索信息，包括名称、描述、图标路径、可执行文件路径以及默认浏览器状态。
* 仅返回在 Windows 中注册了所需功能的浏览器。
...
| `-Firefox` | SwitchParameter | — | — | Named | `False` | 选择Firefox浏览器实例 *(Parameter set: )* |
