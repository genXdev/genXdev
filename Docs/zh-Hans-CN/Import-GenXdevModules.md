...
> 将所有 GenXdev PowerShell 模块导入全局作用域。

## Description

扫描父目录查找GenXdev模块，并将每个模块导入全局作用域。使用位置堆栈管理来保留工作目录，并为成功和失败的导入提供视觉反馈。在导入过程中跟踪函数计数的变化。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-DebugFailedModuleDefinitions` | SwitchParameter | — | — | Named | — | 为失败的模块定义启用调试输出 |
