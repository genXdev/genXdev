...
* 通过合并默认系统位置、Chocolatey 路径、开发工具路径以及自定义包路径，构建一个全面的搜索路径列表。
* 使用这些合并后的路径更新系统的 PATH 环境变量。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-WorkspaceFolder` | String | — | — | 0 | — | 用于 node 模块和 PowerShell 路径的工作区文件夹路径。 |
...
> 用于 node 模块和 PowerShell 路径的工作区文件夹路径。
