...
> 调用 PowerShell 脚本分析器来分析 PowerShell 脚本是否符合规范并遵循最佳实践。

## Description

此函数提供对 PSScriptAnalyzer 的封装，用于分析 PowerShell 脚本中的合规性问题、最佳实践违规和潜在错误。它支持基于文件和基于字符串的脚本分析，并可自定义规则和设置。
...
| `-ScriptDefinition` | String | ✅ | — | Named | — | 将脚本定义指定为一个字符串。 *(Parameter set: )* |
| `-EnableExit` | SwitchParameter | — | — | Named | — | 指定工具应在出错时退出。 |
| `-Fix` | SwitchParameter | — | — | Named | — | 启用自动修复违规行为。 |
