...
此函数处理指定目录中的图像，使用人工智能检测对象。它会创建包含检测到的对象、位置、置信度和标签的JSON元数据文件。该函数支持批量处理，具有可配置的置信度阈值，并可选择跳过现有元数据文件或重新处理先前失败的检测。
...
此示例处理 C:\Photos 和 D:\Pictures 及其所有子目录中的全部图像，使用默认设置，置信度阈值为 0.5。
...
此示例使用GPU加速，置信度阈值设为0.7，以实现更准确但较少的物体检测。
