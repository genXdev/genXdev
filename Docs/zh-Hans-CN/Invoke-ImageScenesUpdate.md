...
该函数处理指定目录中的图像，利用人工智能进行场景分类。它会生成包含场景分类、置信度分数和标签的JSON元数据文件。该函数支持批量处理，可配置置信度阈值，并可选择跳过现有的元数据文件或重试之前失败的分类。
...
处理 C:\Photos 和 D:\Pictures 及其子目录中的所有图像，进行场景分类。
