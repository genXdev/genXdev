...
此函数使用AI模型处理输入文本，执行各种转换，如拼写检查、添加表情符号或通过指令指定的任何其他文本增强。它可以接受直接通过参数、管道或系统剪贴板的输入。
...
| `-Text` | String | — | ✅ (ByValue) | 0 | — | .SYNOPSIS
    删除或禁用指定的 PowerShell 模块。

.DESCRIPTION
    Remove-Module cmdlet 从当前会话中移除指定的模块。如果模块包含程序集，则此 cmdlet 将卸载该程序集。如果模块在运行时生成并包含 .dll 文件，则不能保存此更改。

.PARAMETER Name
    指定要从当前会话中移除的模块的名称。支持通配符。

.PARAMETER ModuleInfo
    指定代表模块的 ModuleInfo 对象。可以传递包含 ModuleInfo 对象的变量，或使用 Get-Module 命令获取 ModuleInfo 对象。

.PARAMETER Force
    如果模块在运行时生成，则强制从当前会话中移除模块。即使在移除模块时出错，此参数也会尝试移除该模块。

.PARAMETER ErrorAction
    确定 cmdlet 如何响应非终止错误。有效值为 Stop、Continue、SilentlyContinue 和 Inquire。默认值为 Continue。

.PARAMETER WarningAction
    确定 cmdlet 如何响应警告。有效值为 Stop、Continue、SilentlyContinue 和 Inquire。默认值为 Continue。

.PARAMETER InformationAction
    确定 cmdlet 如何响应信息流消息。有效值为 Stop、Continue、SilentlyContinue 和 Inquire。默认值为 Continue。

.PARAMETER Verbose
    指示 cmdlet 在执行过程中显示详细输出。
...
.NOTES
    无法移除使用 Import-Module 的 PassThru 参数添加的模块。

.EXAMPLE
    PS C:\> Remove-Module -Name MyModule
    此命令从当前会话中移除名为 MyModule 的模块。

.EXAMPLE
    PS C:\> Get-Module | Where-Object {$_.Name -like "*Test*"} | Remove-Module
    此命令移除所有名称包含“Test”的模块。
...
| `-Instructions` | String | — | — | 1 | `('Check and correct any spelling or grammar ' +
            ...
            'additional comments or explanations.')` | 你是一个设计为输出JSON的有帮助的助手。
## 响应格式

仅回复有效的JSON对象。不允许其他文本。
不要在JSON前后包含任何解释、评论或文本。
你的响应必须严格符合此架构的可解析JSON。

示例响应格式：{"response":"你的实际响应在此"} |
