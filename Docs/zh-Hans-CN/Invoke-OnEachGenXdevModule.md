...
> 在工作区中的每个 GenXdev 模块上执行脚本块。

## Description

此函数遍历工作区中的 GenXdev 模块，并对每个模块执行提供的脚本块。它可以按名称模式筛选模块、排除本地模块、仅包含已发布的模块，或处理脚本而非模块。该函数在执行脚本块之前会自动导航到正确的模块目录。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Script` | ScriptBlock | ✅ | — | 0 | — | 每个 GenXdev 模块要执行的脚本块 |
| `-ModuleName` | String[] | — | — | 1 | `@('GenXdev*')` | 要应用于模块名称的筛选器 🌐 *Supports wildcards* |
| `-NoLocal` | SwitchParameter | — | — | Named | — | 从处理中排除本地开发模块 |
| `-OnlyPublished` | SwitchParameter | — | — | Named | — | 仅包含已发布且拥有LICENSE和README.md文件的模块 |
...
> 仅包含已发布且拥有LICENSE和README.md文件的模块
