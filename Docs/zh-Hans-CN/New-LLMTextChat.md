...
启动一个具有AI能力的交互式聊天会话，允许用户在对话过程中添加或删除PowerShell函数并执行PowerShell命令。此函数提供了AI驱动对话的综合界面，具备广泛的工具集成和自定义选项。
...
此示例在 HKCU:\Software\MyApp 下创建或修改名为 "Count" 的 DWORD 值，数据为 10。 *(Parameter set: )* |
...
| `-MarkupBlocksTypeFilter` | String[] | — | — | Named | `@('json', 'powershell', 'C#', 'python', 'javascript', 'typescript', 'html', 'css', 'yaml', 'xml', 'bash')` | 将仅输出指定类型的标记块 |
...
| `-NoConfirmationToolFunctionNames` | String[] | — | — | Named | — | 不应要求确认的工具函数名称 |
| `-MaxToolcallBackLength` | Int32 | — | — | Named | — | 工具回调响应的最大长度 |
| `-AudioTemperature` | Object | — | — | Named | — | 音频生成的温度 |
| `-TemperatureResponse` | Object | — | — | Named | — | 响应的温度 |
| `-Language` | Object | — | — | Named | — | 模型或输出的语言 |
...
| `-LogProbThreshold` | Object | — | — | Named | — | 输出过滤的对数概率阈值 |
| `-NoSpeechThreshold` | Object | — | — | Named | — | 音频检测无语音阈值 |
| `-DontSpeak` | Object | — | — | Named | — | 禁用语音输出 |
| `-DontSpeakThoughts` | Object | — | — | Named | — | 禁用思维的语音输出 |
| `-NoVOX` | Object | — | — | Named | — | 禁用 VOX（语音激活） |
| `-UseDesktopAudioCapture` | Object | — | — | Named | — | 使用桌面音频捕获 |
| `-NoContext` | Object | — | — | Named | — | 禁用上下文使用 |
| `-WithBeamSearchSamplingStrategy` | Object | — | — | Named | — | 使用束搜索采样策略 |
| `-OnlyResponses` | Object | — | — | Named | — | .PARAMETER 强制
指示此 cmdlet 在未提示确认的情况下强制安装模块。如果模块已安装，则会覆盖现有版本。 |
