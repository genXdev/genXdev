...
> 为给定的搜索词或URL打开所有可能的查询类型。

## Description

执行所有处理网页查询的 cmdlet，同时处理 URL 和搜索词。对于 URL，它执行特定于站点的查询；对于搜索词，它执行常规的 Web 查询。此函数动态发现并执行 GenXdev.Queries 模块系列中的所有可用查询 cmdlet。
...
| `-ClearSession` | SwitchParameter | — | — | Named | — | 在打开浏览器之前清除会话。 |
| `-SkipSession` | SwitchParameter | — | — | Named | — | 跳过浏览器会话恢复。 |
...
在默认监视器上打开 "powershell scripting" 的所有可能的查询类型。
...
使用别名和短参数打开 GitHub URL 的所有可能查询类型。
...
- [ConvertTo-Uris](https://github.com/genXdev/genXdev/blob/main/Docs/zh-Hans-CN/ConvertTo-Uris.md)
