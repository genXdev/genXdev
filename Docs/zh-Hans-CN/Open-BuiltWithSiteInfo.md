...
> 在网页浏览器中打开 BuiltWith 网站查询。

## Description

在网络浏览器中打开 BuiltWith 查询，具有可配置的监控设置和浏览器行为选项。该函数通过查询 BuiltWith.com 的技术分析服务来分析网站技术栈，该服务提供关于网站使用的网络技术、框架和服务的详细信息。

BuiltWith.com 是一个全面的技术查找服务，能够识别任何网站使用的网络技术、框架、内容管理系统、分析工具、托管提供商和其他软件组件。该函数提供了一个 PowerShell 接口，可以快速分析技术栈，而无需手动导航到 BuiltWith 网站。
...
| `-Monitor` | Int32 | — | — | 2 | `-1` | 要使用的显示器，0 = 默认，-1 = 丢弃，-2 = 已配置的辅助显示器，默认值为 -1，无定位 |
...
在默认监视器上打开 Microsoft.com 的 BuiltWith 技术分析。
...
使用管道输入分析 Microsoft.com，并丢弃监视器定位。

## Parameter Details

### `-Queries <String[]>`

> 要在BuiltWith.com上查询的网站URL或域名。
