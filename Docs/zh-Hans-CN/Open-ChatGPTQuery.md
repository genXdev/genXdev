...
> 在网络浏览器中打开 ChatGPT 查询。

## Description

在网页浏览器中打开ChatGPT，自动输入指定的查询并提交。支持多个查询，这些查询将使用底层的Open-WebsiteAndPerformQuery函数来执行。此函数提供了一种便捷的方式，通过PowerShell与OpenAI的ChatGPT进行交互，并拥有丰富的浏览器配置选项。
