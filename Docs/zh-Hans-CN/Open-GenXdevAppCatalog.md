...
> 在网页浏览器中打开 GenXdev 渐进式网络应用目录。

## Description

在网页浏览器中打开已发布的 GenXdev 渐进式 Web 应用目录。默认以全屏应用模式打开，提供最佳的目录浏览体验。支持所有标准网页浏览器配置选项，包括显示器位置、窗口大小和语言偏好。此功能会自动配置浏览器以获得最佳目录浏览体验，并能同时打开多个浏览器实例。它提供全面的语言支持，并尊重用户对浏览器选择和定位的偏好。
