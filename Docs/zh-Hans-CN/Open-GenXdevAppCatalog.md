...
> 在网页浏览器中打开GenXdev渐进式网络应用目录。

## Description

在网页浏览器中打开已发布的 GenXdev 渐进式网页应用目录。默认以全屏应用模式打开，以提供最佳的目录浏览体验。支持所有标准网页浏览器配置选项，包括显示器位置、窗口大小和语言偏好。该功能会自动配置浏览器以获得最佳目录浏览体验，并可同时打开多个浏览器实例。它提供全面的语言支持，并尊重用户对浏览器选择和位置设置的偏好。
...
| `-ShowWindow` | SwitchParameter | — | — | Named | — | 显示浏览器窗口（未最小化或隐藏） |
...
在显示器1上以应用模式打开GenXdev应用目录，并启用全屏。
...
opens GenXdev app catalog on monitor 1
