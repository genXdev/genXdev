...
> 在网页浏览器中打开 GitHub Copilot 查询

## Description

在网页浏览器中打开 GitHub Copilot 查询，并自动输入指定的查询内容。该函数会等待页面加载完成后再输入查询。此函数利用底层的 Open-WebsiteAndPerformQuery 函数进行浏览器自动化，并通过丰富的浏览器配置选项提供了一种便捷的方式，从 PowerShell 与 GitHub Copilot 进行交互。
