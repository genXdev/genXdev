...
> 在网页浏览器中打开谷歌查询，提供可配置的设置和广泛的定制选项。

## Description

在Web浏览器中打开一个或多个Google查询，全面支持多种语言、显示器选择、窗口定位、浏览器选择和键盘自动化。该功能提供了丰富的界面，用于执行Google搜索，并可精确控制浏览器行为和外观。

主要功能包括：
- 通过Google语言过滤器支持100多种语言
- 多显示器支持，可自动或手动选择显示器
- 窗口定位（左、右、上、下、居中、全屏）
- 支持隐私/隐身浏览模式
- 应用模式，实现无干扰浏览
...
该函数自动为Google搜索引擎格式化搜索查询，并在指定时应用语言过滤。所有浏览器定位和管理功能均继承自底层的Open-Webbrowser函数。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 用于在 Google 上查询的搜索词 |
...
在主显示器上打开一个关于“PowerShell 脚本”的英文谷歌搜索。
...
通过管道输入在Chrome中打开多个谷歌搜索，每个搜索均为英文。
