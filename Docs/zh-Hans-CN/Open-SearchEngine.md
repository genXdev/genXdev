...
> 在指定的搜索引擎中使用 Web 浏览器打开搜索查询。

## Description

此函数提供统一的接口，可在多个搜索引擎中进行搜索，包括Google、Bing、GitHub、Wikipedia、YouTube等。它支持广泛的浏览器配置选项，包括窗口定位、大小、浏览器选择和语言偏好。该函数会动态地将查询路由到相应的搜索引擎特定函数。
...
| `-EndPoint` | String | — | — | 1 | `'Google'` | 用于执行查询的端点 |
| `-Language` | String | — | — | 2 | — | 返回的搜索结果的語言 |
| `-Monitor` | Int32 | — | — | 3 | `-1` | 要使用的显示器，0 = 默认，-1 = 丢弃，-2 = 已配置的辅助显示器，默认值为 -1，无定位 |
...
在 Google 上搜索英文结果的“PowerShell cmdlets”。
...
使用别名 'q' 在 Chrome 浏览器的隐私模式下于 Bing 搜索 "GitHub PowerShell"。
