...
> 在网页浏览器中打开指定 URL 的 SimilarWeb 网站信息。

## Description

以可配置的方式通过命令行开关在网页浏览器中打开"Similar web"查询。用于检查网站分析数据和进行对比，以进行网站流量分析、竞争对手研究和数字营销洞察。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 在 SimilarWeb 上分析的网站 URL |
...
在主显示器上打开 Google.com 的 SimilarWeb 分析。
...
使用附带监视器定位的函数别名打开 SimilarWeb 分析。
...
返回 GitHub 的 SimilarWeb URL，且不打开浏览器。

## Parameter Details

### `-Queries <String[]>`

> 在 SimilarWeb 上分析的网站 URL
