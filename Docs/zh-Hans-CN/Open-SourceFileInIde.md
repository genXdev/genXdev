...
此函数在 Visual Studio Code 或 Visual Studio 中打开指定的源文件。它可以直接跳转到特定的行号，并可选地在打开文件后向 IDE 发送键盘输入。该函数将根据当前宿主进程、正在运行的应用程序或用户选择来确定使用哪个 IDE。
...
| `-KeysToSend` | String[] | — | — | 2 | `@()` | 要发送到应用程序的键盘输入数组 |
| `-Code` | SwitchParameter | — | — | Named | — | 用于打开文件的IDE |
| `-VisualStudio` | SwitchParameter | — | — | Named | — | 在 Visual Studio 中打开 |
| `-SendKeyEscape` | SwitchParameter | — | — | Named | — | 转义控制字符和修饰符 |
| `-SendKeyHoldKeyboardFocus` | SwitchParameter | — | — | Named | — | 将键盘焦点保持于目标窗口 |
| `-SendKeyUseShiftEnter` | SwitchParameter | — | — | Named | — | 使用 Shift+Enter 代替 Enter |
