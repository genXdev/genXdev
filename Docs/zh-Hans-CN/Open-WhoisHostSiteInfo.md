...
> 在网络浏览器中打开 Whois 主机信息查询。

## Description

在网络浏览器中打开域名或IP地址的Whois主机信息查询。支持可配置的监视器选择和多查询，具有广泛的浏览器配置选项，包括窗口定位、浏览器选择和自动化功能。
...
| `-SessionOnly` | SwitchParameter | — | — | Named | — | 使用会话中存储的替代设置来调整AI偏好，如语言、图片集等。 |
| `-ClearSession` | SwitchParameter | — | — | Named | — | 清除会话中存储的AI偏好（如语言、图像集合等）的备用设置。 |
| `-SkipSession` | SwitchParameter | — | — | Named | — | 仅将设置存储在持久化首选项中，而不影响会话中的AI偏好设置，如语言、图像集合等。 |
...
使用别名和位置参数打开 example.com 的 Whois 信息。
...
以 Chrome 隐私模式打开管道中域名的 Whois 信息。
...
> 仅将设置存储在持久化首选项中，而不影响会话中的AI偏好设置，如语言、图像集合等。
