...
在网页浏览器中打开一个或多个维基百科搜索查询，提供丰富的配置选项。支持可配置的浏览器行为、显示器选择和窗口定位。查询将进行URL编码，并使用指定语言的维基百科域名打开，支持完整的本地化功能。

该函数提供全面的浏览器控制，包括隐私浏览模式、窗口定位、键盘自动化和多显示器支持。它会自动处理国际维基百科域名的URL编码和语言代码映射。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Queries` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 要执行的维基百科搜索查询 |
| `-Language` | String | — | — | 1 | — | 维基百科搜索结果的语言 |
