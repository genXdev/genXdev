...
> 在网络浏览器中打开 YouTube 搜索查询。

## Description

在网页浏览器中打开一个或多个YouTube搜索查询，提供丰富的定位和配置选项。支持可配置的显示器选择、浏览器设置，并通过继承自Open-Webbrowser cmdlet的动态参数实现按键自动化。每个查询在独立的浏览器实例中打开，可完全控制窗口位置、浏览器类型和搜索语言。
...
在默认监视器上打开 YouTube 搜索“PowerShell 教程”。
...
使用函数别名，在配置好的副显示器上打开 YouTube 搜索“如何编程”
