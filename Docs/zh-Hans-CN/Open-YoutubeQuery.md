...
> 在网络浏览器中打开YouTube搜索查询。

## Description

在一个网络浏览器中打开一个或多个YouTube搜索查询，并提供广泛的定位和配置选项。支持可配置的显示器选择、浏览器设置以及通过继承自Open-Webbrowser cmdlet的动态参数实现的键盘自动化。每个查询在单独的浏览器实例中打开，可完全控制窗口位置、浏览器类型和搜索语言。
...
在已配置的副显示器上使用函数别名打开 YouTube 搜索“如何编程”
