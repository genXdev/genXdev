...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-ImageDirectories` | String[] | — | — | 0 | `@(".\")` | 用于处理图像元数据删除的目录路径数组。如果未指定，则使用默认系统目录。 |
| `-Language` | String | — | — | 1 | — | 用于移除特定语言元数据文件的语言。 |
| `-PreferencesDatabasePath` | String | — | — | 2 | — | 偏好数据文件的数据库路径 |
| `-Recurse` | SwitchParameter | — | — | Named | — | 递归目录。 |
| `-OnlyKeywords` | SwitchParameter | — | — | Named | — | 仅移除 description.json 文件（关键词/描述）。 |
| `-OnlyPeople` | SwitchParameter | — | — | Named | — | 仅移除people.json文件（面部识别数据）。 |
| `-OnlyObjects` | SwitchParameter | — | — | Named | — | 仅删除objects.json文件（目标检测数据）。 |
| `-OnlyScenes` | SwitchParameter | — | — | Named | — | 仅移除场景分类数据文件（scenes.json）。 |
| `-AllLanguages` | SwitchParameter | — | — | Named | — | 删除所有支持语言的元数据文件。 |
...
仅从 MyPhotos 目录中删除 people.json 文件。
...
> 用于处理图像元数据删除的目录路径数组。如果未指定，则使用默认系统目录。
...
> 仅移除 description.json 文件（关键词/描述）。
