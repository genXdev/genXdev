...
* 从指定显示器截取桌面屏幕截图并保存为图像文件。
* 支持多个显示器，并自动检测高DPI显示器的正确屏幕边界和缩放因子。
...
| `-OutputPath` | String | ✅ | ✅ (ByPropertyName) | 1 | — | 截图将保存到的完整路径 |
...
> 要捕获的显示器索引（0=主显示器，1+=特定显示器，-1=所有显示器）
