...
> 发送一个Wake-on-LAN魔法数据包以唤醒网络上的远程计算机。

## Description

构造并向指定的 MAC 地址广播唤醒局域网（WOL）魔包。魔包是一个 UDP 广播，包含目标 MAC 地址重复 16 次，能触发支持的网络接口启动计算机。支持自定义广播地址和端口。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-MacAddress` | String[] | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 一个或多个用于发送魔包的目标MAC地址（例如，00:11:22:33:44:55） |
| `-BroadcastAddress` | String | — | — | 1 | `"255.255.255.255"` | 发送魔法数据包（Magic Packet）的广播 IP 地址（默认值：255.255.255.255） |
| `-Port` | Int32 | — | — | 2 | `4000` | 发送魔术包所使用的UDP端口（默认值：4000） |
...
通过管道输入向多台计算机的第9端口发送魔术包。

## Parameter Details

### `-MacAddress <String[]>`

> 一个或多个用于发送魔包的目标MAC地址（例如，00:11:22:33:44:55）
...
> 发送魔法数据包（Magic Packet）的广播 IP 地址（默认值：255.255.255.255）
