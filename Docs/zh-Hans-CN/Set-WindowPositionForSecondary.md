...
此函数允许通过 Set-WindowPosition 在已配置的辅助显示器上定位窗口，提供包括对齐、大小和边框设置在内的多种布局选项。它基于全局配置处理显示器选择，并为辅助显示器窗口定位提供了便捷的包装器。
...
| `-PassThru` | SwitchParameter | — | — | Named | — | 定位后返回进程对象 |
| `-SideBySide` | SwitchParameter | — | — | Named | — | 在同一台显示器上将窗口与 PowerShell 并排放置 |
