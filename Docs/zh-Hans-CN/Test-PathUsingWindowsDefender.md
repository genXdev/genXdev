...
* 使用 Windows Defender 的命令行界面（MpCmdRun.exe）对指定文件或目录执行定向扫描。
* 可以仅在检测模式下扫描，或启用自动威胁修复功能。
* 如果未检测到威胁则返回 true，如果发现威胁或扫描失败则返回 false。
...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-FilePath` | String | ✅ | ✅ (ByValue, ByPropertyName) | 0 | — | 要扫描的文件或目录路径 |
| `-EnableRemediation` | SwitchParameter | — | — | Named | `False` | 指示 Windows Defender 对威胁采取措施 |
