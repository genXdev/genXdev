...
|:---|:---|:---:|:---|:---:|:---|:---|
| `-Name` | String[] | — | ✅ (ByValue, ByPropertyName) | 0 | `@('*')` | 重构的名称，支持通配符 *(Parameter set: )* 🌐 *Supports wildcards* |
| `-Refactor` | GenXdev.Helpers.RefactorDefinition[] | — | — | 1 | — | 要更新的重构集合 *(Parameter set: )* |
| `-FilesToAdd` | IO.FileInfo[] | — | — | 2 | `@()` | 要添加的文件名 |
| `-FilesToRemove` | IO.FileInfo[] | — | — | 3 | `@()` | 要删除的文件名 |
| `-SelectByModifiedDateFrom` | DateTime | — | — | Named | — | 按修改日期筛选文件，从 |
| `-SelectByModifiedDateTo` | DateTime | — | — | Named | — | 按修改日期选择文件 |
| `-SelectByCreationDateFrom` | DateTime | — | — | Named | — | 按创建日期从以下位置选择文件 |
| `-SelectByCreationDateTo` | DateTime | — | — | Named | — | 按创建日期选择文件 |
| `-PromptKey` | String | — | — | Named | — | prompt键指示要使用哪个提示脚本 |
| `-Prompt` | String | — | — | Named | `''` | prompt键指示要使用哪个提示脚本 |
| `-SelectionScript` | String | — | — | Named | — | 用于选择重构项的 PowerShell 脚本函数 |
| `-SelectionPrompt` | String | — | — | Named | — | 如果提供，将调用LLM根据脚本内容进行选择 |
...
| `-Priority` | Int32 | — | — | Named | — | 本次重构集的优先级 |
| `-ExposedCmdLets` | GenXdev.Helpers.ExposedCmdletDefinition[] | — | — | Named | `@()` | LLM 选择期间用作工具的 PowerShell 命令定义数组 |
| `-KeysToSend` | String[] | — | — | Named | `@()` | 打开文件后要作为按键敲击来调用的键位 |
| `-PreferencesDatabasePath` | String | — | — | Named | — | 偏好数据文件的数据库路径 |
| `-CleanUpDeletedFiles` | SwitchParameter | — | — | Named | — | 清理已删除的文件 |
| `-AskBeforeLLMSelection` | SwitchParameter | — | — | Named | — | 切换到禁止用户交互 |
| `-PerformAutoSelections` | SwitchParameter | — | — | Named | — | 启用基于 LLM 的文件选择处理 |
...
| `-ReprocessModifiedFiles` | SwitchParameter | — | — | Named | — | 自动重新处理自上次更新以来修改过的文件 |
...
> LLM 选择期间用作工具的 PowerShell 命令定义数组
