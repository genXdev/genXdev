Pester\Describe 'Copy-FilesToDateFolder' {
    ...
    Pester\It 'Copies file into the correct date folder when moving' {
        ...
        GenXdev\Copy-FilesToDateFolder -TargetFolder $Script:dest -Root $Script:source -Name '*.txt' -NoRecurse -Confirm:$false
    ...
    Pester\It 'Does not copy files when WhatIf is specified' {
        $null = GenXdev\Copy-FilesToDateFolder -TargetFolder $Script:dest -Root $Script:source -Name '*.txt' -WhatIf -NoRecurse -Confirm:$false -ErrorAction SilentlyContinue
